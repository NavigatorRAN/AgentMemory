...
- Requested URL: https://docs.openclaw.ai/plugins/google-meet
- Final URL: https://docs.openclaw.ai/plugins/google-meet
- Canonical URL: https://docs.openclaw.ai/plugins/google-meet
- Fetched at: 2026-06-23T14:50:17Z
...
bash Copy code brew install blackhole-2ch sox export OPENAI_API_KEY=sk-... # only needed when realtime.voiceProvider is "google" for bidi mode export GEMINI_API_KEY=...
...
bash Copy code sudo reboot
After reboot, verify both pieces:
bash Copy code system_profiler SPAudioDataType | grep -i BlackHole command -v sox
Enable the plugin:
json5 Copy code { plugins : { entries : { "google-meet" : { enabled : true , config : {}, }, }, }, }
Check setup:
bash Copy code openclaw googlemeet setup
...
bash Copy code openclaw googlemeet setup --transport chrome-node --mode transcribe
...
is Chrome:
bash Copy code openclaw googlemeet setup --transport twilio
...
Join a meeting:
bash Copy code openclaw googlemeet join https://meet.google.com/abc-defg-hij
Or let an agent join through the google_meet tool:
json Copy code { "action" : "join" , "url" : "https://meet.google.com/abc-defg-hij" , "transport" : "chrome-node" , "mode" : "agent" }
...
bash Copy code openclaw googlemeet create --transport chrome-node --mode agent
...
account defaults:
bash Copy code openclaw googlemeet create --access-type OPEN --transport chrome-node --mode agent
...
bash Copy code openclaw googlemeet create --no-join
...
json Copy code { "action" : "create" , "transport" : "chrome-node" , "mode" : "agent" }
...
bash Copy code brew install blackhole-2ch sox
...
bash Copy code openclaw plugins enable google-meet
Start the node host in the VM:
bash Copy code openclaw node run --host <gateway-host> --port 18789 --display-name parallels-macos
...
bash Copy code OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \ openclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
Use the same environment variable when installing the node as a LaunchAgent:
bash Copy code OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \ openclaw node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force openclaw node restart
...
bash Copy code openclaw devices list openclaw devices approve <requestId>
Confirm the Gateway sees the node and that it advertises both googlemeet.chrome
and browser capability/ browser.proxy :
bash Copy code openclaw nodes status
Route Meet through that node on the Gateway host:
json5 Copy code { gateway : { nodes : { allowCommands : [ "googlemeet.chrome" , "browser.proxy" ], }, }, plugins : { entries : { "google-meet" : { enabled : true , config : { defaultTransport : "chrome-node" , chrome : { guestName : "OpenClaw Agent" , autoJoin : true , reuseExistingTab : true , }, chromeNode : { node : "parallels-macos" , }, }, }, }, }, }
...
bash Copy code openclaw googlemeet test-speech https://meet.google.com/abc-defg-hij
...
chrome-node hosts.
bash Copy code openclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome openclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
...
Then verify:
bash Copy code openclaw config validate openclaw plugins list | grep -E 'google-meet|voice-call' openclaw googlemeet setup
...
bash Copy code openclaw googlemeet join https://meet.google.com/abc-defg-hij \ --transport twilio \ --dial-in-number +15551234567 \ --pin 123456
Use --dtmf-sequence when the meeting needs a custom sequence:
bash Copy code openclaw googlemeet join https://meet.google.com/abc-defg-hij \ --transport twilio \ --dial-in-number +15551234567 \ --dtmf-sequence ww123456#
OAuth and preflight
...
text Copy code http://localhost:8085/oauth2callback
...
bash Copy code openclaw googlemeet auth login --json
...
Examples:
bash Copy code OPENCLAW_GOOGLE_MEET_CLIENT_ID= "your-client-id" \ OPENCLAW_GOOGLE_MEET_CLIENT_SECRET= "your-client-secret" \ openclaw googlemeet auth login --json
Use manual mode when the browser cannot reach the local callback:
bash Copy code OPENCLAW_GOOGLE_MEET_CLIENT_ID= "your-client-id" \ OPENCLAW_GOOGLE_MEET_CLIENT_SECRET= "your-client-secret" \ openclaw googlemeet auth login --json --manual
The JSON output includes:
json Copy code { "oauth" : { "clientId" : "your-client-id" , "clientSecret" : "your-client-secret" , "refreshToken" : "refresh-token" , "accessToken" : "access-token" , "expiresAt" : 1770000000000 } , "scope" : "..." }
Store the oauth object under the Google Meet plugin config:
json5 Copy code { plugins : { entries : { "google-meet" : { enabled : true , config : { oauth : { clientId : "your-client-id" , clientSecret : "your-client-secret" , refreshToken : "refresh-token" , }, }, }, }, }, }
...
bash Copy code openclaw googlemeet doctor --oauth --json
...
bash Copy code openclaw googlemeet doctor --oauth --create-space --json openclaw googlemeet create --no-join --json
...
bash Copy code openclaw googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json openclaw googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
...
space. A refresh-token error means rerun openclaw googlemeet auth login --json and store the new oauth block.
...
bash Copy code openclaw googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
Run preflight before media work:
bash Copy code openclaw googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
List meeting artifacts and attendance after Meet has created conference records:
bash Copy code openclaw googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij openclaw googlemeet attendance --meeting https://meet.google.com/abc-defg-hij openclaw googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
...
Meet artifacts:
bash Copy code openclaw googlemeet latest --today openclaw googlemeet calendar-events --today --json openclaw googlemeet artifacts --event "Weekly sync" openclaw googlemeet attendance --today --format csv --output attendance.csv
...
bash Copy code openclaw googlemeet latest --meeting https://meet.google.com/abc-defg-hij openclaw googlemeet artifacts --conference-record conferenceRecords/abc123 --json openclaw googlemeet attendance --conference-record conferenceRecords/abc123 --json
End an active conference for an API-created space when you want to close the
room after the call:
bash Copy code openclaw googlemeet end-active-conference https://meet.google.com/abc-defg-hij
...
json Copy code { "action" : "export" , "conferenceRecord" : "conferenceRecords/abc123" , "includeDocumentBodies" : true , "outputDir" : "meet-export" , "zip" : true }
...
json Copy code { "action" : "create" , "transport" : "chrome-node" , "mode" : "agent" , "accessType" : "OPEN" }
And they can end the active conference for a known room:
json Copy code { "action" : "end_active_conference" , "meeting" : "https://meet.google.com/abc-defg-hij" }
For listen-first validation, agents should use test_listen before claiming the
meeting is useful:
json Copy code { "action" : "test_listen" , "url" : "https://meet.google.com/abc-defg-hij" , "transport" : "chrome-node" , "timeoutMs" : 30000 }
Run the guarded live smoke against a real retained meeting:
bash Copy code OPENCLAW_LIVE_TEST=1 \ OPENCLAW_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \ pnpm test :live -- extensions/google-meet/google-meet.live.test.ts
Run the live listen-first browser probe against a meeting where someone will
speak with Meet captions available:
bash Copy code openclaw googlemeet setup --transport chrome-node --mode transcribe openclaw googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
...
bash Copy code openclaw googlemeet create
...
json Copy code { "source" : "browser" , "meetingUri" : "https://meet.google.com/abc-defg-hij" , "joined" : true , "browser" : { "nodeId" : "ba0f4e4bc..." , "targetId" : "tab-1" } , "join" : { "session" : { "id" : "meet_..." , "url" : "https://meet.google.com/abc-defg-hij" } } }
...
json Copy code { "source" : "browser" , "error" : "google-login-required: Sign in to Google in the OpenClaw browser profile, then retry meeting creation." , "manualActionRequired" : true , "manualActionReason" : "google-login-required" , "manualActionMessage" : "Sign in to Google in the OpenClaw browser profile, then retry meeting creation." , "browser" : { "nodeId" : "ba0f4e4bc..." , "targetId" : "tab-1" , "browserUrl" : "https://accounts.google.com/signin" , "browserTitle" : "Sign in - Google Accounts" } }
...
json Copy code { "source" : "api" , "meetingUri" : "https://meet.google.com/abc-defg-hij" , "joined" : true , "space" : { "name" : "spaces/abc-defg-hij" , "meetingCode" : "abc-defg-hij" , "meetingUri" : "https://meet.google.com/abc-defg-hij" } , "join" : { "session" : { "id" : "meet_..." , "url" : "https://meet.google.com/abc-defg-hij" } } }
...
bash Copy code brew install blackhole-2ch sox export OPENAI_API_KEY=sk-... # or export GEMINI_API_KEY=...
...
Optional overrides:
json5 Copy code { defaults : { meeting : "https://meet.google.com/abc-defg-hij" , }, browser : { defaultProfile : "openclaw" , }, chrome : { guestName : "OpenClaw Agent" , waitForInCallMs : 30000 , bargeInInputCommand : [ "sox" , "-q" , "-t" , "coreaudio" , "External Microphone" , "-r" , "24000" , "-c" , "1" , "-b" , "16" , "-e" , "signed-integer" , "-t" , "raw" , "-" , ], }, chromeNode : { node : "parallels-macos" , }, defaultMode : "agent" , realtime : { provider : "openai" , transcriptionProvider : "openai" , voiceProvider : "google" , model : "gemini-2.5-flash-native-audio-preview-12-2025" , agentId : "jay" , toolPolicy : "owner" , introMessage : "Say exactly: I'm here." , providers : { google : { speakerVoice : "Kore" , }, }, }, }
ElevenLabs for both agent-mode listening and speaking:
json5 Copy code { messages : { tts : { provider : "elevenlabs" , providers : { elevenlabs : { modelId : "eleven_v3" , speakerVoiceId : "pMsXgVXv3BLzUgSXRplE" , }, }, }, }, plugins : { entries : { "google-meet" : { config : { realtime : { transcriptionProvider : "elevenlabs" , providers : { elevenlabs : { modelId : "scribe_v2_realtime" , audioFormat : "ulaw_8000" , sampleRate : 8000 , commitStrategy : "vad" , }, }, }, }, }, }, }, }
...
Twilio-only config:
json5 Copy code { defaultTransport : "twilio" , twilio : { defaultDialInNumber : "+15551234567" , defaultPin : "123456" , }, voiceCall : { gatewayUrl : "ws://127.0.0.1:18789" , }, }
...
json Copy code { "action" : "speak" , "sessionId" : "meet_..." , "message" : "Say exactly: I'm here and listening." }
...
Mode Who decides the answer Speech output path Use when agent The configured OpenClaw agent Normal OpenClaw TTS runtime You want "my agent is in the meeting" behavior bidi The realtime voice model Realtime voice provider audio response You want the lowest-latency conversational voice loop
...
bash Copy code openclaw googlemeet speak meet_... "Say exactly: I'm here and listening."
For the full join-and-speak smoke:
bash Copy code openclaw googlemeet test-speech https://meet.google.com/abc-defg-hij \ --transport chrome-node \ --message "Say exactly: I'm here and listening."
Live test checklist
Use this sequence before handing a meeting to an unattended agent:
bash Copy code openclaw googlemeet setup openclaw nodes status openclaw googlemeet test-speech https://meet.google.com/abc-defg-hij \ --transport chrome-node \ --message "Say exactly: Google Meet speech test complete."
...
bash Copy code openclaw googlemeet setup openclaw nodes status --connected openclaw nodes invoke \ --node parallels-macos \ -- command googlemeet.chrome \ --params '{"action":"setup"}'
...
bash Copy code openclaw googlemeet setup openclaw googlemeet join https://meet.google.com/abc-defg-hij \ --transport twilio \ --dial-in-number +15551234567 \ --pin 123456
...
bash Copy code openclaw plugins list | grep google-meet openclaw googlemeet setup
...
bash Copy code openclaw plugins enable google-meet openclaw plugins enable browser OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \ openclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
On the Gateway host, approve the node and verify commands:
bash Copy code openclaw devices list openclaw devices approve <requestId> openclaw nodes status
...
json5 Copy code { gateway : { nodes : { allowCommands : [ "browser.proxy" , "googlemeet.chrome" ], }, }, }
...
bash Copy code OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \ openclaw node install \ --host <gateway-lan-ip> \ --port 18789 \ --display-name parallels-macos \ --force
Then reload the node service and re-run:
bash Copy code openclaw googlemeet setup openclaw nodes status --connected
...
bash Copy code openclaw googlemeet setup openclaw googlemeet doctor
...
bash Copy code openclaw googlemeet recover-tab openclaw googlemeet recover-tab https://meet.google.com/abc-defg-hij
...
bash Copy code export TWILIO_ACCOUNT_SID=AC... export TWILIO_AUTH_TOKEN=... export TWILIO_FROM_NUMBER=+15550001234
...
json5 Copy code { plugins : { entries : { "voice-call" : { enabled : true , config : { provider : "twilio" , fromNumber : "+15550001234" , publicUrl : "https://voice.example.com/voice/webhook" , }, }, }, }, }
For local development, use a tunnel or Tailscale exposure instead of a private
host URL:
json5 Copy code { plugins : { entries : { "voice-call" : { config : { tunnel : { provider : "ngrok" }, // or tailscale : { mode : "funnel" , path : "/voice/webhook" }, }, }, }, }, }
Then restart or reload the Gateway and run:
bash Copy code openclaw googlemeet setup --transport twilio openclaw voicecall setup openclaw voicecall smoke
voicecall smoke is readiness-only by default. To dry-run a specific number:
bash Copy code openclaw voicecall smoke --to "+15555550123"
Only add --yes when you intentionally want to place a live outbound notify
call:
bash Copy code openclaw voicecall smoke --to "+15555550123" -- yes
...
either media-stream TTS playback or the Twilio OPENCLAW_DOCS_MARKER:calloutOpen:U2F5 fallback. If the call
...
See Voice call troubleshooting .
Notes
