...
- Requested URL: https://hermes-agent.nousresearch.com/docs/getting-started/installation
- Final URL: https://hermes-agent.nousresearch.com/docs/getting-started/installation
- Canonical URL: https://hermes-agent.nousresearch.com/docs/getting-started/installation
- Fetched at: 2026-06-23T14:14:45Z
- Content type: text/html; charset=utf-8

## Description

Install Hermes Agent on Linux, macOS, WSL2, native Windows, or Android via Termux
...
Installer Code lives at hermes binary Data directory pip install Python site-packages ~/.local/bin/hermes (console_scripts) ~/.hermes/ Per-user (git installer) ~/.hermes/hermes-agent/ ~/.local/bin/hermes (symlink) ~/.hermes/ Root-mode ( sudo curl … | sudo bash ) /usr/local/lib/hermes-agent/ /usr/local/bin/hermes /root/.hermes/ (or $HERMES_HOME )
...
Nix users
If you use Nix (on NixOS, macOS, or Linux), there's a dedicated setup path with a Nix flake, declarative NixOS module, and optional container mode. See the Nix & NixOS Setup guide.
...
Troubleshooting ​
Problem Solution hermes: command not found Reload your shell ( source ~/.bashrc ) or check PATH API key not set Run hermes model to configure your provider, or hermes config set OPENROUTER_API_KEY your_key Missing config after update Run hermes config check then hermes config migrate
