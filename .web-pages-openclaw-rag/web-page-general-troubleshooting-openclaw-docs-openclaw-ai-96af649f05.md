...
- Requested URL: https://docs.openclaw.ai/help/troubleshooting
- Final URL: https://docs.openclaw.ai/help/troubleshooting
- Canonical URL: https://docs.openclaw.ai/help/troubleshooting
- Fetched at: 2026-06-23T14:44:29Z
...
bash Copy code openclaw status openclaw status --all openclaw gateway probe openclaw gateway status openclaw doctor openclaw channels status --probe openclaw logs --follow
...
bash Copy code openclaw status openclaw status --all openclaw doctor
Common causes:
...
and run openclaw status --all again. See Tools for the profile
...
agent turns:
If the error mentions messages[].content expecting a string, set
models.providers.<provider>.models[].compat.requiresStringContent: true .
If the backend still fails only on OpenClaw agent turns, set
models.providers.<provider>.models[].compat.supportsTools: false and retry.
If tiny direct calls still work but larger OpenClaw prompts crash the
backend, treat the remaining issue as an upstream model/server limitation and
continue in the deep runbook:
/gateway/troubleshooting#local-openai-compatible-backend-passes-direct-probes-but-agent-runs-fail
...
Point entries at built runtime files (usually ./dist/index.js ).
Republish the plugin and run openclaw plugins install <package> again.
Example:
json Copy code { "name" : "@openclaw/my-plugin" , "version" : "1.2.3" , "openclaw" : { "extensions" : [ "./dist/index.js" ] } }
...
request.mode: "update" .
Recovery:
bash Copy code openclaw doctor --deep openclaw plugins update --all openclaw status --all
...
bash Copy code openclaw plugins inspect <plugin-id> --runtime --json openclaw plugins enable <plugin-id>
...
text Copy code blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root) plugin present but blocked
...
bash Copy code sudo chown -R 1000:1000 /path/to/openclaw-config /path/to/openclaw-workspace openclaw doctor --fix
If you intentionally run OpenClaw as root, repair the managed plugin root to
root ownership instead:
bash Copy code sudo chown -R root:root /path/to/openclaw-config/npm openclaw doctor --fix
Deeper docs:
...
No replies bash Copy code openclaw status openclaw gateway status openclaw channels status --probe openclaw pairing list --channel <channel> [--account < id >] openclaw logs --follow
...
/channels/pairing
Dashboard or Control UI will not connect bash Copy code openclaw status openclaw gateway status openclaw logs --follow openclaw doctor openclaw channels status --probe
...
Gateway will not start or service installed but not running bash Copy code openclaw status openclaw gateway status openclaw logs --follow openclaw doctor openclaw channels status --probe
Service: ... (loaded)
...
Channel connects but messages do not flow bash Copy code openclaw status openclaw gateway status openclaw logs --follow openclaw doctor openclaw channels status --probe
...
Cron or heartbeat did not fire or did not deliver bash Copy code openclaw status openclaw gateway status openclaw cron status openclaw cron list openclaw cron runs -- id <jobId> -- limit 20 openclaw logs --follow
...
/gateway/heartbeat
Node is paired but tool fails camera canvas screen exec bash Copy code openclaw status openclaw gateway status openclaw nodes status openclaw nodes describe --node <idOrNameOrIp> openclaw logs --follow
...
Exec suddenly asks for approval bash Copy code openclaw config get tools.exec.host openclaw config get tools.exec.security openclaw config get tools.exec.ask openclaw gateway restart
What changed:
...
bash Copy code openclaw config set tools.exec.host gateway openclaw config set tools.exec.security full openclaw config set tools.exec.ask off openclaw gateway restart
Safer alternatives:
...
Browser tool fails bash Copy code openclaw status openclaw gateway status openclaw browser status openclaw logs --follow openclaw doctor
