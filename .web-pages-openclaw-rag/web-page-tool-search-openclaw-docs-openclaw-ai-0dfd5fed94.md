# Tool Search - OpenClaw

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.openclaw.ai/tools/tool-search
- Final URL: https://docs.openclaw.ai/tools/tool-search
- Canonical URL: https://docs.openclaw.ai/tools/tool-search
- Fetched at: 2026-06-23T14:58:25Z
...
js Copy code const hits = await openclaw. tools . search ( "create a GitHub issue" ); const tool = await openclaw. tools . describe (hits[ 0 ]. id ); return await openclaw. tools . call (tool. id , { title : "Crash on startup" , body : "Steps to reproduce..." , });
...
Expose the OpenClaw code bridge, the structured fallback tools, or the
...
js Copy code const hits = await openclaw. tools . search ( "calendar event" , { limit : 5 });
openclaw.tools.describe(id)
Loads full metadata for one search result, including the exact input schema.
js Copy code const calendarCreate = await openclaw. tools . describe ( "mcp:calendar:create_event" );
openclaw.tools.call(id, args)
Calls a selected tool through OpenClaw.
js Copy code await openclaw. tools . call (calendarCreate. id , { summary : "Planning" , start : "2026-05-09T14:00:00Z" , });
...
bash Copy code openclaw config set tools.toolSearch true
Equivalent JSON:
json5 Copy code { tools : { toolSearch : true , }, }
Use the structured fallback tools instead for OpenClaw runs:
json5 Copy code { tools : { toolSearch : { mode : "tools" , }, }, }
Use the compact directory surface instead for OpenClaw runs:
json5 Copy code { tools : { toolSearch : { mode : "directory" , }, }, }
Tune code-mode timeout and search result limits:
json5 Copy code { tools : { toolSearch : { mode : "code" , codeTimeoutMs : 10000 , searchDefaultLimit : 8 , maxSearchLimit : 20 , }, }, }
Disable it:
json5 Copy code { tools : { toolSearch : false , }, }
...
bash Copy code node --import tsx scripts/tool-search-gateway-e2e.ts
