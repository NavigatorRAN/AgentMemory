# Weixin (WeChat) | Hermes Agent

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/weixin
- Final URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/weixin
- Canonical URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/weixin
- Fetched at: 2026-06-23T14:17:21Z
- Content type: text/html; charset=utf-8

## Description

Connect Hermes Agent to personal WeChat accounts via the iLink Bot API
...
iLink bot identity — ordinary WeChat groups may not work
...
Pair Hermes once with hermes gateway setup and note the connected iLink bot
account.
Have each allowed user send a direct message to that bot/contact.
Read the sender/user ID from the gateway logs or the inbound event payload.
Add those IDs to WEIXIN_ALLOWED_USERS , then restart the gateway.
...
Value Behavior open Bot responds in all groups (if events are delivered) allowlist Bot only responds in group IDs listed in group_allow_from (if events are delivered) disabled All group messages are ignored (default)
...
Type How it's handled Images Downloaded, AES-decrypted, and cached as JPEG. Video Downloaded, AES-decrypted, and cached as MP4. Files Downloaded, AES-decrypted, and cached. Original filename is preserved. Voice If a text transcription is available, it's extracted as text. Otherwise the audio (SILK format) is downloaded and cached.
...
Outbound (sending) ​
Method What it sends send Text messages with Markdown formatting send_image / send_image_file Native image messages (via CDN upload) send_document File attachments (via CDN upload) send_video Video messages (via CDN upload)
...
Condition Behavior Transient error (1st–2nd) Retry after 2 seconds Repeated errors (3+) Back off for 30 seconds, then reset counter Session expired ( errcode=-14 ) Pause for 10 minutes (re-login may be needed) Timeout Immediately re-poll (normal long-poll behavior)
Deduplication ​
