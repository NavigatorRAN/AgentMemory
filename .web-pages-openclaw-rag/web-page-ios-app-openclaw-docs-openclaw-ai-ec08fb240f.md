...
- Requested URL: https://docs.openclaw.ai/platforms/ios
- Final URL: https://docs.openclaw.ai/platforms/ios
- Canonical URL: https://docs.openclaw.ai/platforms/ios
- Fetched at: 2026-06-23T14:49:26Z
...
Start the Gateway:
bash Copy code openclaw gateway --port 18789
...
json5 Copy code { gateway : { nodes : { pairing : { autoApproveCidrs : [ "192.168.1.0/24" ], }, }, }, }
...
Verify connection:
bash Copy code openclaw nodes status openclaw gateway call node.list --params "{}"
...
json5 Copy code { gateway : { push : { apns : { relay : { baseUrl : "https://relay.example.com" , }, }, }, }, }
How the flow works:
...
Optional: set gateway.push.apns.relay.baseUrl on the gateway only when using a deliberately separate custom relay build.
Pair the app to the gateway and let it finish connecting.
The app publishes push.apns.register automatically after it has an APNs token, the operator session is connected, and relay registration succeeds.
After that, push.test , reconnect wakes, and wake nudges can use the stored relay-backed registration.
...
bash Copy code export OPENCLAW_APNS_TEAM_ID= "TEAMID" export OPENCLAW_APNS_KEY_ID= "KEYID" export OPENCLAW_APNS_PRIVATE_KEY_P8= " $(cat /path/to/AuthKey_KEYID.p8) "
...
bash Copy code mkdir -p ~/.openclaw/credentials/apns chmod 700 ~/.openclaw/credentials/apns mv /path/to/AuthKey_KEYID.p8 ~/.openclaw/credentials/apns/AuthKey_KEYID.p8 chmod 600 ~/.openclaw/credentials/apns/AuthKey_KEYID.p8 export OPENCLAW_APNS_PRIVATE_KEY_PATH= " $HOME /.openclaw/credentials/apns/AuthKey_KEYID.p8"
...
bash Copy code openclaw nodes invoke --node "iOS Node" -- command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__openclaw__/canvas/"}'
Notes:
