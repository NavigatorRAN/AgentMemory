...
- Requested URL: https://docs.openclaw.ai/help/environment
- Final URL: https://docs.openclaw.ai/help/environment
- Canonical URL: https://docs.openclaw.ai/help/environment
- Fetched at: 2026-06-23T14:44:19Z
...
.env in the current working directory (dotenv default; does not override; provider credentials and protected runtime controls are ignored).
Global .env at ~/.openclaw/.env (aka $OPENCLAW_STATE_DIR/.env ; recommended for provider API keys; does not override).
...
See Secrets Management and the
SecretRef credential surface for
supported fields.
Shell env import
...
json5 Copy code { models : { providers : { "vercel-gateway" : { apiKey : "${VERCEL_GATEWAY_API_KEY}" , }, }, }, }
...
xml Copy code < key > EnvironmentVariables </ key > < dict > < key > OPENCLAW_HOME </ key > < string > /Users/user </ string > </ dict >
...
bash Copy code export NODE_EXTRA_CA_CERTS=/etc/ssl/certs/ca-certificates.crt openclaw gateway run
