# Workspaces | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/guides/projects-workflow/workspaces
- Final URL: https://docs.paperclip.ing/guides/projects-workflow/workspaces/
- Canonical URL: https://docs.paperclip.ing/guides/projects-workflow/workspaces
- Fetched at: 2026-06-23T13:38:10Z
- Content type: text/html; charset=utf-8

## Description

When an agent picks up a task that involves working with code or files, it needs a place to do that work — a folder with the right code checked out at the right state, ready for th
...
This guide covers what execution workspaces are, how they relate to projects, and how to use the workspace detail screen — its tabs, runtime controls, and logs — to inspect and manage them.
Before you start: turning workspaces on
Workspaces are an experimental feature, so the Workspaces screen is hidden until you opt in.
Go to Instance settings → Experimental .
Turn on Enable Isolated Workspaces .
...
What the Workspaces screen is — and isn't
The most common surprise is opening Workspaces for the first time and finding it empty, with nothing to click. That's expected, and it's worth understanding why:
...
The heartbeat fires and the agent picks up a task
Paperclip resolves the workspace — creating a new one, reusing an existing one, or sticking with the project default, depending on your settings
The agent receives the workspace path and works within it
The workspace persists after the run if you're using an isolated or reusable workspace mode
Workspace modes
...
Open your project and click on the workspace
Click Start Services (or the specific service you want to start)
The service runs in the background; the status updates in the UI
...
Review the agent's work (merge the branch, approve changes, etc.)
Click Archive on the workspace
Paperclip cleans up the workspace according to its mode and project settings
...
Tasks — every task that has run against this workspace (this is the default tab)
Services — the runtime services and one-shot jobs for this workspace, with start/stop/restart and run controls
Configuration — workspace settings, workspace context, and concrete location
Runtime logs — history of runtime and cleanup operations
Routines — routines that use workspace-specific variables, which you can run against this workspace
...
Services tab
The Services tab is the control surface for runtime behaviour: services you want running in the background, and one-shot jobs you want to fire off against this workspace. (The header above the tabs mirrors the service controls as quick start/stop/restart buttons, so you can act on them from any tab.)
Two rules govern what you can do here:
You need a working directory. Every command Paperclip runs against the workspace needs an absolute path to run in. If the workspace's cwd is empty, the action buttons are disabled and a hint reminds you to fix that first. Fill in the Working directory field on the Configuration tab, save, and the controls light up again.
Services need runtime config. Service commands only show up if there's a runtime config somewhere in the chain — either inherited from the project workspace or overridden on this execution workspace. If neither exists, the services list stays empty with a short note; jobs may still run if they're defined and a cwd is set.
...
The project workspace defines a baseline runtime config. This is what gets applied to every execution workspace carved out of that project workspace unless you explicitly override it.
The execution workspace can either inherit that baseline or override it with its own workspaceRuntime object.
...
Clicking Start on a service in the Services tab creates an operation with the service's command, a started phase, and — on a successful start — a running or finished status depending on whether the service stays alive or exits immediately.
...
Tasks tab
The Tasks tab shows every issue that has ever run against this execution workspace — not just the source issue that created it, but any follow-ups, retries, and related tasks that inherited the workspace.
...
Cleanup — the cleanup command (if any) runs first. This is where you stop stray dev servers, free ports, and do anything that would otherwise prevent teardown from succeeding.
Teardown — the teardown command then dismantles the workspace itself. For git worktrees this typically removes the worktree directory and prunes the branch; for other providers it does whatever that provider considers cleanup.
...
It picks a path under the project workspace's worktree directory.
It creates a new branch off the configured base ref (for example, origin/main ).
It checks that branch out into the new worktree path.
It runs the provision command so dependencies, env files, and any other per-worktree setup are in place before the agent starts.
...
Pulling together the tabs, here's how the detail screen maps to typical questions you'll ask:
...
"Who's been running against this workspace?" → Tasks tab.
"Is anything running right now?" → Services tab (or the header quick controls) for service status plus Tasks tab (live-run indicator).
...
Create the issue and assign it to the agent who'll do the work.
Let Paperclip provision an isolated workspace when the agent checks out the task.
When the agent finishes and the branch is ready, review the branch in your usual git workflow.
Once merged, come back to the Workspaces tab and archive the workspace.
Recovering from a failed close
Open the workspace and look at the Runtime logs tab to see which command failed.
Fix the underlying problem — a stuck process, a missing permission, a command that assumes the wrong shell.
Click Retry close and confirm the workspace transitions to archived .
If retries keep failing, drop into the worktree directory yourself and run the teardown command manually.
Shipping a migration
Create a job in the workspace's runtime config (for example, db:migrate ).
Open the workspace and click Run on that job from the Services tab.
Wait for the operation to appear in Runtime logs with a successful status.
Link the resulting evidence to the issue — the operation timestamp is enough.
Reusing a workspace across tasks
When creating the follow-up issue, set parentId if it's a child task, or inheritExecutionWorkspaceFromIssueId if it's a sibling that should share the same workspace.
Paperclip will wire the new issue up to the existing workspace on checkout.
On the Tasks tab of the workspace, you'll see the new issue alongside the original.
These patterns are just defaults — adapt them to how your team actually operates.
You're set
Execution workspaces give agents the isolated, stateful environments they need to do reliable file-based work. The Workspace list shows you everything provisioned for a project; the Configuration tab keeps the path, repo, and runtime config in sync with reality; the Runtime logs tab tells you what actually ran; and the Tasks tab shows you who's been using it.
