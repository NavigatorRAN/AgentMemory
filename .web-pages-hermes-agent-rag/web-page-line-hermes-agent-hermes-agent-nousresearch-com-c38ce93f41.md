...
- Requested URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/line
- Final URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/line
- Canonical URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/line
- Fetched at: 2026-06-23T14:16:58Z
...
Context Behavior 1:1 chat ( U IDs) Responds to every message Group chat ( C IDs) Responds when the group is on the allowlist Multi-user room ( R IDs) Responds when the room is on the allowlist
...
Create a Provider, then under it a Messaging API channel.
From the channel's Basic settings tab, copy the Channel secret .
From the Messaging API tab, scroll to Channel access token (long-lived) and click Issue . Copy the token.
In the Messaging API tab, also disable Auto-reply messages and Greeting messages so they don't fight your bot's replies.
...
Open your channel → Messaging API tab.
Under Webhook settings → Webhook URL , paste https://<your-tunnel>/line/webhook (note the /line/webhook path — the adapter listens there).
Click Verify . LINE pings the URL; you should see a 200.
