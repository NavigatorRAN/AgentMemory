# MCP Zero Trust Auth (JWT Signer) | liteLLM

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.litellm.ai/docs/mcp_zero_trust
- Final URL: https://docs.litellm.ai/docs/mcp_zero_trust
- Canonical URL: https://docs.litellm.ai/docs/mcp_zero_trust
- Fetched at: 2026-06-23T14:24:47Z
...
Build a verified MCP server with FastMCP :
weather_server.py
...
Source Resolves to token:<claim> Any claim from the verified incoming JWT (e.g. token:sub , token:email , token:oid ) litellm:user_id LiteLLM's internal user ID litellm:email User email from LiteLLM auth context litellm:end_user_id End-user ID if set separately litellm:team_id Team ID from LiteLLM auth context
...
Related ​
MCP Guardrails — PII masking and blocking for MCP calls
MCP OAuth — upstream OAuth2 for MCP server access
MCP AWS SigV4 — AWS-signed requests to MCP servers
Basic setup
Thread IdP identity into MCP JWTs
Block callers missing required attributes
Add custom metadata to every JWT
