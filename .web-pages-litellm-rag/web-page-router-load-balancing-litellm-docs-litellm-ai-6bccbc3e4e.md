...
- Requested URL: https://docs.litellm.ai/docs/routing
- Final URL: https://docs.litellm.ai/docs/routing
- Canonical URL: https://docs.litellm.ai/docs/routing
- Fetched at: 2026-06-23T14:32:49Z
...
Load Balancing ​
(s/o @paulpierre and sweep proxy for their contributions to this implementation)
See Code
Quick Start ​
...
tip
You can also create, edit, and delete routing groups from the dashboard. See Manage Routing Groups via UI .
...
simple-shuffle picks one of the two deployments using weight .
If the picked deployment raises a provider error (e.g. RateLimitError , InternalServerError ), its deployment ID is added to metadata._failover_excluded_ids .
The router re-enters simple-shuffle with the failed deployment excluded and weights renormalized over what's left.
Steps 2–3 repeat until a deployment succeeds, every peer has been excluded, or max_fallbacks is reached.
Only after all peers are exhausted does the router fall through to any fallbacks configured for the group.
See enable_weighted_failover in the router settings reference for the flag.
