...
- Requested URL: https://docs.openclaw.ai/install/updating
- Final URL: https://docs.openclaw.ai/install/updating
- Canonical URL: https://docs.openclaw.ai/install/updating
- Fetched at: 2026-06-23T14:45:08Z
...
bash Copy code openclaw update
To switch channels or target a specific version:
bash Copy code openclaw update --channel beta openclaw update --channel dev openclaw update --dry-run # preview without applying
...
bash Copy code # npm package install -> editable git checkout openclaw update --channel dev # git checkout -> npm package install openclaw update --channel stable
Run with --dry-run first to preview the exact install-mode switch:
bash Copy code openclaw update --channel dev --dry-run openclaw update --channel stable --dry-run
...
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method npm
To pin the recovery to a specific version or dist-tag, add --version :
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method npm --version <version-or-dist-tag>
Alternative: manual npm, pnpm, or bun
bash Copy code npm i -g openclaw@latest
...
bash Copy code openclaw gateway stop sudo /usr/bin/npm i -g openclaw@latest openclaw gateway install --force openclaw gateway restart
Then verify the service:
bash Copy code openclaw --version curl -fsS http://127.0.0.1:18789/readyz openclaw plugins list --json openclaw gateway status --deep --json openclaw doctor --lint --json
...
bash Copy code pnpm add -g openclaw@latest bash Copy code bun add -g openclaw@latest
...
ini Copy code ReadWritePaths =/var/lib/openclaw /home/openclaw/.openclaw /tmp Disk-space preflight
...
json5 Copy code { update : { channel : "stable" , auto : { enabled : true , stableDelayHours : 6 , stableJitterHours : 12 , betaCheckIntervalHours : 1 , }, }, }
Channel Behavior stable Waits stableDelayHours , then applies with deterministic jitter across stableJitterHours (spread rollout). beta Checks every betaCheckIntervalHours (default: hourly) and applies immediately. dev No automatic apply. Use openclaw update manually.
...
Verify
bash Copy code openclaw health
Rollback
Pin a version (npm)
bash Copy code npm i -g openclaw@<version> openclaw doctor openclaw gateway restart Tip
npm view openclaw version shows the current published version.
Pin a commit (source)
bash Copy code git fetch origin git checkout " $(git rev-list -n 1 --before=\"2026-01-01\" origin/main) " pnpm install && pnpm build openclaw gateway restart
