...
- Requested URL: https://docs.openclaw.ai/tools/browser
- Final URL: https://docs.openclaw.ai/tools/browser
- Canonical URL: https://docs.openclaw.ai/tools/browser
- Fetched at: 2026-06-23T14:57:26Z
...
json5 Copy code { plugins : { entries : { browser : { enabled : false , }, }, }, }
...
stage:
json5 Copy code { tools : { profile : "coding" , alsoAllow : [ "browser" ], }, }
...
How it works:
Agent calls browser screenshot → image captured to disk as usual.
The browser tool asks the existing image-understanding runtime whether it
...
If image understanding is unavailable, skipped, or fails, the browser falls
...
bash Copy code openclaw config set browser.executablePath "/usr/bin/google-chrome" openclaw config set browser.profiles.work.executablePath "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
Or set it in config, per platform:
macOS
json5 Copy code { browser : { executablePath : "/Applications/Brave Browser.app/Contents/MacOS/Brave Browser" , }, }
Windows
json5 Copy code { browser : { executablePath : "C:\\Program Files\\BraveSoftware\\Brave-Browser\\Application\\brave.exe" , }, }
Linux
json5 Copy code { browser : { executablePath : "/usr/bin/brave-browser" , }, }
...
Example:
json5 Copy code { browser : { enabled : true , defaultProfile : "browserless" , remoteCdpTimeoutMs : 2000 , remoteCdpHandshakeTimeoutMs : 4000 , profiles : { browserless : { cdpUrl : "wss://production-sfo.browserless.io?token=<BROWSERLESS_API_KEY>" , color : "#00AA00" , }, }, }, }
...
json5 Copy code { browser : { enabled : true , defaultProfile : "browserless" , profiles : { browserless : { cdpUrl : "ws://127.0.0.1:3000" , attachOnly : true , color : "#00AA00" , }, }, }, }
...
proxies.
json5 Copy code { browser : { enabled : true , defaultProfile : "browserbase" , remoteCdpTimeoutMs : 3000 , remoteCdpHandshakeTimeoutMs : 5000 , profiles : { browserbase : { cdpUrl : "wss://connect.browserbase.com?apiKey=<BROWSERBASE_API_KEY>" , color : "#F97316" , }, }, }, }
...
Notte
Notte is a cloud platform for running headless
browsers with built-in stealth, residential proxies, and a CDP-native
WebSocket gateway.
json5 Copy code { browser : { enabled : true , defaultProfile : "notte" , remoteCdpTimeoutMs : 3000 , remoteCdpHandshakeTimeoutMs : 5000 , profiles : { notte : { cdpUrl : "wss://us-prod.notte.cc/sessions/connect?token=<NOTTE_API_KEY>" , color : "#7C3AED" , }, }, }, }
Sign up and copy your API Key from the
...
hours. See pricing for paid plan limits.
See the Notte docs for full API reference, SDK
...
json5 Copy code { browser : { profiles : { brave : { driver : "existing-session" , attachOnly : true , userDataDir : "~/Library/Application Support/BraveSoftware/Brave-Browser" , color : "#FB542B" , }, }, }, }
...
bash Copy code openclaw browser --browser-profile user start openclaw browser --browser-profile user status openclaw browser --browser-profile user tabs openclaw browser --browser-profile user snapshot --format ai
...
Field What it does mcpCommand Executable to spawn instead of npx . Resolved as-is; absolute paths are honored. mcpArgs Argument array passed verbatim to mcpCommand . Replaces the default chrome-devtools-mcp@latest --autoConnect arguments.
...
bash Copy code openclaw browser --browser-profile openclaw start openclaw browser --browser-profile openclaw tabs openclaw browser --browser-profile openclaw open https://example.com
