# Subscription Proxy | Hermes Agent

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://hermes-agent.nousresearch.com/docs/user-guide/features/subscription-proxy
- Final URL: https://hermes-agent.nousresearch.com/docs/user-guide/features/subscription-proxy
- Canonical URL: https://hermes-agent.nousresearch.com/docs/user-guide/features/subscription-proxy
- Fetched at: 2026-06-23T14:16:35Z
...
API server Subscription proxy What it serves Your agent (full toolset, memory, skills) Raw model inference Use case "Use Hermes as a chat backend" "Use my Portal sub from another app" Auth Your API_SERVER_KEY Any bearer (proxy attaches the real one) Tool calls Yes — the agent runs tools No — passthrough only
...
Portal:
Path Purpose /v1/chat/completions Chat completions (streaming + non-streaming) /v1/completions Legacy text completions /v1/embeddings Embeddings /v1/models Model list
...
Karakeep takes an OpenAI-compatible API for
...
Receive POST /v1/chat/completions from your app
Look up the adapter's current credential (refresh if expiring)
Forward the request body verbatim, with Authorization: Bearer <minted-key>
