...
- Requested URL: https://docs.openclaw.ai/automation/tasks
- Final URL: https://docs.openclaw.ai/automation/tasks
- Canonical URL: https://docs.openclaw.ai/automation/tasks
- Fetched at: 2026-06-23T14:35:48Z
...
Quick start
List and filter
bash Copy code # List all tasks (newest first) openclaw tasks list # Filter by runtime or status openclaw tasks list --runtime acp openclaw tasks list --status running
Inspect
bash Copy code # Show details for a specific task (by ID, run ID, or session key) openclaw tasks show <lookup>
Cancel and notify
bash Copy code # Cancel a running task (kills the child session) openclaw tasks cancel <lookup> # Change notification policy for a task openclaw tasks notify <lookup> state_changes
Audit and maintenance
bash Copy code # Run a health audit openclaw tasks audit # Preview or apply maintenance openclaw tasks maintenance openclaw tasks maintenance --apply
Task flow
bash Copy code # Inspect TaskFlow state openclaw tasks flow list openclaw tasks flow show <lookup> openclaw tasks flow cancel <lookup>
What creates a task
Source Runtime type When a task record is created Default notify policy ACP background runs acp Spawning a child ACP session done_only Subagent orchestration subagent Spawning a subagent via sessions_spawn done_only Cron jobs (all types) cron Every cron execution (main-session and isolated) silent CLI operations cli openclaw agent commands that run through the gateway silent Agent media jobs cli Session-backed image_generate / music_generate / video_generate runs silent
...
Status What it means queued Created, waiting for the agent to start running Agent turn is actively executing succeeded Completed successfully failed Completed with an error timed_out Exceeded the configured timeout cancelled Stopped by the operator via openclaw tasks cancel lost The runtime lost authoritative backing state after a 5-minute grace period
...
Policy What is delivered done_only (default) Only terminal state (succeeded, failed, etc.) - this is the default state_changes Every state transition and progress update silent Nothing at all
Change the policy while a task is running:
bash Copy code openclaw tasks notify <lookup> state_changes
CLI reference
tasks list bash Copy code openclaw tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
Output columns: Task ID, Kind, Status, Delivery, Run ID, Child Session, Summary.
tasks show bash Copy code openclaw tasks show <lookup>
...
tasks cancel bash Copy code openclaw tasks cancel <lookup>
...
tasks notify bash Copy code openclaw tasks notify <lookup> <done_only|state_changes|silent> tasks audit bash Copy code openclaw tasks audit [--json]
Surfaces operational issues. Findings also appear in openclaw status when issues are detected.
Finding Severity Trigger stale_queued warn Queued for more than 10 minutes stale_running error Running for more than 30 minutes lost warn/error Runtime-backed task ownership disappeared; retained lost tasks warn until cleanupAfter , then become errors delivery_failed warn Delivery failed and notify policy is not silent missing_cleanup warn Terminal task with no cleanup timestamp inconsistent_timestamps warn Timeline violation (for example ended before started)
tasks maintenance bash Copy code openclaw tasks maintenance [--json] openclaw tasks maintenance --apply [--json]
...
tasks flow list | show | cancel bash Copy code openclaw tasks flow list [--status <status>] [--json] openclaw tasks flow show <lookup> [--json] openclaw tasks flow cancel <lookup>
...
Code Copy code Tasks: 3 queued · 2 running · 1 issues
The summary reports:
...
Code Copy code $OPENCLAW_STATE_DIR/tasks/runs.sqlite
