# Admin HTTP RPC plugin - OpenClaw

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.openclaw.ai/plugins/admin-http-rpc
- Final URL: https://docs.openclaw.ai/plugins/admin-http-rpc
- Canonical URL: https://docs.openclaw.ai/plugins/admin-http-rpc
- Fetched at: 2026-06-23T14:49:57Z
...
CLI
bash Copy code openclaw plugins enable admin-http-rpc openclaw gateway restart
Config
json5 Copy code { plugins : { entries : { "admin-http-rpc" : { enabled : true }, }, }, }
...
bash Copy code openclaw plugins disable admin-http-rpc openclaw gateway restart
Verify the route
Use health as the smallest safe request:
bash Copy code curl -sS http://<gateway-host>:<port>/api/v1/admin/rpc \ -H 'Authorization: Bearer <gateway-token>' \ -H 'Content-Type: application/json' \ -d '{"method":"health","params":{}}'
A successful response has ok: true :
json Copy code { "id" : "generated-request-id" , "ok" : true , "payload" : { "status" : "ok" } }
...
Request
http Copy code POST /api/v1/admin/rpc Authorization : Bearer <gateway-token> Content-Type : application/json json Copy code { "id" : "optional-request-id" , "method" : "health" , "params" : { } }
Fields:
...
json Copy code { "id" : "optional-request-id" , "ok" : true , "payload" : { } }
Gateway method errors use:
json Copy code { "id" : "optional-request-id" , "ok" : false , "error" : { "code" : "INVALID_REQUEST" , "message" : "bad params" } }
