# Adapter Commands | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/reference/cli/adapter
- Final URL: https://docs.paperclip.ing/reference/cli/adapter/
- Canonical URL: https://docs.paperclip.ing/reference/cli/adapter
- Fetched at: 2026-06-23T13:39:23Z
- Content type: text/html; charset=utf-8

## Description

Adapters are the server-side runtimes that actually execute an agent's work — they wrap Claude, Codex, or another provider and turn a wakeup into a real run. The CLI does not run a
...
Note: Most agent work in Paperclip is executed server-side by the runtime and its adapters. The CLI triggers and observes that work — see What runs where . The adapter commands let a board operator look under the hood of that execution layer.
Adapter Types
...
When you create an agent you set its adapterType , so reading an adapter's config schema first (see below) tells you exactly which fields that agent's payload-json accepts. See Agent Commands for the hire flow.
Inspect Adapters
...
Command What it returns adapter list Every registered adapter, built-in and external. Start here to discover valid types. adapter get <type> The full record for one adapter — its current settings, install source, and status. adapter config-schema <type> The JSON schema for that adapter's configuration. Read this before writing an agent or adapter payload-json so you know the accepted fields. adapter ui-parser <type> The adapter's UI parser JavaScript — the client-side code the UI uses to render the adapter's run output. Useful when debugging how a run is displayed. # Discover what's installed, then read one adapter's schema
...
Flag Use -C, --company-id <id> The company whose model list you want. Required unless a company is set in your context profile. --refresh Re-fetch the provider's model list instead of returning the cached set. Use this after rotating keys or when a newly released model is missing. Defaults to off. --environment-id <id> Scope the lookup to a specific environment for environment-aware adapters.
...
Command Notes adapter install Requires --payload-json describing the package to register (for example packageName and version ). adapter delete <type> Removes an external adapter registration.
...
Command Use Payload adapter update <type> Update an adapter's settings. Validate your patch against config-schema first. --payload-json required adapter override <type> Pause or resume a built-in adapter. This is how you disable a built-in without deleting it. --payload-json required adapter reload <type> Reload the adapter in place — re-reads its configuration without reinstalling. --payload-json optional, defaults to {} adapter reinstall <type> Reinstall the adapter from its source. Use when a reload is not enough and you need a clean install. --payload-json optional, defaults to {}
...
Common Options
Every adapter subcommand accepts the standard client options: --data-dir <path> , --api-base <url> , --api-key <token> , --context <path> , --profile <name> , and --json . The company-scoped subcommands — models , model-profiles , detect-model , and test-environment — additionally take -C, --company-id <id> and require a resolvable company. See Common Options for how API base and context resolution work, and Output and Scripting for --json usage.
...
See also
Agent Commands — set an agent's adapterType and payload-json using the schema you read here
What runs where — why adapters execute server-side and the CLI only triggers and observes
