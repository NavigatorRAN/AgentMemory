# Kanban worker lanes | Hermes Agent

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-worker-lanes
- Final URL: https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-worker-lanes
- Canonical URL: https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-worker-lanes
- Fetched at: 2026-06-23T14:16:21Z
...
If you're writing the worker code itself — the agent that runs inside a lane — the kanban lifecycle and reference details are injected into the worker's system prompt automatically (the KANBAN_GUIDANCE block in agent/prompt_builder.py ).
The hierarchy ​
...
The dispatcher matches task.assignee against either a Hermes profile name (the default lane shape) or a registered non-spawnable identifier (the plugin lane shape — see Adding an external CLI worker lane below). Tasks whose assignee doesn't resolve are left on ready with a skipped_nonspawnable event so a board operator can fix them; they are not silently dropped or executed by an arbitrary fallback.
2. A spawn mechanism ​
