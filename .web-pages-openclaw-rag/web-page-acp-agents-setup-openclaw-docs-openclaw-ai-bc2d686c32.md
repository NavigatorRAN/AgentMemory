...
- Requested URL: https://docs.openclaw.ai/tools/acp-agents-setup
- Final URL: https://docs.openclaw.ai/tools/acp-agents-setup
- Canonical URL: https://docs.openclaw.ai/tools/acp-agents-setup
- Fetched at: 2026-06-23T14:57:22Z
...
Route Config/command Setup page Native Codex app-server /codex ... , openai/gpt-* agent refs Codex harness Explicit Codex ACP adapter /acp spawn codex , runtime: "acp", agentId: "codex" This page
...
Required config
Core ACP baseline:
json5 Copy code { acp : { enabled : true , // Optional. Default is true; set false to pause ACP dispatch while keeping /acp controls. dispatch : { enabled : true }, backend : "acpx" , defaultAgent : "codex" , allowedAgents : [ "claude" , "codex" , "copilot" , "cursor" , "droid" , "gemini" , "iflow" , "kilocode" , "kimi" , "kiro" , "openclaw" , "opencode" , "openclaw" , "qwen" , ], maxConcurrentSessions : 8 , stream : { coalesceIdleMs : 300 , maxChunkChars : 1200 , }, runtime : { ttlMinutes : 120 , }, }, }
Thread binding config is channel-adapter specific. Example for Discord:
json5 Copy code { session : { threadBindings : { enabled : true , idleHours : 24 , maxAgeHours : 0 , }, }, channels : { discord : { threadBindings : { enabled : true , spawnSessions : true , }, }, }, }
...
bash Copy code openclaw plugins install ./path/to/local/acpx-plugin
...
json Copy code { "plugins" : { "entries" : { "acpx" : { "enabled" : true , "config" : { "command" : "../acpx/dist/cli.js" , "expectedVersion" : "any" } } } } }
...
json Copy code { "plugins" : { "entries" : { "acpx" : { "enabled" : true , "config" : { "agents" : { "claude" : { "command" : "node" , "args" : [ "/path/to/custom adapter.mjs" , "--verbose" ] } } } } } } }
...
bash Copy code openclaw config set plugins.entries.acpx.config.pluginToolsMcpBridge true
What this does:
...
bash Copy code openclaw config set plugins.entries.acpx.config.openClawToolsMcpBridge true
...
bash Copy code openclaw config set plugins.entries.acpx.config.timeoutSeconds 180
...
bash Copy code openclaw config set plugins.entries.acpx.config.probeAgent claude
...
Value Behavior approve-all Auto-approve all file writes and shell commands. approve-reads Auto-approve reads only; writes and exec require prompts. deny-all Deny all permission prompts.
...
Value Behavior fail Abort the session with AcpRuntimeError . (default) deny Silently deny the permission and continue (graceful degradation).
Configuration
Set via plugin config:
bash Copy code openclaw config set plugins.entries.acpx.config.permissionMode approve-all openclaw config set plugins.entries.acpx.config.nonInteractivePermissions fail
