...
- Requested URL: https://docs.openclaw.ai/gateway/secrets
- Final URL: https://docs.openclaw.ai/gateway/secrets
- Canonical URL: https://docs.openclaw.ai/gateway/secrets
- Fetched at: 2026-06-23T14:44:10Z
- Content type: text/html; charset=utf-8

## Description

Secrets management: SecretRef contract, runtime snapshot behavior, and safe one-way scrubbing
...
env
json5 Copy code { source : "env" , provider : "default" , id : "OPENAI_API_KEY" }
Supported SecretInput fields also accept exact string shorthands:
json5 Copy code "${OPENAI_API_KEY}" "$OPENAI_API_KEY"
Validation:
provider must match ^[a-z][a-z0-9_-]{0,63}$
id must match ^[A-Z][A-Z0-9_]{0,127}$
file
json5 Copy code { source : "file" , provider : "filemain" , id : "/providers/openai/apiKey" }
id must be an absolute JSON pointer ( /... )
RFC6901 escaping in segments: ~ => ~0 , / => ~1
exec
json5 Copy code { source : "exec" , provider : "vault" , id : "providers/openai/apiKey#value" }
...
json Copy code { "protocolVersion" : 1 , "provider" : "vault" , "ids" : [ "providers/openai/apiKey" ] }
Response payload (stdout):
jsonc Copy code { "protocolVersion" : 1 , "values" : { "providers/openai/apiKey" : "<openai-api-key>" } } // pragma: allowlist secret
Optional per-id errors:
json Copy code { "protocolVersion" : 1 , "values" : { } , "errors" : { "providers/openai/apiKey" : { "message" : "not found" } } }
...
json5 Copy code { secrets : { providers : { xai_key_file : { source : "file" , path : "~/.openclaw/secrets/xai-api-key.txt" , mode : "singleValue" , }, }, }, models : { providers : { xai : { apiKey : { source : "file" , provider : "xai_key_file" , id : "value" }, }, }, }, }
...
1Password CLI json5 Copy code { secrets : { providers : { onepassword_openai : { source : "exec" , command : "/opt/homebrew/bin/op" , allowSymlinkCommand : true , // required for Homebrew symlinked binaries trustedDirs : [ "/opt/homebrew" ], args : [ "read" , "op://Personal/OpenClaw QA API Key/password" ], passEnv : [ "HOME" ], jsonOnly : false , }, }, }, models : { providers : { openai : { baseUrl : "https://api.openai.com/v1" , models : [{ id : "gpt-5" , name : "gpt-5" }], apiKey : { source : "exec" , provider : "onepassword_openai" , id : "value" }, }, }, }, } Bitwarden Secrets Manager (`bws`)
...
Bitwarden instance.
json5 Copy code { secrets : { providers : { bws : { source : "exec" , command : "/usr/local/bin/openclaw-bws-resolver.mjs" , passEnv : [ "BWS_ACCESS_TOKEN" , "BWS_SERVER_URL" , "PATH" , "BWS_BIN" ], jsonOnly : true , }, }, }, models : { providers : { openai : { baseUrl : "https://api.openai.com/v1" , models : [{ id : "gpt-5" , name : "gpt-5" }], apiKey : { source : "exec" , provider : "bws" , id : "openclaw/providers/openai/apiKey" , }, }, }, }, }
...
bash Copy code openclaw secrets audit --check openclaw secrets audit --allow-exec sops json5 Copy code { secrets : { providers : { sops_openai : { source : "exec" , command : "/opt/homebrew/bin/sops" , allowSymlinkCommand : true , // required for Homebrew symlinked binaries trustedDirs : [ "/opt/homebrew" ], args : [ "-d" , "--extract" , '["providers"]["openai"]["apiKey"]' , "/path/to/secrets.enc.json" ], passEnv : [ "SOPS_AGE_KEY_FILE" ], jsonOnly : false , }, }, }, models : { providers : { openai : { baseUrl : "https://api.openai.com/v1" , models : [{ id : "gpt-5" , name : "gpt-5" }], apiKey : { source : "exec" , provider : "sops_openai" , id : "value" }, }, }, }, }
...
json5 Copy code { plugins : { entries : { acpx : { enabled : true , config : { mcpServers : { github : { command : "npx" , args : [ "-y" , "@modelcontextprotocol/server-github" ], env : { GITHUB_PERSONAL_ACCESS_TOKEN : { source : "env" , provider : "default" , id : "MCP_GITHUB_PAT" , }, }, }, }, }, }, }, }, }
...
json5 Copy code { agents : { defaults : { sandbox : { mode : "all" , backend : "ssh" , ssh : { target : "user@gateway-host:22" , identityData : { source : "env" , provider : "default" , id : "SSH_IDENTITY" }, certificateData : { source : "env" , provider : "default" , id : "SSH_CERTIFICATE" }, knownHostsData : { source : "env" , provider : "default" , id : "SSH_KNOWN_HOSTS" }, }, }, }, }, }
Runtime behavior:
...
Audit current state
bash Copy code openclaw secrets audit --check
Configure and apply SecretRefs
bash Copy code openclaw secrets configure --apply
Re-audit
...
secrets apply
Apply a saved plan:
bash Copy code openclaw secrets apply --from /tmp/openclaw-secrets-plan.json openclaw secrets apply --from /tmp/openclaw-secrets-plan.json --allow-exec openclaw secrets apply --from /tmp/openclaw-secrets-plan.json --dry-run openclaw secrets apply --from /tmp/openclaw-secrets-plan.json --dry-run --allow-exec
...
For strict target/path contract details and exact rejection rules, see Secrets Apply Plan Contract .
