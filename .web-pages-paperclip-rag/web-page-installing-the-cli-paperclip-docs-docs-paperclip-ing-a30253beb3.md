# Installing the CLI | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/reference/cli/installation
- Final URL: https://docs.paperclip.ing/reference/cli/installation/
- Canonical URL: https://docs.paperclip.ing/reference/cli/installation
- Fetched at: 2026-06-23T13:39:40Z
- Content type: text/html; charset=utf-8

## Description

The Paperclip CLI ships as a single binary, paperclipai. Use this page to get that binary on your machine, point it at an instance, and bring up a working local instance from a col
...
Path What you get Quickstart Local defaults that are ready to run: an embedded PostgreSQL database, local-disk file storage, and local encrypted secrets. This is the recommended path. Advanced setup Step-by-step prompts for the database, LLM provider, logging, server binding, storage, and secrets. Use this when you need explicit control over any of those.
...
Flag Use --yes , -y Accept Quickstart defaults non-interactively and start immediately. Without --bind , this forces trusted-local loopback defaults, ignoring any reachability env vars. Use it in scripts and CI. --run Save the config, then start the server right away (equivalent to answering "yes" to the start prompt). --bind <mode> Apply a reachability preset to the Quickstart server config. One of loopback , lan , or tailnet . --config <path> , -c Write the config to a specific path instead of the default instance config. --data-dir <path> , -d Isolate all local Paperclip state away from ~/.paperclip — handy for clean test instances and worktrees. # Non-interactive Quickstart that boots immediately
...
Resolves the active instance and config path, and loads the instance env file.
If no config exists and the terminal is interactive, hands off to onboarding first.
Runs the doctor checks with repair enabled by default. If any check fails, it stops before starting the server.
Starts the Paperclip server.
Flag Use --instance <id> , -i Select the local instance id (default: default ). Use this to run more than one isolated instance side by side. --bind <mode> On a first run (when no config exists yet), pass a reachability preset into onboarding: loopback , lan , or tailnet . --repair / --no-repair Repair is on by default; --no-repair runs doctor in read-only mode so it reports problems without changing files. --config <path> , -c Use a specific config file. --data-dir <path> , -d Isolate local state away from ~/.paperclip . # Run a second, isolated instance
...
That step is covered in authentication . For how the CLI finds your server (the --api-base / PAPERCLIP_API_URL resolution order) and the flags shared by every client command, see common options .
Next steps
Authentication — connect the CLI, choose a persona, and mint board tokens or agent keys.
Setup commands — doctor , configure , env , and allowed-hostname for repairing and tuning an instance.
Common options — shared flags and how the CLI resolves which server to talk to.
run — the run subcommands for inspecting heartbeat runs on a live instance.
Overview — what the CLI is for and how it fits the wider operating model.
