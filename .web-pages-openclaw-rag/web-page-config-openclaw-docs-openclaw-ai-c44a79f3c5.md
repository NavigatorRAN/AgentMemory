...
- Requested URL: https://docs.openclaw.ai/cli/config
- Final URL: https://docs.openclaw.ai/cli/config
- Canonical URL: https://docs.openclaw.ai/cli/config
- Fetched at: 2026-06-23T14:37:25Z
...
bash Copy code openclaw config schema
Pipe it into a file when you want to inspect or validate it with other tools:
bash Copy code openclaw config schema > openclaw.schema.json
Paths
...
bash Copy code openclaw config get agents.defaults.workspace openclaw config get 'agents.list[0].id'
Use the agent list index to target a specific agent:
bash Copy code openclaw config get agents.list openclaw config set 'agents.list[1].tools.exec.node' "node-id-or-name"
Values
...
bash Copy code openclaw config set agents.defaults.heartbeat.every "0m" openclaw config set gateway.port 19001 --strict-json openclaw config set channels.whatsapp.groups '["*"]' --strict-json
...
bash Copy code openclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge openclaw config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
Use --replace only when you intentionally want the provided value to become the complete target value.
config set modes
openclaw config set supports four assignment styles:
Value mode
bash Copy code openclaw config set <path> <value>
SecretRef builder mode
bash Copy code openclaw config set channels.discord.token \ --ref-provider default \ --ref-source env \ --ref-id DISCORD_BOT_TOKEN
Provider builder mode
Provider builder mode targets secrets.providers.<alias> paths only:
bash Copy code openclaw config set secrets.providers.vault \ --provider-source exec \ --provider-command /usr/local/bin/openclaw-vault \ --provider-arg read \ --provider-arg openai/api-key \ --provider-timeout-ms 5000
Batch mode
bash Copy code openclaw config set --batch-json '[ { "path" : "secrets.providers.default" , "provider" : { "source" : "env" } }, { "path" : "channels.discord.token" , "ref" : { "source" : "env" , "provider" : "default" , "id" : "DISCORD_BOT_TOKEN" } } ] ' bash Copy code openclaw config set --batch-file ./config-set.batch.json --dry-run
Warning
...
bash Copy code openclaw config patch --file ./openclaw.patch.json5 --dry-run openclaw config patch --file ./openclaw.patch.json5
You can also pipe a patch over stdin, which is useful for remote setup scripts:
bash Copy code ssh openclaw-host 'openclaw config patch --stdin --dry-run' < ./openclaw.patch.json5 ssh openclaw-host 'openclaw config patch --stdin' < ./openclaw.patch.json5
Example patch:
json5 Copy code { channels : { slack : { enabled : true , mode : "socket" , botToken : { source : "env" , provider : "default" , id : "SLACK_BOT_TOKEN" }, appToken : { source : "env" , provider : "default" , id : "SLACK_APP_TOKEN" }, groupPolicy : "open" , requireMention : false , }, discord : { enabled : true , token : { source : "env" , provider : "default" , id : "DISCORD_BOT_TOKEN" }, dmPolicy : "disabled" , dm : { enabled : false }, groupPolicy : "allowlist" , }, }, agents : { defaults : { model : { primary : "openai/gpt-5.5" }, models : { "openai/gpt-5.5" : { params : { fastMode : true } }, }, }, }, }
...
bash Copy code openclaw config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
...
bash Copy code openclaw config set channels.discord.token \ '{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}' \ --strict-json openclaw config set secrets.providers.vaultfile \ '{"source":"file","path":"/etc/openclaw/secrets.json","mode":"json"}' \ --strict-json
...
bash Copy code openclaw config set secrets.providers.vault \ --provider-source exec \ --provider-command /usr/local/bin/openclaw-vault \ --provider-arg read \ --provider-arg openai/api-key \ --provider-json-only \ --provider-pass-env VAULT_TOKEN \ --provider-trusted-dir /usr/local/bin \ --provider-timeout-ms 5000
Dry run
Use --dry-run to validate changes without writing openclaw.json .
bash Copy code openclaw config set channels.discord.token \ --ref-provider default \ --ref-source env \ --ref-id DISCORD_BOT_TOKEN \ --dry-run openclaw config set channels.discord.token \ --ref-provider default \ --ref-source env \ --ref-id DISCORD_BOT_TOKEN \ --dry-run \ --json openclaw config set channels.discord.token \ --ref-provider vault \ --ref-source exec \ --ref-id discord/token \ --dry-run \ --allow-exec
Dry-run behavior
...
JSON output shape
json5 Copy code { ok : boolean, operations : number, configPath : string, inputModes : [ "value" | "json" | "builder" | "unset" , ...], checks : { schema : boolean, resolvability : boolean, resolvabilityComplete : boolean, }, refsChecked : number, skippedExecRefs : number, errors?: [ { kind : "missing-path" | "schema" | "resolvability" , message : string, ref?: string, // present for resolvability errors }, ], }
Success example
json Copy code { "ok" : true , "operations" : 1 , "configPath" : "~/.openclaw/openclaw.json" , "inputModes" : [ "builder" ] , "checks" : { "schema" : false , "resolvability" : true , "resolvabilityComplete" : true } , "refsChecked" : 1 , "skippedExecRefs" : 0 }
Failure example
json Copy code { "ok" : false , "operations" : 1 , "configPath" : "~/.openclaw/openclaw.json" , "inputModes" : [ "builder" ] , "checks" : { "schema" : false , "resolvability" : true , "resolvabilityComplete" : true } , "refsChecked" : 1 , "skippedExecRefs" : 0 , "errors" : [ { "kind" : "resolvability" , "message" : "Error: Environment variable \"MISSING_TEST_SECRET\" is not set." , "ref" : "env:default:MISSING_TEST_SECRET" } ] }
If dry-run fails
...
bash Copy code openclaw config set gateway.reload.mode hybrid --dry-run openclaw config set gateway.reload.mode hybrid openclaw config validate
If a write is rejected, inspect the saved payload and fix the full config shape:
bash Copy code CONFIG= " $(openclaw config file) " ls -lt " $CONFIG " .rejected.* 2>/dev/null | head openclaw config validate
...
bash Copy code openclaw config validate openclaw config validate --json
...
bash Copy code openclaw chat
Then inside the TUI:
text Copy code !openclaw config file !openclaw docs gateway auth token secretref !openclaw config validate !openclaw doctor
Typical repair loop:
Compare with docs
