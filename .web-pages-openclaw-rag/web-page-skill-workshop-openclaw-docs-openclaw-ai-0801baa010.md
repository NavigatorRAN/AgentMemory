...
- Requested URL: https://docs.openclaw.ai/tools/skill-workshop
- Final URL: https://docs.openclaw.ai/tools/skill-workshop
- Canonical URL: https://docs.openclaw.ai/tools/skill-workshop
- Fetched at: 2026-06-23T14:58:14Z
...
Workshop service.
Lifecycle
text Copy code create/update -> pending revise -> pending apply -> applied reject -> rejected quarantine -> quarantined target change -> stale
...
Create:
text Copy code Make a skill called morning-catchup that runs my Monday inbox routine.
Update an existing workspace skill:
text Copy code Update trip-planning to also check seat maps before booking.
Iterate on a pending proposal:
text Copy code Show me the morning-catchup proposal. Revise it to also flag anything marked urgent. Apply the morning-catchup proposal.
...
bash Copy code openclaw skills workshop propose-create \ --name morning-catchup \ --description "Daily inbox catch-up: triage, archive, surface, draft, plan" \ --proposal ./PROPOSAL.md
Create an update proposal for an existing workspace skill:
bash Copy code openclaw skills workshop propose-update trip-planning --proposal ./PROPOSAL.md
List and inspect:
bash Copy code openclaw skills workshop list openclaw skills workshop inspect <proposal-id>
Revise before approval:
bash Copy code openclaw skills workshop revise <proposal-id> --proposal ./PROPOSAL.md
Close out the proposal:
bash Copy code openclaw skills workshop apply <proposal-id> openclaw skills workshop reject <proposal-id> --reason "Duplicate" openclaw skills workshop quarantine <proposal-id> --reason "Needs security review"
Proposal content
While pending, the proposal is stored as PROPOSAL.md with proposal-only
frontmatter:
markdown Copy code --- name: "morning-catchup" description: "Daily inbox catch-up: triage, archive, surface, draft, plan" status: proposal version: "v1" date: "2026-05-30T00:00:00.000Z" ---
...
bash Copy code openclaw skills workshop propose-create \ --name weekly-update \ --description "Friday wrap-up: stats, highlights, next week's top three" \ --proposal-dir ./weekly-update-proposal
...
text Copy code action: create | update | revise | list | inspect | apply | reject | quarantine
...
json5 Copy code { skills : { workshop : { autonomous : { enabled : false , }, allowSymlinkTargetWrites : false , approvalPolicy : "pending" , maxPending : 50 , maxSkillBytes : 40000 , }, }, }
...
Gateway methods
text Copy code skills.proposals.list skills.proposals.inspect skills.proposals.create skills.proposals.update skills.proposals.revise skills.proposals.apply skills.proposals.reject skills.proposals.quarantine
Read-only methods require operator.read . Mutating methods require
operator.admin .
Storage
text Copy code <OPENCLAW_STATE_DIR>/skill-workshop/ proposals.json proposals/<proposal-id>/ proposal.json PROPOSAL.md rollback.json assets/ examples/ references/ scripts/ templates/
