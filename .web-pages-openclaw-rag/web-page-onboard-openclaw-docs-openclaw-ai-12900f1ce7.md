...
- Requested URL: https://docs.openclaw.ai/cli/onboard
- Final URL: https://docs.openclaw.ai/cli/onboard
- Canonical URL: https://docs.openclaw.ai/cli/onboard
- Fetched at: 2026-06-23T14:37:55Z
...
Examples
bash Copy code openclaw onboard openclaw onboard --modern openclaw onboard --flow quickstart openclaw onboard --flow manual openclaw onboard --flow import openclaw onboard --import-from hermes --import-source ~/.hermes openclaw onboard --skip-bootstrap openclaw onboard --mode remote --remote-url wss://gateway-host:18789
...
remain literal.
Example:
bash Copy code OPENCLAW_LOCALE=zh-CN openclaw onboard
Non-interactive custom provider:
bash Copy code openclaw onboard --non-interactive \ --auth-choice custom-api-key \ --custom-base-url "https://llm.example.com/v1" \ --custom-model-id "foo-large" \ --custom-api-key " $CUSTOM_API_KEY " \ --secret-input-mode plaintext \ --custom-compatibility openai \ --custom-image-input
...
bash Copy code openclaw onboard --non-interactive \ --auth-choice lmstudio \ --custom-base-url "http://localhost:1234/v1" \ --custom-model-id "qwen/qwen3.5-9b" \ --lmstudio-api-key " $LM_API_TOKEN " \ --accept-risk
...
bash Copy code openclaw onboard --non-interactive \ --auth-choice openai-api-key \ --secret-input-mode ref \ --accept-risk
...
bash Copy code export OPENCLAW_GATEWAY_TOKEN= "your-token" openclaw onboard --non-interactive \ --mode local \ --auth-choice skip \ --gateway-auth token \ --gateway-token-ref-env OPENCLAW_GATEWAY_TOKEN \ --accept-risk
...
zai-coding-cn .
bash Copy code # Promptless endpoint selection openclaw onboard --non-interactive \ --auth-choice zai-coding-global \ --zai-api-key " $ZAI_API_KEY " # Other Z.AI endpoint choices: # --auth-choice zai-coding-cn # --auth-choice zai-global # --auth-choice zai-cn
Non-interactive Mistral example:
bash Copy code openclaw onboard --non-interactive \ --auth-choice mistral-api-key \ --mistral-api-key " $MISTRAL_API_KEY "
Flow notes
Flow types
...
bash Copy code openclaw channels add openclaw configure openclaw agents add <name>
