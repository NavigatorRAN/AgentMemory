...
- Requested URL: https://docs.openclaw.ai/gateway/authentication
- Final URL: https://docs.openclaw.ai/gateway/authentication
- Canonical URL: https://docs.openclaw.ai/gateway/authentication
- Fetched at: 2026-06-23T14:43:27Z
...
bash Copy code export <PROVIDER>_API_KEY= "..." openclaw models status
If the Gateway runs under systemd/launchd, prefer putting the key in
~/.openclaw/.env so the daemon can read it:
bash Copy code cat >> ~/.openclaw/.env << 'EOF' <PROVIDER>_API_KEY=... EOF
Then restart the daemon (or restart your Gateway process) and re-check:
bash Copy code openclaw models status openclaw doctor
...
bash Copy code # Run on the gateway host claude auth login claude auth status --text openclaw models auth login --provider anthropic --method cli --set-default
This is a two-step setup:
Log Claude Code itself into Anthropic on the gateway host.
Tell OpenClaw to switch Anthropic model selection to the local claude-cli
...
bash Copy code openclaw models auth paste-token --provider openrouter
The auth profile store keeps credentials only. Legacy auth-profiles.json files used this canonical shape:
json Copy code { "version" : 1 , "profiles" : { "openrouter:default" : { "type" : "api_key" , "provider" : "openrouter" , "key" : "OPENROUTER_API_KEY" } } }
...
bash Copy code openclaw models status --check
Live auth probes:
...
bash Copy code openclaw doctor --fix openclaw models auth list --provider openai
...
OpenAI-specific model/runtime routing, see OpenAI .
During login (CLI)
...
bash Copy code openclaw models auth login --provider anthropic --force
...
bash Copy code openclaw models auth order get --provider anthropic openclaw models auth order set --provider anthropic anthropic:default openclaw models auth order clear --provider anthropic
