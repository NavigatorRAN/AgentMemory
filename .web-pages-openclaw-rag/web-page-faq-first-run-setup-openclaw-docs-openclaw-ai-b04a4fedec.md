...
- Requested URL: https://docs.openclaw.ai/help/faq-first-run
- Final URL: https://docs.openclaw.ai/help/faq-first-run
- Canonical URL: https://docs.openclaw.ai/help/faq-first-run
- Fetched at: 2026-06-23T14:44:21Z
...
and troubleshooting see the main FAQ .
...
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method git
...
https://github.com/openclaw/openclaw/issues https://github.com/openclaw/openclaw/pulls
Start with these commands (share outputs when asking for help):
bash Copy code openclaw status openclaw models status openclaw doctor
What they do:
...
Quick debug loop: First 60 seconds if something is broken .
...
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash openclaw onboard --install-daemon
The wizard can also build UI assets automatically. After onboarding, you typically run the Gateway on port 18789 .
From source (contributors/dev):
bash Copy code git clone https://github.com/openclaw/openclaw.git cd openclaw pnpm install pnpm build pnpm ui:build openclaw onboard
...
bash Copy code curl -fsSL --proto '=https' --tlsv1.2 https://openclaw.ai/install.sh | bash -s -- --beta bash Copy code curl -fsSL --proto '=https' --tlsv1.2 https://openclaw.ai/install.sh | bash -s -- --install-method git
...
bash Copy code openclaw update --channel dev
This switches to the main branch and updates from source.
Hackable install (from the installer site):
That gives you a local repo you can edit, then update via git.
If you prefer a clean clone manually, use:
bash Copy code git clone https://github.com/openclaw/openclaw.git cd openclaw pnpm install pnpm build
Docs: Update , Development channels ,
Install .
How long does install and onboarding usually take?
Rough guide:
Install: 2-5 minutes
Onboarding: 5-15 minutes depending on how many channels/models you configure
If it hangs, use Installer stuck
and the fast debug loop in I am stuck .
Installer stuck? How do I get more feedback?
Re-run the installer with verbose output :
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash -s -- --verbose
Beta install with verbose:
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash -s -- --beta --verbose
For a hackable (git) install:
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method git --verbose
...
Check the path:
powershell Copy code npm config get prefix
...
powershell Copy code chcp 65001 [ Console ]::InputEncoding = [ System.Text.UTF8Encoding ]::new( $false ) [ Console ]::OutputEncoding = [ System.Text.UTF8Encoding ]::new( $false ) $OutputEncoding = [ System.Text.UTF8Encoding ]::new( $false )
Then restart the Gateway and retry your command:
powershell Copy code openclaw gateway restart
...
Hub: Platforms . Remote access: Gateway remote .
...
Use the CLI:
bash Copy code openclaw update openclaw update status openclaw update --channel stable|beta|dev openclaw update --tag <dist-tag|version> openclaw update --no-restart
If you must automate from an agent:
bash Copy code openclaw update -- yes --no-restart openclaw gateway restart
...
See Model providers and Onboarding (CLI) .
...
See OAuth , Model providers , and Onboarding (CLI) .
How do I set up Gemini CLI OAuth?
Gemini CLI uses a plugin auth flow , not a client id or secret in openclaw.json .
Steps:
Install Gemini CLI locally so gemini is on PATH
Homebrew: brew install gemini-cli
npm: npm install -g @google/gemini-cli
Enable the plugin: openclaw plugins enable google
Login: openclaw models auth login --provider google-gemini-cli --set-default
Default model after login: google-gemini-cli/gemini-3-flash-preview
If requests fail, set GOOGLE_CLOUD_PROJECT or GOOGLE_CLOUD_PROJECT_ID on the gateway host
...
You only need a Mac for macOS-only tools . For iMessage, use iMessage with imsg on any Mac signed into Messages. If the Gateway runs on Linux or elsewhere, set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac. If you want other macOS-only tools, run the Gateway on a Mac or pair a macOS node.
Docs: iMessage , Nodes , Mac remote mode .
Do I need a Mac mini for iMessage support?
You need some macOS device signed into Messages. It does not have to be a Mac mini -
any Mac works. Use iMessage with imsg ; the Gateway can run on that Mac, or it can run elsewhere with an SSH wrapper cliPath .
Common setups:
...
bash Copy code /bin/bash -c " $(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh) " echo 'eval "$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)"' >> ~/.profile eval " $(/home/linuxbrew/.linuxbrew/bin/brew shellenv) " brew install <formula>
...
From npm to git:
From git to npm:
bash Copy code openclaw update --channel stable
...
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method git curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method npm
...
OpenClaw-specific note: WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is headless browser vs a visible window. See Browser .
