...
- Requested URL: https://docs.openclaw.ai/concepts/context-engine
- Final URL: https://docs.openclaw.ai/concepts/context-engine
- Canonical URL: https://docs.openclaw.ai/concepts/context-engine
- Fetched at: 2026-06-23T14:38:37Z
...
bash Copy code openclaw doctor # or inspect config directly: cat ~/.openclaw/openclaw.json | jq '.plugins.slots.contextEngine'
Install a plugin engine
Context engine plugins are installed like any other OpenClaw plugin.
From npm
bash Copy code openclaw plugins install @martian-engineering/lossless-claw
From a local path
bash Copy code openclaw plugins install -l ./my-context-engine
Enable and select the engine
json5 Copy code // openclaw.json { plugins : { slots : { contextEngine : "lossless-claw" , // must match the plugin's registered engine id }, entries : { "lossless-claw" : { enabled : true , // Plugin-specific config goes here (see the plugin's docs) }, }, }, }
...
json5 Copy code { plugins : { slots : { contextEngine : "my-engine" , }, entries : { "my-engine" : { enabled : true , }, }, }, }
The ContextEngine interface
Required members:
Member Kind Purpose info Property Engine id, name, version, and whether it owns compaction ingest(params) Method Store a single message assemble(params) Method Build context for a model run (returns AssembleResult ) compact(params) Method Summarize/reduce context
...
ts Copy code info : { id : "my-context-engine" , name : "My Context Engine" , hostRequirements : { "agent-run" : { requiredCapabilities : [ "assemble-before-prompt" ], unsupportedMessage : "Use the native Codex or OpenClaw embedded runtime, or select the legacy context engine." , }, }, }
...
json5 Copy code { plugins : { slots : { // Select the active context engine. Default: "legacy". // Set to a plugin id to use a plugin engine. contextEngine : "legacy" , }, }, } Note
