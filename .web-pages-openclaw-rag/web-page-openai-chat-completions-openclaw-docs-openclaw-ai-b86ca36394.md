...
- Requested URL: https://docs.openclaw.ai/gateway/openai-http-api
- Final URL: https://docs.openclaw.ai/gateway/openai-http-api
- Canonical URL: https://docs.openclaw.ai/gateway/openai-http-api
- Fetched at: 2026-06-23T14:43:55Z
...
For native mobile clients that connect directly to a remote gateway, prefer WebChat or the Gateway Protocol and implement the paired-device bootstrap/device-token flow so the device does not need a shared HTTP token/password.
Build a channel plugin instead when you are integrating an external messaging network with its own users, rooms, webhook delivery, or outbound transport. See Building plugins .
...
json5 Copy code { gateway : { http : { endpoints : { chatCompletions : { enabled : true }, }, }, }, }
Disabling the endpoint
Set gateway.http.endpoints.chatCompletions.enabled to false :
json5 Copy code { gateway : { http : { endpoints : { chatCompletions : { enabled : false }, }, }, }, }
Session behavior
...
Examples:
x-openclaw-model: openai/gpt-5.4 x-openclaw-model: gpt-5.5
...
Quick smoke:
bash Copy code curl -sS http://127.0.0.1:18789/v1/models \ -H 'Authorization: Bearer YOUR_TOKEN'
...
bash Copy code curl -sS http://127.0.0.1:18789/v1/chat/completions \ -H 'Authorization: Bearer YOUR_TOKEN' \ -H 'Content-Type: application/json' \ -d '{ "model" : "openclaw/default" , "user" : "conv:YOUR_CONVERSATION_ID" , "messages" : [{ "role" : "user" , "content" : "Summarize my tasks for today" }] } '
Reuse the same user value on later calls for that conversation to continue the same agent session.
Non-streaming:
bash Copy code curl -sS http://127.0.0.1:18789/v1/chat/completions \ -H 'Authorization: Bearer YOUR_TOKEN' \ -H 'Content-Type: application/json' \ -d '{ "model" : "openclaw/default" , "messages" : [{ "role" : "user" , "content" : "hi" }] } '
Streaming:
bash Copy code curl -N http://127.0.0.1:18789/v1/chat/completions \ -H 'Authorization: Bearer YOUR_TOKEN' \ -H 'Content-Type: application/json' \ -H 'x-openclaw-model: openai/gpt-5.4' \ -d '{ "model" : "openclaw/research" , "stream" : true , "messages" : [{ "role" : "user" , "content" : "hi" }] } '
List models:
Fetch one model:
bash Copy code curl -sS http://127.0.0.1:18789/v1/models/openclaw%2Fdefault \ -H 'Authorization: Bearer YOUR_TOKEN'
Create embeddings:
bash Copy code curl -sS http://127.0.0.1:18789/v1/embeddings \ -H 'Authorization: Bearer YOUR_TOKEN' \ -H 'Content-Type: application/json' \ -H 'x-openclaw-model: openai/text-embedding-3-small' \ -d '{ "model" : "openclaw/default" , "input" : [ "alpha" , "beta" ] } '
