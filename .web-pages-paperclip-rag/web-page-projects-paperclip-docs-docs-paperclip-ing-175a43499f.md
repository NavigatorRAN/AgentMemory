# Projects | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/guides/projects-workflow/projects
- Final URL: https://docs.paperclip.ing/guides/projects-workflow/projects/
- Canonical URL: https://docs.paperclip.ing/guides/projects-workflow/projects
- Fetched at: 2026-06-23T13:38:08Z
- Content type: text/html; charset=utf-8

## Description

A project is the container Paperclip uses to group related work. If goals answer "why are we doing this?" and issues answer "what exactly needs doing?", projects sit in between: th
...
Each issue may be tied to an execution workspace — a specific working copy of the project's code — when the issue is picked up by an agent. The Workspaces tab (next section) groups issues by the workspace they are running in, which is often a more useful lens once work is in flight. See Execution Workspaces for the underlying model.
Workspaces Tab
...
The project's primary working directory is not shown as a separate row here — it is the implicit default every issue runs in when no isolated workspace is provisioned. This tab lists the non-default workspaces only. For the full mental model of primary, isolated, and reused workspaces, see Execution Workspaces .
...
Goals — the list of goals the project is linked to. Add or remove goal links here; linked goals appear on the Overview area and in the properties panel. See the Goals guide for the goal model.
...
These settings map to the behaviours described in Execution Workspaces . You will usually set them once per project and rarely touch them again.
...
Project budgets are useful when a single project needs a hard spending ceiling independent of the agent doing the work. For a first principles tour of budgeting, see Costs & Budgets .
...
See Costs & Budgets for how these three interact, how to read the company-wide cost overview, and how to resolve a budget override approval when a hard stop is reached.
Resume after a hard stop
When the project is paused by a hard stop, you have two options:
Raise the amount from the Budget tab. Once the new amount exceeds observed spend, the project can resume on the next run.
Resolve the budget override approval from the approvals queue. Budget approvals are the formal path an agent uses to ask for more headroom without a human manually raising the cap.
Both flows are described in more detail in the approvals and costs guides.
Where to go next
Task-level work: Managing Tasks .
The goal model that sits above projects: Goals .
The execution surface beneath projects: Execution Workspaces .
Budget mechanics in more depth: Costs & Budgets .
