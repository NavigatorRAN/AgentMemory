...
- Requested URL: https://docs.openclaw.ai/cli/devices
- Final URL: https://docs.openclaw.ai/cli/devices
- Canonical URL: https://docs.openclaw.ai/cli/devices
- Fetched at: 2026-06-23T14:37:33Z
...
Code Copy code openclaw devices list openclaw devices list --json
...
operator.admin .
Code Copy code openclaw devices remove <deviceId> openclaw devices remove <deviceId> --json
openclaw devices clear --yes [--pending]
Clear paired devices in bulk.
Code Copy code openclaw devices clear --yes openclaw devices clear --yes --pending openclaw devices clear --yes --pending --json
...
Operator scopes for the approval-time checks.
Code Copy code openclaw devices approve openclaw devices approve <requestId> openclaw devices approve --latest
...
bash Copy code openclaw devices approve --latest
...
bash Copy code openclaw devices approve --latest --url <gateway-ws-url> --token <gateway-token>
...
json Copy code { "adapterConfig" : { "devicePrivateKeyPem" : "<ed25519-private-key-pkcs8-pem>" } }
If approval keeps failing, run openclaw devices list first to confirm a pending request exists.
openclaw devices reject <requestId>
Reject a pending device pairing request.
Code Copy code openclaw devices reject <requestId>
...
caller already has.
Code Copy code openclaw devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
...
Code Copy code openclaw devices revoke --device <deviceId> --role node
...
operator.admin ; see Operator scopes .
...
bash Copy code openclaw config get gateway.auth.token
...
bash Copy code openclaw devices rotate --device <deviceId> --role operator
If rotation is not enough, remove stale pairing and approve again:
bash Copy code openclaw devices remove <deviceId> openclaw devices list openclaw devices approve <requestId>
