...
- Requested URL: https://docs.openclaw.ai/gateway/sandbox-vs-tool-policy-vs-elevated
- Final URL: https://docs.openclaw.ai/gateway/sandbox-vs-tool-policy-vs-elevated
- Canonical URL: https://docs.openclaw.ai/gateway/sandbox-vs-tool-policy-vs-elevated
- Fetched at: 2026-06-23T14:44:07Z
...
Sandbox ( agents.defaults.sandbox.* / agents.list[].sandbox.* ) decides where tools run (sandbox backend vs host).
Tool policy ( tools.* , tools.sandbox.tools.* , agents.list[].tools.* ) decides which tools are available/allowed .
Elevated ( tools.elevated.* , agents.list[].tools.elevated.* ) is an exec-only escape hatch to run outside the sandbox when you're sandboxed ( gateway by default, or node when the exec target is configured to node ).
Quick debug
...
json5 Copy code { tools : { sandbox : { tools : { allow : [ "group:runtime" , "group:fs" , "group:sessions" , "group:memory" ], }, }, }, }
Available groups:
