...
- Requested URL: https://docs.openclaw.ai/cli/gateway
- Final URL: https://docs.openclaw.ai/cli/gateway
- Canonical URL: https://docs.openclaw.ai/cli/gateway
- Fetched at: 2026-06-23T14:37:40Z
...
Restart the Gateway
bash Copy code openclaw gateway restart openclaw gateway restart --safe openclaw gateway restart --safe --skip-deferral openclaw gateway restart --force
...
gateway health
bash Copy code openclaw gateway health --url ws://127.0.0.1:18789 openclaw gateway health --port 18789
...
bash Copy code openclaw gateway usage-cost openclaw gateway usage-cost --days 7 openclaw gateway usage-cost --agent work --json openclaw gateway usage-cost --all-agents openclaw gateway usage-cost --json
...
bash Copy code openclaw gateway stability openclaw gateway stability -- type payload.large openclaw gateway stability --bundle latest openclaw gateway stability --bundle latest -- export openclaw gateway stability --json
...
bash Copy code openclaw gateway diagnostics export openclaw gateway diagnostics export --output openclaw-diagnostics.zip openclaw gateway diagnostics export --json
...
bash Copy code openclaw gateway status openclaw gateway status --json openclaw gateway status --require-rpc
...
bash Copy code openclaw gateway probe openclaw gateway probe --json openclaw gateway probe --port 18789
...
CLI equivalent:
bash Copy code openclaw gateway probe --ssh user@gateway-host
...
bash Copy code openclaw gateway call status openclaw gateway call logs.tail --params '{"sinceMs": 60000}'
Mainly for agent-style RPCs that stream intermediate events before a final payload.
Machine-readable JSON output.
--params must be valid JSON.
Manage the Gateway service
bash Copy code openclaw gateway install openclaw gateway start openclaw gateway stop openclaw gateway restart openclaw gateway uninstall
...
bash Copy code cat > ~/.local/bin/openclaw-doppler << 'EOF' #!/usr/bin/env bash set -euo pipefail exec doppler run --project my-project --config production -- openclaw " $@ " EOF chmod +x ~/.local/bin/openclaw-doppler openclaw gateway install --wrapper ~/.local/bin/openclaw-doppler --force openclaw gateway restart
...
repairs.
bash Copy code OPENCLAW_WRAPPER= " $HOME /.local/bin/openclaw-doppler" openclaw gateway install --force openclaw doctor
To remove a persisted wrapper, clear OPENCLAW_WRAPPER while reinstalling:
bash Copy code OPENCLAW_WRAPPER= openclaw gateway install --force openclaw gateway restart
Command options
...
gateway discover
bash Copy code openclaw gateway discover
Machine-readable output (also disables styling/spinner).
Examples:
bash Copy code openclaw gateway discover -- timeout 4000 openclaw gateway discover --json | jq '.beacons[].wsUrl' Note
