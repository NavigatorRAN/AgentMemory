# Claw Supervisor - OpenClaw

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.openclaw.ai/specs/claw-supervisor
- Final URL: https://docs.openclaw.ai/specs/claw-supervisor
- Canonical URL: https://docs.openclaw.ai/specs/claw-supervisor
- Fetched at: 2026-06-23T14:57:02Z
- Content type: text/html; charset=utf-8

## Description

Fleet supervision plan for Codex app-server sessions controlled by OpenClaw.
...
Architecture
text Copy code user SSH session -> codex --remote unix://... or ws://... -> local codex app-server daemon <-> host sidecar / supervisor connector <-> OpenClaw fleet supervisor <-> supervisor MCP exposed back to Codex
...
json Copy code { "sessionId" : "codex-thread-id" , "endpointId" : "host-a" , "host" : "host-a.example" , "workspace" : "/workspace/repo" , "repo" : "owner/repo" , "branch" : "feature/example" , "source" : "vscode" , "status" : "idle" , "humanAttached" : true , "lastSeenAt" : "2026-05-28T10:00:00.000Z" , "summary" : "Short working-state summary" }
...
User SSHes into a CRAB host.
SSH service starts or verifies codex app-server daemon start .
Login wrapper launches codex --remote unix:// --cd <workspace> .
Host connector registers endpoint and loaded thread.
Supervisor emits a high-priority fleet event: new Codex session, workspace, human-attached state, current task preview.
Supervisor Claw can read and steer immediately.
Autonomous spawn:
Supervisor selects host and workspace.
Host connector opens or resumes a Codex app-server thread.
Supervisor starts the first turn with task text and MCP config.
Session registry marks it autonomous and attachable.
Human can later attach with codex --remote <endpoint> resume <threadId> once Codex supports that exact UX, or via current resume flow on the same app-server.
Deployment
