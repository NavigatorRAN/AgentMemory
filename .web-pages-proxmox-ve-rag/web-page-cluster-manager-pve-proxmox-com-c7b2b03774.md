...
- Requested URL: https://pve.proxmox.com/pve-docs/chapter-pvecm.html
- Final URL: https://pve.proxmox.com/pve-docs/chapter-pvecm.html
- Fetched at: 2026-06-23T13:55:37Z
...
By default, pvecm add authenticates against the existing cluster
node’s API, prompting for that node’s root@pam password and verifying
its SHA-256 certificate fingerprint. SSH password authentication is not
required for this default flow, so administrators may keep
PasswordAuthentication no set in sshd_config .
Pass --use_ssh to fall back to the legacy SSH-based join, which uses
ssh-copy-id to merge the existing cluster’s root key onto the new
node. That flow does require root SSH login (with password or key) on
the existing node.
...
rebalance in Ceph.
By default, Ceph pools have a size/min_size of 3/2 and a
...
Ensure that sufficient monitors ,
managers and, if using CephFS,
metadata servers remain available in the Ceph
cluster.
...
the Ceph-, and Proxmox VE corosync-cluster.
node4# pvecm nodes
1 1 node1
2 1 node2
3 1 node3
...
steps:
Start by destroying the remaining Ceph OSDs on the
node to be deleted, one after another.
Wait until the CEPH status reaches HEALTH_OK
again.
If any exists, destroy the remaining Ceph metadata servers
via the web interface at Ceph → CephFS or by running:
node4# pveceph mds destroy node4
Destroy the remaining Ceph monitor.
Destroy the remaining Ceph manager.
Finally, remove the now empty Ceph bucket (Proxmox VE node to be removed) from the
CRUSH hierarchy.
node4# ceph osd crush remove node4
Now, power off node4 and ensure that it will not power on again in this
network with its current configuration.
...
Log into one of the remaining cluster nodes and remove the node
node4 from the cluster.
node1# pvecm delnode node4
It is possible that you will receive an error message stating
...
safely ignored.
Verify the node was successfully removed from the cluster
node1# pvecm nodes
1 1 node1 (local)
Cleanup Steps After Node Removal
The configuration files of the removed node will still reside in
/etc/pve/nodes/node4 in the cluster filesystem. Recover any
configuration you still need and remove the directory afterwards.
The SSH fingerprint of the removed node will still reside in the
known_hosts file on the other nodes. If you receive an SSH error
after rejoining a node with the same IP or hostname, run pvecm
updatecerts once on the re-added node to update its fingerprint
cluster wide.
Existing high availability rules will still
reference the deleted node, therefore remove the node from any
existing rules.
The public key of the removed node will still be listed in
/etc/pve/priv/authorized_keys and should be deleted if it is no
longer needed.
Rejoin the same node again:
If you want the same server to join the same cluster again, you have to:
Reinstall Proxmox VE on the server,
then rejoin the node to the cluster
...
you would execute:
pvecm create test --link 0 10.10 . 10.1
...
[ token in the corosync manual page
https://manpages.debian.org/stable/corosync/corosync.conf.5.en.html#token ]
. The
consensus timeout
[ consensus in the corosync manual page
...
time needed for reestablishing a new cluster membership after a node failure.
...
watchdog timeout of 60 seconds expires, which would
trigger a node fence. A safety margin below this 45 second limit is advisable
so that transient timing variations do not push the cluster past the watchdog
...
corosync.conf, edit corosync.conf and add
token_coefficient: 125 to the totem section. Do not forget to
increase the config_version .
Otherwise, if 125 milliseconds is not low enough to keep the timeouts within
the limits above, select a token_coefficient with which the token and
consensus timeouts sum up to at most 45 seconds. By default, corosync computes
the token and consensus timeouts (in milliseconds) according to the following
formula:
token = 3000 + (number_of_nodes - 2) * token_coefficient
consensus = 1.2 * token
Edit corosync.conf and set an appropriate
token_coefficient option in the totem section. Do not forget to
increase the config_version . Test your setup
