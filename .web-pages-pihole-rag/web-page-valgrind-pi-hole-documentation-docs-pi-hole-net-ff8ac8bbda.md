# valgrind - Pi-hole documentation

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.pi-hole.net/ftldns/valgrind
- Final URL: https://docs.pi-hole.net/ftldns/valgrind/
- Canonical URL: https://docs.pi-hole.net/ftldns/valgrind/
- Fetched at: 2026-06-23T13:42:08Z
...
trace-children=yes - Valgrind will trace into sub-processes initiated via the exec system call. This is necessary for multi-process programs. We use this to go down into possibly user scripts on DHCP activity, etc.
leak-check=full - When enabled, search for memory leaks when the client program finishes. Each individual leak will be shown in detail and/or counted as an error.
track-origins=yes - Memcheck tracks the origin of uninitialised values. By default, it does not, which means that although it can tell you that an uninitialised value is being used in a dangerous way, it cannot tell you where the uninitialised value came from. This often makes it difficult to track down the root problem.
...
Prepare gdb as described in the gdb guide .
Start pihole-FTL in valgrind as described above. The --vgdb=full option tells valgrind to start a GDB server.
Once FTL has started, you can attach gdb to the running process using
...
main_dnsmasq : Syscall param sendmsg(msg.msg_control) points to uninitialised byte(s)
...
main_dnsmasq : Syscall param write(buf) points to uninitialised byte(s)
...
gethostbyaddr : 28 bytes in 1 blocks are definitely lost
...
gravityDB_open : (some) bytes in 1 blocks are definitely lost
