...
- Requested URL: https://docs.openclaw.ai/providers/ds4
- Final URL: https://docs.openclaw.ai/providers/ds4
- Canonical URL: https://docs.openclaw.ai/providers/ds4
- Fetched at: 2026-06-23T14:55:14Z
...
metadata:
json5 Copy code { contextWindow : 393216 , maxTokens : 384000 , compat : { supportsUsageInStreaming : true , supportsReasoningEffort : true , maxTokensField : "max_tokens" , supportsStrictMode : false , thinkingFormat : "deepseek" , supportedReasoningEfforts : [ "low" , "medium" , "high" , "xhigh" , "max" ], }, }
Test
Start with a direct HTTP check:
bash Copy code curl http://127.0.0.1:18000/v1/chat/completions \ -H 'content-type: application/json' \ -d '{"model":"deepseek-v4-flash","messages":[{"role":"user","content":"Reply with exactly: ds4-ok"}],"max_tokens":16,"stream":false,"thinking":{"type":"disabled"}}'
Then test OpenClaw model routing:
For a full agent and tool-call smoke, use a context of at least 32768:
bash Copy code openclaw agent \ -- local \ --session-id ds4-tool-smoke \ --model ds4/deepseek-v4-flash \ --thinking off \ --message "Use the shell command pwd once, then reply exactly: tool-ok <output>" \ --json \ -- timeout 240
Expected result:
...
bash Copy code curl http://127.0.0.1:18000/v1/models 500 prompt exceeds context
