...
- Requested URL: https://docs.openclaw.ai/concepts/parallel-specialist-lanes
- Final URL: https://docs.openclaw.ai/concepts/parallel-specialist-lanes
- Canonical URL: https://docs.openclaw.ai/concepts/parallel-specialist-lanes
- Fetched at: 2026-06-23T14:39:00Z
...
the command queue . Specialist lanes add policy on top:
...
json5 Copy code { agents : { defaults : { maxConcurrent : 4 , subagents : { maxConcurrent : 8 , delegationMode : "prefer" }, }, }, messages : { queue : { mode : "collect" , debounceMs : 1000 , cap : 20 , drop : "summarize" , }, }, }
...
md Copy code # Lane contract ## Owns - < job this lane is responsible for > ## Does not own - < work to hand off > ## Chat budget - Answer quick questions directly. - For multi-step, slow, or tool-heavy work: acknowledge briefly, spawn/background the work, then return the result when complete. ## Handoff If another lane owns the request, reply with: - target lane - objective - relevant context - exact next action ## Tool posture Use the smallest tool surface that can complete the task. Avoid broad shell or network work unless this lane explicitly owns it.
Related
Multi-agent routing
Command queue
Sub-agents
