# Matrix push rules for quiet previews - OpenClaw

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.openclaw.ai/channels/matrix-push-rules
- Final URL: https://docs.openclaw.ai/channels/matrix-push-rules
- Canonical URL: https://docs.openclaw.ai/channels/matrix-push-rules
- Fetched at: 2026-06-23T14:36:21Z
...
json5 Copy code { channels : { matrix : { streaming : "quiet" , }, }, }
Get the recipient's access token
Reuse an existing client session token where possible. To mint a fresh one:
bash Copy code curl -sS -X POST \ "https://matrix.example.org/_matrix/client/v3/login" \ -H "Content-Type: application/json" \ --data '{ "type" : "m.login.password" , "identifier" : { "type" : "m.id.user" , "user" : "@alice:example.org" }, "password" : "REDACTED" } '
Verify pushers exist
bash Copy code curl -sS \ -H "Authorization: Bearer $USER_ACCESS_TOKEN " \ "https://matrix.example.org/_matrix/client/v3/pushers"
...
Verify
bash Copy code curl -sS \ -H "Authorization: Bearer $USER_ACCESS_TOKEN " \ "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/openclaw-finalized-preview-botname"
