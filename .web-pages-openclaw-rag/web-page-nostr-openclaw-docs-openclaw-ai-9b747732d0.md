...
- Requested URL: https://docs.openclaw.ai/channels/nostr
- Final URL: https://docs.openclaw.ai/channels/nostr
- Canonical URL: https://docs.openclaw.ai/channels/nostr
- Fetched at: 2026-06-23T14:36:27Z
...
bash Copy code openclaw plugins install @openclaw/nostr
Use the bare package to follow the current official release tag. Pin an exact
version only when you need a reproducible install.
Use a local checkout (dev workflows):
bash Copy code openclaw plugins install -- link <path-to-local-nostr-plugin>
Restart the Gateway after installing or enabling plugins.
Non-interactive setup
bash Copy code openclaw channels add --channel nostr --private-key " $NOSTR_PRIVATE_KEY " openclaw channels add --channel nostr --private-key " $NOSTR_PRIVATE_KEY " --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
...
bash Copy code # Using nak nak key generate
Add to config:
json5 Copy code { channels : { nostr : { privateKey : "${NOSTR_PRIVATE_KEY}" , }, }, }
Export the key:
bash Copy code export NOSTR_PRIVATE_KEY= "nsec1..."
Restart the Gateway.
...
Example:
json5 Copy code { channels : { nostr : { privateKey : "${NOSTR_PRIVATE_KEY}" , profile : { name : "openclaw" , displayName : "OpenClaw" , about : "Personal assistant DM bot" , picture : "https://example.com/avatar.png" , banner : "https://example.com/banner.png" , website : "https://example.com" , nip05 : "openclaw@example.com" , lud16 : "openclaw@example.com" , }, }, }, }
Notes:
...
Allowlist example
json5 Copy code { channels : { nostr : { privateKey : "${NOSTR_PRIVATE_KEY}" , dmPolicy : "allowlist" , allowFrom : [ "npub1abc..." , "npub1xyz..." ], }, }, }
Key formats
Accepted formats:
...
json5 Copy code { channels : { nostr : { privateKey : "${NOSTR_PRIVATE_KEY}" , relays : [ "wss://relay.damus.io" , "wss://relay.primal.net" , "wss://nostr.wine" ], }, }, }
Tips:
...
Protocol support
NIP Status Description NIP-01 Supported Basic event format + profile metadata NIP-04 Supported Encrypted DMs ( kind:4 ) NIP-17 Planned Gift-wrapped DMs NIP-44 Planned Versioned encryption
Testing
Local relay
bash Copy code # Start strfry docker run -p 7777:7777 ghcr.io/hoytech/strfry json5 Copy code { channels : { nostr : { privateKey : "${NOSTR_PRIVATE_KEY}" , relays : [ "ws://localhost:7777" ], }, }, }
Manual test
