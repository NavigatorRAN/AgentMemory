# Export & Import | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/guides/power/export-import
- Final URL: https://docs.paperclip.ing/guides/power/export-import/
- Canonical URL: https://docs.paperclip.ing/guides/power/export-import
- Fetched at: 2026-06-23T13:38:04Z
- Content type: text/html; charset=utf-8

## Description

Once you've built a company — given it a goal, hired agents, configured their adapters, and set up projects — that configuration has real value. Export and import let you capture t
...
Export options
Option What it does --out <path> Where to save the export (required) --include <values> What to include: company , agents , projects , issues , tasks , skills — comma-separated --skills <slugs> Export only specific skills by name --projects <names> Export only specific projects
...
Strategy What happens rename (default) Appends a suffix to avoid conflicts — e.g. ceo becomes ceo-2 skip Leaves existing entities untouched; skips anything that would conflict replace Overwrites existing entities. Use with care.
...
After an import:
Open each imported agent and verify the adapter configuration looks right
Set per-agent budgets appropriate for your usage
Add any API keys or environment variables that the package declared but didn't include values for
Enable heartbeats when you're ready for agents to start working
You're set
