...
- Requested URL: https://docs.openclaw.ai/install/migrating
- Final URL: https://docs.openclaw.ai/install/migrating
- Canonical URL: https://docs.openclaw.ai/install/migrating
- Fetched at: 2026-06-23T14:44:51Z
...
bash Copy code openclaw gateway stop cd ~ tar -czf openclaw-state.tgz .openclaw
...
bash Copy code cd ~ tar -xzf openclaw-state.tgz
...
bash Copy code openclaw doctor openclaw gateway restart openclaw status
...
bash Copy code awk -F= '/^(TELEGRAM_BOT_TOKEN|DISCORD_BOT_TOKEN)=/ { print $1 "=present" }' ~/.openclaw/.env
