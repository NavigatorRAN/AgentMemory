...
- Requested URL: https://docs.openclaw.ai/cli/message
- Final URL: https://docs.openclaw.ai/cli/message
- Canonical URL: https://docs.openclaw.ai/cli/message
- Fetched at: 2026-06-23T14:37:49Z
...
Usage
Code Copy code openclaw message <subcommand> [flags]
Channel selection:
...
Shared presentation payloads: --presentation sends semantic blocks ( text , context , divider , buttons , select ) that core renders through the selected channel's declared capabilities. See Message Presentation .
...
Code Copy code openclaw message send --channel discord \ --target channel:123 --message "hi" --reply-to 456
Send a message with semantic buttons:
Code Copy code openclaw message send --channel discord \ --target channel:123 --message "Choose:" \ --presentation '{"blocks":[{"type":"buttons","buttons":[{"label":"Approve","value":"approve","style":"success"},{"label":"Decline","value":"decline","style":"danger"}]}]}'
...
bash Copy code openclaw message send --channel googlechat --target spaces/AAA... \ --message "Choose:" \ --presentation '{"title":"Deploy approval","tone":"warning","blocks":[{"type":"text","text":"Choose a path"},{"type":"buttons","buttons":[{"label":"Approve","value":"approve"},{"label":"Decline","value":"decline"}]}]}'
Create a Discord poll:
Code Copy code openclaw message poll --channel discord \ --target channel:123 \ --poll-question "Snack?" \ --poll-option Pizza --poll-option Sushi \ --poll-multi --poll-duration-hours 48
Create a Telegram poll (auto-close in 2 minutes):
Code Copy code openclaw message poll --channel telegram \ --target @mychat \ --poll-question "Lunch?" \ --poll-option Pizza --poll-option Sushi \ --poll-duration-seconds 120 --silent
Send a Teams proactive message:
Code Copy code openclaw message send --channel msteams \ --target conversation:19:abc@thread.tacv2 --message "hi"
Create a Teams poll:
Code Copy code openclaw message poll --channel msteams \ --target conversation:19:abc@thread.tacv2 \ --poll-question "Lunch?" \ --poll-option Pizza --poll-option Sushi
React in Slack:
Code Copy code openclaw message react --channel slack \ --target C123 --message-id 456 --emoji "✅"
React in a Signal group:
Code Copy code openclaw message react --channel signal \ --target signal:group:abc123 --message-id 1737630212345 \ --emoji "✅" --target-author-uuid 123e4567-e89b-12d3-a456-426614174000
Send Telegram inline buttons through generic presentation:
Code Copy code openclaw message send --channel telegram --target @mychat --message "Choose:" \ --presentation '{"blocks":[{"type":"buttons","buttons":[{"label":"Yes","value":"cmd:yes"},{"label":"No","value":"cmd:no"}]}]}'
Send a Telegram Mini App button through generic presentation:
Code Copy code openclaw message send --channel telegram --target 123456789 --message "Open app:" \ --presentation '{"blocks":[{"type":"buttons","buttons":[{"label":"Launch","webApp":{"url":"https://example.com/app"}}]}]}'
...
bash Copy code openclaw message send --channel msteams \ --target conversation:19:abc@thread.tacv2 \ --presentation '{"title":"Status update","blocks":[{"type":"text","text":"Build completed"}]}'
Send a Telegram or WhatsApp image as a document to avoid compression:
bash Copy code openclaw message send --channel telegram --target @mychat \ --media ./diagram.png --force-document
Related
CLI reference
Agent send
