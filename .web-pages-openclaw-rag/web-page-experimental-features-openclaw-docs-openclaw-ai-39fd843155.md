# Experimental features - OpenClaw

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.openclaw.ai/concepts/experimental-features
- Final URL: https://docs.openclaw.ai/concepts/experimental-features
- Canonical URL: https://docs.openclaw.ai/concepts/experimental-features
- Fetched at: 2026-06-23T14:38:40Z
...
Surface Key Use it when More Local model runtime agents.defaults.experimental.localModelLean , agents.list[].experimental.localModelLean A smaller or stricter local backend chokes on OpenClaw's full default tool surface Local Models Memory search agents.defaults.memorySearch.experimental.sessionMemory You want memory_search to index prior session transcripts and accept the extra storage/indexing cost Memory configuration reference Codex harness plugins.entries.codex.config.appServer.experimental.sandboxExecServer You want native Codex app-server 0.132.0 or newer to target an OpenClaw sandbox-backed exec-server instead of disabling Code Mode Codex harness reference Structured planning tool tools.experimental.planTool You want the structured update_plan tool exposed for multi-step work tracking in compatible runtimes and UIs Gateway configuration reference
...
openclaw infer model run --gateway --model <ref> --prompt "Reply with exactly: pong" succeeds.
A normal agent turn fails with malformed tool calls, oversized prompts, or the model ignoring its tools.
Toggling localModelLean: true clears the failure.
...
Enable
json5 Copy code { agents : { defaults : { experimental : { localModelLean : true , }, }, }, }
For one agent only:
json5 Copy code { agents : { list : [ { id : "local" , model : "lmstudio/gemma-4-e4b-it" , experimental : { localModelLean : true , }, }, ], }, }
