...
- Requested URL: https://docs.openclaw.ai/install/podman
- Final URL: https://docs.openclaw.ai/install/podman
- Canonical URL: https://docs.openclaw.ai/install/podman
- Fetched at: 2026-06-23T14:44:59Z
...
bash Copy code ./scripts/podman/setup.sh --quadlet
...
Container start:
bash Copy code ./scripts/run-openclaw-podman.sh launch
...
Onboarding:
bash Copy code ./scripts/run-openclaw-podman.sh launch setup
...
Host CLI default:
bash Copy code export OPENCLAW_CONTAINER=openclaw
Then commands such as these will run inside that container automatically:
bash Copy code openclaw dashboard --no-open openclaw gateway status --deep # includes extra service scan openclaw doctor openclaw channels login
...
bash Copy code ~/.config/containers/systemd/openclaw.container
Useful commands:
...
bash Copy code systemctl --user daemon-reload systemctl --user restart openclaw.service
For boot persistence on SSH/headless hosts, enable lingering for your current user:
bash Copy code sudo loginctl enable-linger " $(whoami) "
