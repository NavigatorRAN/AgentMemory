# Adapter UI Parser Contract | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/reference/adapters/adapter-ui-parser
- Final URL: https://docs.paperclip.ing/reference/adapters/adapter-ui-parser/
- Canonical URL: https://docs.paperclip.ing/reference/adapters/adapter-ui-parser
- Fetched at: 2026-06-23T13:38:40Z
- Content type: text/html; charset=utf-8

## Description

Paperclip streams adapter stdout to the run viewer in real time. If your adapter emits structured lines, ship a UI parser so the viewer can render tool calls, tool results, thinkin
...
How It Loads
The adapter package exposes ./ui-parser in package.json .
The server reads that module and serves it at GET /api/adapters/:type/ui-parser.js .
The browser fetches the module when the user opens a run.
...
Host expects Adapter declares Result 1.x 1.0.0 Parser loads 1.x 2.0.0 Host logs a warning and falls back to the generic parser 1.x missing Parser loads for now, but future versions may require the field
Export Shapes
...
No DOM or Node APIs.
Deterministic output for the same (line, ts) input.
Error tolerant behavior on every line.
