# Dynamic TPM/RPM Allocation | liteLLM

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.litellm.ai/docs/proxy/dynamic_rate_limit
- Final URL: https://docs.litellm.ai/docs/proxy/dynamic_rate_limit
- Canonical URL: https://docs.litellm.ai/docs/proxy/dynamic_rate_limit
- Fetched at: 2026-06-23T14:30:10Z
...
Copy as Markdown
Prevent projects from gobbling too much tpm/rpm.
See Also: Request Prioritization - Prioritize LLM API requests in high-traffic by adding them to a priority queue.
...
If key belongs to a team with metadata.priority set → use team priority
Else if key has metadata.priority set → use key priority
Else → use default_priority from config
...
Production keys can make up to 9 requests per minute (90% of 10 RPM)
Development keys can make up to 1 request per minute (10% of 10 RPM)
Keys without explicit priority get the default_priority weight (0 = 0%), which allocates 0 requests per minute (0% of 10 RPM)
Named priorities in priority_reservation and keys with default_priority operate independently
