...
- Requested URL: https://docs.openclaw.ai/tools/subagents
- Final URL: https://docs.openclaw.ai/tools/subagents
- Canonical URL: https://docs.openclaw.ai/tools/subagents
- Fetched at: 2026-06-23T14:58:20Z
...
text Copy code /subagents list /subagents log <id|#> [limit] [tools] /subagents info <id|#>
...
See Thread supporting channels below.
text Copy code /focus <subagent-label|session-key|session-id|session-label> /unfocus /agents /session idle <duration|off> /session max-age <duration|off>
Spawn behavior
...
For ACP harness sessions (Claude Code, Gemini CLI, OpenCode, or explicit Codex ACP/acpx), use sessions_spawn with runtime: "acp" when the tool advertises that runtime. See ACP delivery model when debugging completions or agent-to-agent loops. When the codex plugin is enabled, Codex chat/thread control should prefer /codex ... over ACP unless the user explicitly asks for ACP/acpx.
...
json5 Copy code { agents : { defaults : { subagents : { delegationMode : "prefer" , maxConcurrent : 4 , }, }, list : [ { id : "coordinator" , subagents : { delegationMode : "prefer" }, }, ], }, }
Tool parameters
...
Manual controls
Command Effect /focus <target> Bind the current thread (or create one) to a sub-agent/session target /unfocus Remove the binding for the current bound thread /agents List active runs and binding state ( thread:<id> or unbound ) /session idle Inspect/update idle auto-unfocus (focused bound threads only) /session max-age Inspect/update hard cap (focused bound threads only)
Config switches
...
See Configuration reference and
Slash commands for current adapter details.
Allowlist
...
json5 Copy code { agents : { defaults : { subagents : { maxSpawnDepth : 2 , // allow sub-agents to spawn children (default: 1) maxChildrenPerAgent : 5 , // max active children per agent session (default: 5) maxConcurrent : 8 , // global concurrency lane cap (default: 8) runTimeoutSeconds : 900 , // default timeout for sessions_spawn (0 = no timeout) announceTimeoutMs : 120000 , // per-call gateway announce timeout }, }, }, }
Depth levels
Depth Session key shape Role Can spawn? 0 agent:<id>:main Main agent Always 1 agent:<id>:subagent:<uuid> Sub-agent (orchestrator when depth 2 allowed) Only if maxSpawnDepth >= 2 2 agent:<id>:subagent:<uuid>:subagent:<uuid> Sub-sub-agent (leaf worker) Never
Announce chain
...
Override via config
json5 Copy code { agents : { defaults : { subagents : { maxConcurrent : 1 , }, }, }, tools : { subagents : { tools : { // deny wins deny : [ "gateway" , "cron" ], // if allow is set, it becomes allow-only (deny still wins) // allow: ["read", "exec", "process"] }, }, }, }
