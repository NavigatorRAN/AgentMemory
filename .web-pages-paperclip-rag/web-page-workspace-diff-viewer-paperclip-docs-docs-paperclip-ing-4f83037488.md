# Workspace diff viewer | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/reference/plugins/workspace-diff
- Final URL: https://docs.paperclip.ing/reference/plugins/workspace-diff/
- Canonical URL: https://docs.paperclip.ing/reference/plugins/workspace-diff
- Fetched at: 2026-06-23T13:40:12Z
- Content type: text/html; charset=utf-8

## Description

If you want to see what your agents actually changed inside a workspace — which files moved, what got added, the staged-versus-unstaged split, the line-level patch — without leavin
...
If you're running from a monorepo checkout, see Develop a plugin locally and point the installer at packages/plugins/plugin-workspace-diff instead.
...
Looks up the workspace through the SDK's executionWorkspaces.get or projects.listWorkspaces , depending on entity type.
Resolves a base ref — either the workspace's own baseRef , or, for execution workspaces, the parent project workspace's defaultRef / repoRef . You can also pass an explicit baseRef in the query.
Confirms the workspace cwd is an absolute, real directory inside a Git repository (with O_NOFOLLOW checks and git rev-parse --show-toplevel ).
Runs git with a 10 second timeout and a 2 MiB list buffer to enumerate changed files and produce patches.
...
Cap Default maxFiles 200 changed files maxFileBytes 512 KiB per file (binary / oversized files are skipped) maxPatchBytes 256 KiB per file patch maxTotalPatchBytes 1 MiB total patch payload
...
Reference → Plugin SDK — the authoring surface, if you want to extend the diff viewer or write your own detail-tab plugin.
