...
- Requested URL: https://docs.openclaw.ai/tools/exec
- Final URL: https://docs.openclaw.ai/tools/exec
- Canonical URL: https://docs.openclaw.ai/tools/exec
- Fetched at: 2026-06-23T14:57:44Z
...
No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both tools.exec.* and the host approvals file; see Exec approvals .
...
Example:
json5 Copy code { tools : { exec : { pathPrepend : [ "~/bin" , "/opt/oss/bin" ], }, }, }
PATH handling
...
Code Copy code /exec host=auto security=allowlist ask=on-miss node=mac-1
Authorization model
...
Examples
Foreground:
json Copy code { "tool" : "exec" , "command" : "ls -la" }
Background + poll:
json Copy code { "tool" : "exec" , "command" : "npm run build" , "yieldMs" : 1000 } { "tool" : "process" , "action" : "poll" , "sessionId" : "<id>" }
...
json Copy code { "tool" : "process" , "action" : "send-keys" , "sessionId" : "<id>" , "keys" : [ "Enter" ] } { "tool" : "process" , "action" : "send-keys" , "sessionId" : "<id>" , "keys" : [ "C-c" ] } { "tool" : "process" , "action" : "send-keys" , "sessionId" : "<id>" , "keys" : [ "Up" , "Up" , "Enter" ] }
Submit (send CR only):
json Copy code { "tool" : "process" , "action" : "submit" , "sessionId" : "<id>" }
Paste (bracketed by default):
json Copy code { "tool" : "process" , "action" : "paste" , "sessionId" : "<id>" , "text" : "line1\nline2\n" }
apply_patch
...
json5 Copy code { tools : { exec : { applyPatch : { workspaceOnly : true , allowModels : [ "gpt-5.5" ] }, }, }, }
