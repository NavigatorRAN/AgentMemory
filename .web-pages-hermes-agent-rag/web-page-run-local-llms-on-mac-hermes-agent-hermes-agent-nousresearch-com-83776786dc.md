# Run Local LLMs on Mac | Hermes Agent

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://hermes-agent.nousresearch.com/docs/guides/local-llm-on-mac
- Final URL: https://hermes-agent.nousresearch.com/docs/guides/local-llm-on-mac
- Canonical URL: https://hermes-agent.nousresearch.com/docs/guides/local-llm-on-mac
- Fetched at: 2026-06-23T14:15:05Z
...
Backend Install Best at Format llama.cpp brew install llama.cpp Fastest time-to-first-token, quantized KV cache for low memory GGUF omlx omlx.ai Fastest token generation, native Metal optimization MLX (safetensors)
...
Variant Size on disk RAM needed (128K context) Backend Qwen3.5-9B-Q4_K_M (GGUF) 5.3 GB ~10 GB with quantized KV cache llama.cpp Qwen3.5-9B-mlx-lm-mxfp4 (MLX) ~5 GB ~12 GB omlx
...
KV cache type KV cache memory (128K ctx, 9B model) f16 (default) ~16 GB q8_0 ~8 GB q4_0 ~4 GB
...
Results ​
Metric llama.cpp (Q4_K_M) MLX (mxfp4) Winner TTFT (avg) 67 ms 289 ms llama.cpp (4.3x faster) TTFT (p50) 66 ms 286 ms llama.cpp (4.3x faster) Generation (avg) 70 tok/s 96 tok/s MLX (37% faster) Generation (p50) 70 tok/s 96 tok/s MLX (37% faster) Total time (512 tokens) 7.3s 5.5s MLX (25% faster)
What this means ​
...
Use case Recommendation Interactive chat, low-latency tools llama.cpp Long-form generation, bulk processing MLX (omlx) Memory-constrained (8-16 GB) llama.cpp (quantized KV cache is unmatched) Serving multiple models simultaneously omlx (built-in multi-model support) Maximum compatibility (Linux too) llama.cpp
Connect to Hermes ​
...
Timeout Default Local auto-adjustment Env var override Stream read (socket-level) 120s Raised to 1800s HERMES_STREAM_READ_TIMEOUT Stale stream detection 180s Disabled entirely HERMES_STREAM_STALE_TIMEOUT API call (non-streaming) 1800s No change needed HERMES_API_TIMEOUT
