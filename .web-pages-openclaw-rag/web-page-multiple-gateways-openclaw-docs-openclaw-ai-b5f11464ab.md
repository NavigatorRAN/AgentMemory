...
- Requested URL: https://docs.openclaw.ai/gateway/multiple-gateways
- Final URL: https://docs.openclaw.ai/gateway/multiple-gateways
- Canonical URL: https://docs.openclaw.ai/gateway/multiple-gateways
- Fetched at: 2026-06-23T14:43:53Z
...
bash Copy code # Rescue bot (separate Telegram bot, separate profile, port 19789) openclaw --profile rescue onboard openclaw --profile rescue gateway install --port 19789
...
own base port:
bash Copy code # main (default profile) openclaw setup openclaw gateway --port 18789 # extra gateway openclaw --profile ops setup openclaw --profile ops gateway --port 19789
If you want both Gateways to use named profiles, that also works:
bash Copy code openclaw --profile main setup openclaw --profile main gateway --port 18789 openclaw --profile ops setup openclaw --profile ops gateway --port 19789
Services follow the same pattern:
bash Copy code openclaw gateway install openclaw --profile ops gateway install --port 19789
...
Manual env example
bash Copy code OPENCLAW_CONFIG_PATH=~/.openclaw/main.json \ OPENCLAW_STATE_DIR=~/.openclaw \ openclaw gateway --port 18789 OPENCLAW_CONFIG_PATH=~/.openclaw/rescue.json \ OPENCLAW_STATE_DIR=~/.openclaw-rescue \ openclaw gateway --port 19789
Quick checks
bash Copy code openclaw gateway status --deep openclaw --profile rescue gateway status --deep openclaw --profile rescue gateway probe openclaw status openclaw --profile rescue status openclaw --profile rescue browser status
Interpretation:
