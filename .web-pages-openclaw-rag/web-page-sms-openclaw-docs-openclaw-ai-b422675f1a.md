...
- Requested URL: https://docs.openclaw.ai/channels/sms
- Final URL: https://docs.openclaw.ai/channels/sms
- Canonical URL: https://docs.openclaw.ai/channels/sms
- Fetched at: 2026-06-23T14:36:35Z
- Content type: text/html; charset=utf-8

## Description

Twilio SMS channel setup, access controls, and webhook configuration
...
Quick Setup
Install the plugin
bash Copy code openclaw plugins install @openclaw/sms
...
json5 Copy code { channels : { sms : { enabled : true , accountSid : "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" , authToken : "twilio-auth-token" , fromNumber : "+15551234567" , publicWebhookUrl : "https://gateway.example.com/webhooks/sms" , dmPolicy : "pairing" , }, }, }
Apply it:
bash Copy code openclaw config patch --file ./sms.patch.json5 --dry-run openclaw config patch --file ./sms.patch.json5
Point Twilio at the Gateway webhook
In the Twilio phone number settings, open Messaging and set A message comes in to:
text Copy code https://gateway.example.com/webhooks/sms
...
bash Copy code tailscale funnel -- bg --set-path /webhooks/sms http://127.0.0.1:<gateway-port>/webhooks/sms tailscale funnel status
...
bash Copy code openclaw pairing list sms openclaw pairing approve sms <CODE>
...
bash Copy code export TWILIO_ACCOUNT_SID= "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" export TWILIO_AUTH_TOKEN= "<twilio-auth-token>" export TWILIO_PHONE_NUMBER= "+15551234567" export SMS_PUBLIC_WEBHOOK_URL= "https://gateway.example.com/webhooks/sms"
Then enable the channel in config:
json5 Copy code { channels : { sms : { enabled : true , dmPolicy : "pairing" , }, }, }
...
json5 Copy code { channels : { sms : { enabled : true , accountSid : "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" , authToken : { source : "env" , provider : "default" , id : "TWILIO_AUTH_TOKEN" }, fromNumber : "+15551234567" , publicWebhookUrl : "https://gateway.example.com/webhooks/sms" , dmPolicy : "pairing" , }, }, }
...
json5 Copy code { channels : { sms : { enabled : true , accountSid : "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" , authToken : "twilio-auth-token" , fromNumber : "+15551234567" , publicWebhookUrl : "https://gateway.example.com/webhooks/sms" , dmPolicy : "allowlist" , allowFrom : [ "+15557654321" ], }, }, }
...
json5 Copy code { channels : { sms : { enabled : true , accountSid : "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" , authToken : "twilio-auth-token" , messagingServiceSid : "MGxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" , publicWebhookUrl : "https://gateway.example.com/webhooks/sms" , dmPolicy : "pairing" , }, }, }
...
json5 Copy code { channels : { sms : { enabled : true , accountSid : "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" , authToken : "twilio-auth-token" , fromNumber : "+15551234567" , defaultTo : "+15557654321" , publicWebhookUrl : "https://gateway.example.com/webhooks/sms" , }, }, }
Access control
...
bash Copy code openclaw message send --channel sms --target sms:+15551234567 --message "hello"
...
bash Copy code openclaw message send --target twilio-sms:+15551234567 --message "hello"
...
Confirm the Gateway log shows the SMS webhook route.
Run a Twilio-side probe:
bash Copy code openclaw channels capabilities --channel sms openclaw channels status --channel sms --probe --json
Send an SMS to the Twilio number from your phone.
Run openclaw pairing list sms .
Approve the pairing code with openclaw pairing approve sms <CODE> .
Send another SMS and confirm the agent replies.
For outbound-only testing, use:
bash Copy code openclaw message send --channel sms --target sms:+15557654321 --message "OpenClaw SMS test"
...
bash Copy code imsg send --to "+15551234567" --service sms --text "OpenClaw SMS E2E $(date -u +%Y%m%dT%H%M%SZ) " --json openclaw pairing list sms openclaw pairing approve sms <CODE> imsg send --to "+15551234567" --service sms --text "reply exactly SMS pong" --json
...
json5 Copy code { channels : { sms : { dangerouslyDisableSignatureValidation : true , }, }, }
...
json5 Copy code { channels : { sms : { accounts : { support : { enabled : true , accountSid : "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" , authToken : "twilio-auth-token" , fromNumber : "+15551234567" , publicWebhookUrl : "https://gateway.example.com/webhooks/sms/support" , webhookPath : "/webhooks/sms/support" , dmPolicy : "allowlist" , allowFrom : [ "+15557654321" ], }, }, }, }, }
