# Openclaw Migration — Migrate a user's OpenClaw customization footprint into Hermes Agent | Hermes Agent

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/migration/migration-openclaw-migration
- Final URL: https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/migration/migration-openclaw-migration
- Canonical URL: https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/migration/migration-openclaw-migration
- Fetched at: 2026-06-23T14:19:47Z
- Content type: text/html; charset=utf-8

## Description

Migrate a user's OpenClaw customization footprint into Hermes Agent
...
Skill metadata ​
Source Optional — install with hermes skills install official/migration/openclaw-migration Path optional-skills/migration/openclaw-migration Version 1.0.0 Author Hermes Agent (Nous Research) License MIT Platforms linux, macos, windows Tags Migration , OpenClaw , Hermes , Memory , Persona , Import Related skills hermes-agent
...
Prefer the primary OpenClaw workspace ( ~/.openclaw/workspace/ ) over workspace.default/ . Only use the default workspace as fallback when the primary files are missing.
Even in secret-migration mode, only migrate secrets with a clean Hermes destination. Unsupported auth blobs must still be reported as skipped.
If the dry run shows a large asset copy, a conflicting SOUL.md , or overflowed memory entries, call those out separately before execution.
Default to user-data only if the user is unsure.
Only include workspace-agents when the user has explicitly provided a destination workspace path.
Treat category-level --include / --exclude as an advanced escape hatch, not the normal flow.
Do not end the dry-run summary with a vague “What would you like to do?” if clarify is available. Use structured follow-up prompts instead.
Do not use an open-ended clarify prompt when a real choice prompt would work. Prefer selectable choices first, then free text only for absolute paths or file review requests.
After a dry run, never stop after summarizing if there is still an unresolved decision. Use clarify immediately for the highest-priority blocking decision.
Priority order for follow-up questions:
SOUL.md conflict
imported skill conflicts
migration mode
workspace instructions destination
Do not promise to present choices later in the same message. Present them by actually calling clarify .
After the migration-mode answer, explicitly check whether workspace-agents is still unresolved. If it is, your next action must be the workspace-instructions clarify call.
After any clarify answer, if another required decision remains, do not narrate what was just decided. Ask the next required question immediately.
Expected result ​
