# Google Chat | Hermes Agent

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/google_chat
- Final URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/google_chat
- Canonical URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/google_chat
- Fetched at: 2026-06-23T14:16:56Z
- Content type: text/html; charset=utf-8

## Description

Set up Hermes Agent as a Google Chat bot using Cloud Pub/Sub
...
There is NO "Chat Bot Caller" role
...
Supported Not supported *bold* , _italic_ , ~strike~ , `code` Headings, lists Inline images via URL Interactive Card v2 buttons (v1 of this gateway) Native file attachments (after /setup-files — see Step 10) Native voice notes / circular video notes
...
Download the JSON. Move it onto the host that runs Hermes.
Register the client with Hermes (run under the profile you want it scoped to):
# Default profile:
...
They send /setup-files to the bot. It replies with status and the next
They send /setup-files start . The bot replies with an OAuth URL.
They open the URL, click Allow , and watch the browser fail to load
http://localhost:1/?...&code=... . That failure is expected — the auth
code is in the URL bar.
They copy the failed URL (or just the code=... value) and paste it back
...
Check the Pub/Sub subscription has undelivered messages in the console.
...
If the subscription has zero messages, Google Chat isn't publishing.
Double-check the IAM binding on the topic :
chat-api-push@system.gserviceaccount.com must have Pub/Sub Publisher .
Check hermes gateway logs for [GoogleChat] Connected . If you see
...
User OAuth scope : the per-user attachment flow requests only chat.messages.create — the minimum that covers media.upload plus the
