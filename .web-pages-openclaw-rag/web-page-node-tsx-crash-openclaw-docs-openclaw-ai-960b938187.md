...
- Requested URL: https://docs.openclaw.ai/debug/node-issue
- Final URL: https://docs.openclaw.ai/debug/node-issue
- Canonical URL: https://docs.openclaw.ai/debug/node-issue
- Fetched at: 2026-06-23T14:39:25Z
...
Code Copy code [openclaw] Failed to start CLI: TypeError: __name is not a function at createSubsystemLogger (.../src/logging/subsystem.ts:203:25) at .../src/agents/auth-profiles/constants.ts:25:20
...
Repro (Node-only)
bash Copy code # in repo root node --version pnpm install node --import tsx src/entry.ts status
Minimal repro in repo
bash Copy code node --import tsx scripts/repro/tsx-name-repro.ts
Node version check
Node 25.3.0: fails
...
bash Copy code pnpm tsgo node openclaw.mjs status
