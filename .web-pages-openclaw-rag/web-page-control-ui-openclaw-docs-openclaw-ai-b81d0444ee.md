...
- Requested URL: https://docs.openclaw.ai/web/control-ui
- Final URL: https://docs.openclaw.ai/web/control-ui
- Canonical URL: https://docs.openclaw.ai/web/control-ui
- Fetched at: 2026-06-23T14:58:36Z
...
bash Copy code openclaw devices list
Approve by request ID
bash Copy code openclaw devices approve <requestId>
...
Toggle enablement when a server should remain configured but stay out of runtime discovery.
Edit the scoped mcp config section for server definitions, headers, TLS/mTLS paths, OAuth metadata, tool filters, and Codex projection metadata.
Use Save for a config write, or Save & Publish when the running Gateway should apply the changed config.
Run openclaw mcp status --verbose , openclaw mcp doctor --probe , or openclaw mcp reload from a terminal when the edited process needs static diagnostics, live proof, or cached-runtime disposal.
The page redacts credential-bearing URL-like values before rendering and quotes server names in command snippets so copied commands still work with spaces or shell metacharacters. The full CLI and config reference lives in MCP .
Activity tab
...
Web Push is independent of the iOS APNS relay path (see Configuration for relay-backed push) and the existing push.test method, which target native mobile pairing.
Hosted embeds
...
Example:
json5 Copy code { gateway : { controlUi : { embedSandbox : "scripts" , }, }, } Warning
...
json5 Copy code { gateway : { controlUi : { chatMessageMaxWidth : "min(1280px, 82%)" , }, }, }
...
bash Copy code openclaw gateway -- bind tailnet --token " $(openssl rand -hex 32) "
Then open:
...
Insecure-auth toggle behavior json5 Copy code { gateway : { controlUi : { allowInsecureAuth : true }, bind : "tailnet" , auth : { mode : "token" , token : "replace-me" }, }, }
...
Break-glass only json5 Copy code { gateway : { controlUi : { dangerouslyDisableDeviceAuth : true }, bind : "tailnet" , auth : { mode : "token" , token : "replace-me" }, }, } Warning
...
bash Copy code pnpm ui:build
Optional absolute base (when you want fixed asset URLs):
bash Copy code OPENCLAW_CONTROL_UI_BASE_PATH=/openclaw/ pnpm ui:build
For local development (separate dev server):
bash Copy code pnpm ui:dev
...
text Copy code http://localhost:5173/?gatewayUrl=ws%3A%2F%2F<gateway-host>%3A18789
Optional one-time auth (if needed):
text Copy code http://localhost:5173/?gatewayUrl=wss%3A%2F%2F<gateway-host>%3A18789#token=<gateway-token>
Notes
...
json5 Copy code { gateway : { controlUi : { allowedOrigins : [ "http://localhost:5173" ], }, }, }
