...
- Requested URL: https://docs.openclaw.ai/plugins/dependency-resolution
- Final URL: https://docs.openclaw.ai/plugins/dependency-resolution
- Canonical URL: https://docs.openclaw.ai/plugins/dependency-resolution
- Fetched at: 2026-06-23T14:50:16Z
...
bash Copy code cd ~/.openclaw/npm/projects/<encoded-package> npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
...
bash Copy code pnpm deps:shrinkwrap:generate pnpm deps:shrinkwrap:check
...
bash Copy code npm install --omit=dev --ignore-scripts --no-audit --no-fund
...
bash Copy code openclaw plugins update < id > openclaw plugins install < source > openclaw doctor --fix
...
externally, or stay source-only, see Plugin inventory .
...
Install shape Bundled plugin location Dependency owner npm install -g openclaw Built runtime tree inside the package OpenClaw package and explicit plugin install/update/doctor flows Git checkout plus pnpm install extensions/<id> workspace packages The pnpm workspace, including each plugin package's own dependencies openclaw plugins install ... Managed npm project/git/ClawHub root The plugin install/update flow
Legacy cleanup
