...
- Requested URL: https://docs.openclaw.ai/platforms/windows
- Final URL: https://docs.openclaw.ai/platforms/windows
- Canonical URL: https://docs.openclaw.ai/platforms/windows
- Fetched at: 2026-06-23T14:49:54Z
...
powershell Copy code openclaw devices list openclaw devices approve < request-id > openclaw nodes status
...
Mode matrix:
Node mode MCP server Behavior off off Operator-only desktop app on off Gateway-connected Windows node off on Local MCP server only on on Gateway node plus local MCP server
...
Verify:
powershell Copy code openclaw --version openclaw doctor openclaw gateway status --json
...
powershell Copy code openclaw gateway install openclaw gateway status --json
If you only want CLI use without a managed Gateway service:
powershell Copy code openclaw onboard --non-interactive --skip-health openclaw gateway run
WSL2 Gateway
...
your own distro.
Manual setup:
powershell Copy code wsl --install # Or pick a distro explicitly: wsl --list --online wsl --install -d Ubuntu -24 . 04
Enable systemd inside WSL:
bash Copy code sudo tee /etc/wsl.conf >/dev/null << 'EOF' [boot] systemd= true EOF
Restart WSL from PowerShell:
powershell Copy code wsl --shutdown
Then install OpenClaw inside WSL with the Linux quickstart:
bash Copy code curl -fsSL https://openclaw.ai/install.sh | bash openclaw gateway status
Gateway auto-start before Windows login
For headless WSL setups, ensure the full boot chain runs even when no one logs
into Windows.
Inside WSL:
bash Copy code sudo apt-get install -y dbus-x11 sudo loginctl enable-linger " $(whoami) " openclaw gateway install
In PowerShell as Administrator:
powershell Copy code schtasks /create /tn "WSL Boot" /tr "wsl.exe -d Ubuntu --exec dbus-launch true" / sc onstart /ru " $env:USERNAME "
Replace Ubuntu with your distro name from:
powershell Copy code wsl --list --verbose
Note: Two changes from older recipes:
dbus-launch true instead of /bin/true — On WSL ≥ 2.6.1.0 a regression ( microsoft/WSL #13416 ) causes the distro to idle-terminate 15–20 seconds after the last client exits, even with linger enabled. dbus-launch true keeps a child-of-init process alive as a workaround ( community discussion, microsoft/WSL #9245 ).
...
bash Copy code systemctl --user is-enabled openclaw-gateway.service systemctl --user status openclaw-gateway.service --no-pager
...
powershell Copy code notepad " $env:LOCALAPPDATA \OpenClawTray\Logs\Setup\easy-setup-latest.txt"
...
powershell Copy code openclaw devices list openclaw devices approve < request-id >
...
powershell Copy code $env:GH_TOKEN = "<your-token>" gh auth status gh auth setup -git
