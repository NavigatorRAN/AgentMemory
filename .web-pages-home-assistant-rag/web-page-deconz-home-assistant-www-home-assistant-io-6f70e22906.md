...
- Requested URL: https://www.home-assistant.io/integrations/deconz
- Final URL: https://www.home-assistant.io/integrations/deconz
- Canonical URL: https://www.home-assistant.io/integrations/deconz/
- Fetched at: 2026-06-28T02:36:15Z
- Content type: text/html; charset=UTF-8

## Description

Instructions on how to set up ConBee/RaspBee devices with deCONZ from dresden elektronik within Home Assistant.
...
Otherwise, use community container for your deCONZ needs.
...
If you are running the deCONZ app for Home Assistant, you can see the hostname on the app page under Settings > App > deCONZ , under Hostname .
...
To add the deCONZ hub to your Home Assistant instance, use this My button:
deCONZ can be auto-discovered by Home Assistant. If an instance was found,
...
If the state of entities An entity represents a sensor, actor, or function in Home Assistant. Entities are used to monitor physical properties or to control other entities. An entity is usually part of a device or a service. [Learn more] are only reflected in Home Assistant when the integration Integrations connect and integrate Home Assistant with your devices, services, and more. [Learn more] is loaded (during restart, reload, setup) you probably have an issue with the WebSocket configuration where your deCONZ instance is running. The deCONZ integration uses the WebSocket port provided by the deCONZ REST API. If you’re running the deCONZ Docker container make sure that it properly configures the WebSocket port so deCONZ can report what port is exposed outside of the containerized environment. Also, make sure to review firewall rules that might block communication over certain ports.
The deCONZ integration Integrations connect and integrate Home Assistant with your devices, services, and more. [Learn more] provides the following actions. Each link below opens a dedicated page with examples, parameters, and a step-by-step UI walkthrough.
Configure ( deconz.configure )
Set an attribute of a device endpoint in deCONZ or of the deCONZ service itself.
Device refresh ( deconz.device_refresh )
Refresh the list of available devices from deCONZ.
Remove orphaned entries ( deconz.remove_orphaned_entries )
Clean up device and entity registry entries that deCONZ no longer provides.
...
Switch code Description 1XXX Button #1 up to #8 XXX1 Button hold XXX2 Button short release XXX3 Button long release
...
Gesture Description 0 Awake 1 Shake 2 Free fall 3 Flip 90 4 Flip 180 5 Move on any side 6 Double tap on any side 7 Turn clockwise 8 Turn counter clockwise
Go to Settings > Developer tools > Events . In the section Listen to events add deconz_event and press START LISTENING . All events from deCONZ will now be shown and by pushing your remote button while monitoring the log it should be fairly easy to find the events you are looking for.
...
automation : - alias : " 'Toggle lamp from dimmer'" initial_state : " on" triggers : - trigger : event event_type : deconz_event event_data : id : remote_control_1 event : 1002 actions : - action : light.toggle target : entity_id : light.lamp - alias : " Increase brightness of lamp from dimmer" initial_state : " on" triggers : - trigger : event event_type : deconz_event event_data : id : remote_control_1 event : 2002 actions : - action : light.turn_on target : entity_id : light.lamp data : brightness : > {% set bri = state_attr('light.lamp', 'brightness') | int %} {{ [bri+30, 249] | min }} - alias : " Decrease brightness of lamp from dimmer" initial_state : " on" triggers : - trigger : event event_type : deconz_event event_data : id : remote_control_1 event : 3002 actions : - action : light.turn_on target : entity_id : light.lamp data : brightness : > {% set bri = state_attr('light.lamp', 'brightness') | int %} {{ [bri-30, 0] | max }} - alias : ' Turn lamp on when turning cube clockwise' initial_state : " on" triggers : - trigger : event event_type : deconz_event event_data : id : remote_control_1 gesture : 7 actions : - action : light.turn_on target : entity_id : light.lamp
Changing color through the Müller Licht tint remote control
automation : - alias : " React to color wheel changes" triggers : - trigger : event event_type : deconz_event event_data : id : tint_remote_1 event : 6002 actions : - action : light.turn_on data : xy_color : - ' {{ trigger.event.data.xy.0 }}' - ' {{ trigger.event.data.xy.1 }}' entity_id : light.example_color_light_1 mode : restart
...
automation : - alias : " Flash Hue Bulb with Doorbell Motion" triggers : - trigger : state entity_id : binary_sensor.doorbell_motion to : " on" actions : - action : deconz.configure data : entity : light.hue_lamp field : /state data : ' on' : true hue : 65535 sat : 255 bri : 255 alert : " breathe" - delay : 00:00:15 - action : deconz.configure data : entity : light.hue_lamp field : " /state" data : ' on' : false
...
Switches
Temperature sensor
Switches aren’t exposed as ordinary entities, see the deCONZ main integration for more details.
deCONZ Daylight Sensor
The deCONZ Daylight sensor is a special sensor built into the deCONZ software since version 2.05.12. It is represented in Home Assistant as a sensor called sensor.daylight. The sensor’s state value is a string corresponding to the phase of daylight (descriptions below taken from SunCalc , on which the deCONZ implementation is based):
...
The deCONZ hub was introduced in Home Assistant 0.61, and it's used by
3229 active installations.
