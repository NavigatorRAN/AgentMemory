...
- Requested URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/slack
- Final URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/slack
- Canonical URL: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/slack
- Fetched at: 2026-06-23T14:17:11Z
...
the steps below.
Overview ​
Component Value Library slack-bolt / slack_sdk for Python (Socket Mode) Connection WebSocket — no public URL required Auth tokens needed Bot Token ( xoxb- ) + App-Level Token ( xapp- ) User identification Slack Member IDs (e.g., U01ABC2DEF3 )
...
Pick your workspace, paste the JSON contents, review, click Next
→ Create
Skip ahead to Step 6: Install App to Workspace . The manifest
...
Optional scopes:
Scope Purpose groups:read List and get info about private channels
...
Event Required? Purpose message.im Yes Bot receives direct messages message.channels Yes Bot receives messages in public channels it's added to message.groups Recommended Bot receives messages in private channels it's invited to app_mention Yes Prevents Bolt SDK errors when bot is @mentioned
...
Context Behavior DMs Bot responds to every message — no @mention needed Channels Bot only responds when @mentioned (e.g., @Hermes Agent what time is it? ). In channels, Hermes replies in a thread attached to that message. Threads If you @mention Hermes inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, subsequent replies in that thread do not require @mention — the bot follows the conversation naturally.
