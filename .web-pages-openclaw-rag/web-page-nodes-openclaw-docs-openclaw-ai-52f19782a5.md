...
- Requested URL: https://docs.openclaw.ai/nodes
- Final URL: https://docs.openclaw.ai/nodes
- Canonical URL: https://docs.openclaw.ai/nodes
- Fetched at: 2026-06-23T14:49:04Z
...
Quick CLI:
bash Copy code openclaw devices list openclaw devices approve <requestId> openclaw devices reject <requestId> openclaw nodes status openclaw nodes describe --node <idOrNameOrIp>
...
On the node machine:
bash Copy code openclaw node run --host <gateway-host> --port 18789 --display-name "Build Node"
...
bash Copy code openclaw node install --host <gateway-host> --port 18789 --display-name "Build Node" openclaw node start openclaw node restart
Pair + name
On the gateway host:
...
bash Copy code openclaw approvals allowlist add --node < id |name|ip> "/usr/bin/uname" openclaw approvals allowlist add --node < id |name|ip> "/usr/bin/sw_vers"
Approvals live on the node host at ~/.openclaw/exec-approvals.json .
Point exec at the node
Configure defaults (gateway config):
bash Copy code openclaw config set tools.exec.host node openclaw config set tools.exec.security allowlist openclaw config set tools.exec.node "<id-or-name>"
Or per session:
Code Copy code /exec host=node security=allowlist node=<id-or-name>
...
Invoking commands
Low-level (raw RPC):
bash Copy code openclaw nodes invoke --node <idOrNameOrIp> -- command canvas.eval --params '{"javaScript":"location.href"}'
...
json5 Copy code { agents : { list : [ { id : "main" , tools : { exec : { node : "build-node" } }, }, ], }, }
Screenshots (canvas snapshots)
If the node is showing the Canvas (WebView), canvas.snapshot returns { format, base64 } .
CLI helper (writes to a temp file and prints the saved path):
bash Copy code openclaw nodes canvas snapshot --node <idOrNameOrIp> --format png openclaw nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
Canvas controls
bash Copy code openclaw nodes canvas present --node <idOrNameOrIp> --target https://example.com openclaw nodes canvas hide --node <idOrNameOrIp> openclaw nodes canvas navigate https://example.com --node <idOrNameOrIp> openclaw nodes canvas eval --node <idOrNameOrIp> --js "document.title"
canvas present accepts URLs or local file paths ( --target ), plus optional --x/--y/--width/--height for positioning.
canvas eval accepts inline JS ( --js ) or a positional arg.
A2UI (Canvas)
bash Copy code openclaw nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello" openclaw nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl openclaw nodes canvas a2ui reset --node <idOrNameOrIp>
...
Photos ( jpg ):
bash Copy code openclaw nodes camera list --node <idOrNameOrIp> openclaw nodes camera snap --node <idOrNameOrIp> # default: both facings (2 MEDIA lines) openclaw nodes camera snap --node <idOrNameOrIp> --facing front
Video clips ( mp4 ):
bash Copy code openclaw nodes camera clip --node <idOrNameOrIp> --duration 10s openclaw nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
The node must be foregrounded for canvas.* and camera.* (background calls return NODE_BACKGROUND_UNAVAILABLE ).
Clip duration is clamped (currently <= 60s ) to avoid oversized base64 payloads.
Android will prompt for CAMERA / RECORD_AUDIO permissions when possible; denied permissions fail with *_PERMISSION_REQUIRED .
Screen recordings (nodes)
Supported nodes expose screen.record (mp4). Example:
bash Copy code openclaw nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 openclaw nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
...
CLI helper:
bash Copy code openclaw nodes location get --node <idOrNameOrIp> openclaw nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
...
Low-level invoke:
bash Copy code openclaw nodes invoke --node <idOrNameOrIp> -- command sms.send --params '{"to":"+15555550123","message":"Hello from OpenClaw"}'
...
Examples:
bash Copy code openclaw nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready" openclaw nodes invoke --node <idOrNameOrIp> -- command system.which --params '{"name":"git"}'
...
Global default:
bash Copy code openclaw config set tools.exec.node "node-id-or-name"
Per-agent override:
bash Copy code openclaw config get agents.list openclaw config set 'agents.list[0].tools.exec.node' "node-id-or-name"
Unset to allow any node:
bash Copy code openclaw config unset tools.exec.node openclaw config unset 'agents.list[0].tools.exec.node'
Permissions map
...
Start it:
bash Copy code openclaw node run --host <gateway-host> --port 18789
