...
- Requested URL: https://docs.openclaw.ai/tools/exec-approvals
- Final URL: https://docs.openclaw.ai/tools/exec-approvals
- Canonical URL: https://docs.openclaw.ai/tools/exec-approvals
- Fetched at: 2026-06-23T14:57:45Z
...
Command What it shows openclaw approvals get / --gateway / --node <id|name|ip> Requested policy, host policy sources, and the effective result. openclaw exec-policy show Local-machine merged view. openclaw exec-policy set / preset Synchronize the local requested policy with the local host approvals file in one step.
...
text Copy code $OPENCLAW_STATE_DIR/exec-approvals.json # otherwise ~/.openclaw/exec-approvals.json
...
per-call ask tool parameter (see Exec tool )
...
fall back to allow.
Layer YOLO setting tools.exec.security full on gateway / node tools.exec.ask off Host askFallback full
Warning
...
bash Copy code openclaw approvals set --stdin << 'EOF' { version: 1, defaults: { security: "full" , ask: "off" , askFallback: "full" } } EOF
Local shortcut
bash Copy code openclaw exec-policy preset yolo
...
bash Copy code openclaw approvals set --node < id |name|ip> --stdin << 'EOF' { version: 1, defaults: { security: "full" , ask: "off" , askFallback: "full" } } EOF Note
...
json Copy code { "version" : 1 , "agents" : { "main" : { "allowlist" : [ { "pattern" : "python3" , "argPattern" : "^safe\\.py$" } ] } } }
...
Field Meaning pattern Resolved binary path glob or bare command-name glob argPattern Optional argv regex; omitted entries are path-only id Stable UUID used for UI identity source Entry source, such as allow-always commandText Command text captured when an approval flow created the entry lastUsedAt Last-used timestamp lastUsedCommand Last command that matched lastResolvedPath Last resolved binary path
