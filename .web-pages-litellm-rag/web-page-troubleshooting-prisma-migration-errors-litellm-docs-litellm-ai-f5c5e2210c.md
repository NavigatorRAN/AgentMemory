# Troubleshooting Prisma Migration Errors | liteLLM

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.litellm.ai/docs/troubleshoot/prisma_migrations
- Final URL: https://docs.litellm.ai/docs/troubleshoot/prisma_migrations
- Canonical URL: https://docs.litellm.ai/docs/troubleshoot/prisma_migrations
- Fetched at: 2026-06-23T14:33:37Z
...
Copy as Markdown
Common Prisma migration issues encountered when upgrading or downgrading LiteLLM proxy versions, and how to fix them.
For a full guide on safely reverting your LiteLLM version, see the Safe Rollback Guide .
How Prisma Migrations Work in LiteLLM ​
LiteLLM uses Prisma to manage its PostgreSQL database schema.
...
ORDER BY started_at DESC ;
Delete the failed entry and restart LiteLLM:
If that doesn't work, use prisma db push (see warning above — back up your database first):
...
Fix:
Inspect the migration table for problematic entries:
SELECT migration_name , started_at , finished_at , rolled_back_at , logs
LIMIT 20 ;
For each migration that shouldn't be there (i.e., from the version you rolled back from), delete the entry:
DELETE FROM "_prisma_migrations" WHERE migration_name = '<migration_name>' ;
Restart LiteLLM to re-run migrations.
How Prisma Migrations Work in LiteLLM
Common Errors
1. relation "X" does not exist
2. New migrations cannot be applied before the error is recovered from
