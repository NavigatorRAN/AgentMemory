...
- Requested URL: https://docs.openclaw.ai/help/faq
- Final URL: https://docs.openclaw.ai/help/faq
- Canonical URL: https://docs.openclaw.ai/help/faq
- Fetched at: 2026-06-23T14:44:20Z
...
bash Copy code openclaw status
Fast local summary: OS + update, gateway/service reachability, agents/sessions, provider config + runtime issues (when gateway is reachable).
Pasteable report (safe to share)
bash Copy code openclaw status --all
Read-only diagnosis with log tail (tokens redacted).
Daemon + port state
bash Copy code openclaw gateway status
Shows supervisor runtime vs RPC reachability, the probe target URL, and which config the service likely used.
Deep probes
bash Copy code openclaw status --deep
Runs a live gateway health probe, including channel probes when supported
(requires a reachable gateway). See Health .
Tail the latest log
bash Copy code openclaw logs --follow
If RPC is down, fall back to:
bash Copy code tail -f " $(ls -t /tmp/openclaw/openclaw-*.log | head -1) "
...
Gateway snapshot
bash Copy code openclaw health --json openclaw health --verbose # shows the target URL + config path on errors
...
Docs: Gateway , Channels , Multi-agent ,
Memory .
...
json5 Copy code { agents : { list : [ { id : "coder" , model : "xiaomi/mimo-v2.5-pro" , thinkingDefault : "high" , params : { temperature : 0.1 }, }, { id : "chat" , model : "xiaomi/mimo-v2.5-pro" , thinkingDefault : "off" , params : { temperature : 0.8 }, }, ], }, }
...
See Cron jobs , Multi-Agent Routing , Configuration , and Slash commands .
...
Debug:
bash Copy code openclaw tasks show <runId-or-sessionKey>
...
bash Copy code openclaw cron run <jobId> openclaw cron runs -- id <jobId> -- limit 50
...
bash Copy code openclaw cron runs -- id <jobId> -- limit 50 openclaw tasks show <runId-or-sessionKey>
...
bash Copy code openclaw skills search "calendar" openclaw skills search -- limit 20 openclaw skills install @owner/<skill-slug> openclaw skills install @owner/<skill-slug> --version <version> openclaw skills install @owner/<skill-slug> --force openclaw skills install @owner/<skill-slug> --global openclaw skills update --all openclaw skills update --all --global openclaw skills list --eligible openclaw skills check
...
bash Copy code #!/usr/bin/env bash set -euo pipefail exec ssh -T user@mac-host /opt/homebrew/bin/memo " $@ "
...
markdown Copy code --- name: apple-notes description: Manage Apple Notes via the memo CLI on macOS. metadata: { "openclaw": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } } ---
...
Install skills:
bash Copy code openclaw skills install @owner/<skill-slug> openclaw skills update --all
Native installs land in the active workspace skills/ directory. For shared skills across all local agents, use openclaw skills install @owner/<skill-slug> --global (or place them manually in ~/.openclaw/skills/<name>/SKILL.md ). If only some agents should see a shared install, configure agents.defaults.skills or agents.list[].skills . Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See Skills , Skills config , and ClawHub .
...
bash Copy code openclaw browser --browser-profile user tabs openclaw browser --browser-profile user snapshot
If you want a custom name, create an explicit MCP profile:
bash Copy code openclaw browser create-profile --name chrome-live --driver existing-session openclaw browser --browser-profile chrome-live tabs
...
See Sandboxing and Sandbox vs Tool Policy vs Elevated for examples and safety notes.
...
embedding models - see Memory for the setup details.
...
Path Purpose $OPENCLAW_STATE_DIR/openclaw.json Main config (JSON5) $OPENCLAW_STATE_DIR/credentials/oauth.json Legacy OAuth import (copied into auth profiles on first use) $OPENCLAW_STATE_DIR/agents/<agentId>/agent/auth-profiles.json Auth profiles (OAuth, API keys, and optional keyRef / tokenRef ) $OPENCLAW_STATE_DIR/secrets.json Optional file-backed secret payload for file SecretRef providers $OPENCLAW_STATE_DIR/agents/<agentId>/agent/auth.json Legacy compatibility file (static api_key entries scrubbed) $OPENCLAW_STATE_DIR/credentials/ Provider state (e.g. whatsapp/<accountId>/creds.json ) $OPENCLAW_STATE_DIR/agents/ Per-agent state (agentDir + sessions) $OPENCLAW_STATE_DIR/agents/<agentId>/sessions/ Conversation history & state (per agent) $OPENCLAW_STATE_DIR/agents/<agentId>/sessions/sessions.json Session metadata (per agent)
...
json5 Copy code { agents : { defaults : { bootstrapMaxChars : 50000 , bootstrapTotalMaxChars : 300000 , }, }, }
Or override one agent:
json5 Copy code { agents : { list : [ { id : "main" , bootstrapMaxChars : 50000 , bootstrapTotalMaxChars : 300000 , }, ], }, }
...
json5 Copy code { agents : { defaults : { workspace : "~/Projects/my-repo" , }, }, } Remote mode: where is the session store?
...
Code Copy code $OPENCLAW_CONFIG_PATH
...
json5 Copy code { gateway : { bind : "lan" , auth : { mode : "token" , token : "replace-me" , }, }, }
Notes:
...
json5 Copy code { cli : { banner : { taglineMode : "off" , // random | default | off }, }, }
...
json5 Copy code { plugins : { entries : { brave : { config : { webSearch : { apiKey : "BRAVE_API_KEY_HERE" , }, }, }, }, }, tools : { web : { search : { enabled : true , provider : "brave" , maxResults : 5 , }, fetch : { enabled : true , provider : "firecrawl" , // optional; omit for auto-detect }, }, }, }
...
json5 Copy code { browser : { headless : true }, agents : { defaults : { sandbox : { browser : { headless : true } }, }, }, }
...
bash Copy code openclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
...
Minimal sane config for a first install json5 Copy code { agents : { defaults : { workspace : "~/.openclaw/workspace" } }, channels : { whatsapp : { allowFrom : [ "+15555550123" ] } }, }
This sets your workspace and restricts who can trigger the bot.
How do I set up Tailscale on a VPS and connect from my Mac?
Minimal steps:
Install + login on the VPS
bash Copy code curl -fsSL https://tailscale.com/install.sh | sh sudo tailscale up
...
bash Copy code openclaw gateway --tailscale serve
...
Use the macOS app in Remote mode (SSH target can be the tailnet hostname).
...
json5 Copy code { env : { OPENROUTER_API_KEY : "sk-or-..." , vars : { GROQ_API_KEY : "gsk-..." }, }, }
...
json5 Copy code { env : { shellEnv : { enabled : true , timeoutMs : 15000 , }, }, }
...
json5 Copy code { session : { idleMinutes : 240 , }, } Is there a way to make a team of OpenClaw instances (one CEO and many agents)?
...
bash Copy code openclaw reset
Non-interactive full reset:
bash Copy code openclaw reset --scope full -- yes --non-interactive
Then re-run setup:
...
Code Copy code /compact
or /compact <instructions> to guide the summary.
Reset (fresh session ID for the same chat key):
Code Copy code /new /reset
...
json5 Copy code { agents : { defaults : { heartbeat : { every : "2h" , // or "0m" to disable }, }, }, }
...
json5 Copy code { channels : { whatsapp : { groupPolicy : "allowlist" , groupAllowFrom : [ "+15551234567" ], }, }, } How do I get the JID of a WhatsApp group?
Option 1 (fastest): tail logs and send a test message in the group:
bash Copy code openclaw logs --follow --json
Look for chatId (or from ) ending in @g.us , like:
1234567890-1234567890@g.us .
Option 2 (if already configured/allowlisted): list groups from config:
bash Copy code openclaw directory groups list --channel whatsapp
...
Docs: Multi-Agent Routing , Slack ,
Browser , Nodes .
Models, failover, and auth profiles
Model Q&A — defaults, selection, aliases, switching, failover, auth profiles —
lives on the Models FAQ .
Gateway: ports, "already running", and remote mode
What port does the Gateway use?
gateway.port controls the single multiplexed port for WebSocket + HTTP (Control UI, hooks, etc.).
Precedence:
Code Copy code --port > OPENCLAW_GATEWAY_PORT > gateway.port > default 18789 Why does openclaw gateway status say "Runtime: running" but "Connectivity probe: failed"?
...
Fix:
bash Copy code openclaw gateway install --force
...
json5 Copy code { gateway : { mode : "remote" , remote : { url : "ws://gateway.tailnet:18789" , token : "your-token" , password : "your-password" , }, }, }
...
Code Copy code openclaw tui --url ws://<host>:18789 --token <token>
...
bash Copy code openclaw gateway status openclaw gateway restart
...
powershell Copy code wsl openclaw gateway status openclaw gateway restart
If you never installed the service, start it in the foreground:
bash Copy code openclaw gateway run
3) Native Windows CLI/Gateway: the Gateway runs directly in Windows.
Open PowerShell and run:
powershell Copy code openclaw gateway status openclaw gateway restart
If you run it manually (no service), use:
powershell Copy code openclaw gateway run
Docs: Windows , Gateway service runbook .
The Gateway is up but replies never arrive. What should I check?
Start with a quick health sweep:
bash Copy code openclaw status openclaw models status openclaw channels status openclaw logs --follow
...
bash Copy code openclaw channels status openclaw channels logs --channel telegram
...
bash Copy code openclaw status openclaw models status openclaw logs --follow
In the TUI, use /status to see the current state. If you expect replies in a chat
channel, make sure delivery is enabled ( /deliver on ).
Docs: TUI , Slash commands .
How do I completely stop then start the Gateway?
If you installed the service:
bash Copy code openclaw gateway stop openclaw gateway start
...
CLI sending:
bash Copy code openclaw message send --target +15555550123 --message "Here you go" --media /path/to/file.png
Also check:
...
bash Copy code openclaw security audit --deep openclaw gateway status
A safer baseline is:
...
bash Copy code openclaw pairing list telegram
...
bash Copy code openclaw pairing approve whatsapp <code>
List pending requests:
bash Copy code openclaw pairing list whatsapp
...
Code Copy code /verbose off /trace off /reasoning off
...
Code Copy code stop stop action stop current action stop run stop current run stop agent stop the agent stop openclaw openclaw stop stop don't do anything stop do not do anything stop doing anything please stop stop please abort esc wait exit interrupt
These are abort triggers (not slash commands).
For background processes (from the exec tool), you can ask the agent to run:
Code Copy code process action:kill sessionId:XXX
...
json5 Copy code { tools : { message : { crossContext : { allowAcrossProviders : true , marker : { enabled : true , prefix : "[from {channel}] " }, }, }, }, }
...
Default mode is steer . You can add options like debounce:0.5s cap:25 drop:summarize for queued modes. See Command queue and Steering queue .
Miscellaneous
