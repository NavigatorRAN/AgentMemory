# Routine Commands | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/reference/cli/routine
- Final URL: https://docs.paperclip.ing/reference/cli/routine/
- Canonical URL: https://docs.paperclip.ing/reference/cli/routine
- Fetched at: 2026-06-23T13:39:49Z
- Content type: text/html; charset=utf-8

## Description

Routines are Paperclip's recurring and event-driven work. A routine carries a payload describing the work to spawn, a status, a revision history, and one or more triggers — schedul
...
Note: Like every other work object in Paperclip, the actual work a routine produces runs server-side. These commands create, configure, fire, and observe routines through the API — they do not execute the work themselves. See What Runs Where .
Two command groups
There are two groups here, and they are not interchangeable.
Group Talks to Use it when routine (singular) The control-plane REST API Day-to-day: list, create, update, fire, and inspect routines and their triggers. routines (plural) The configured local database directly Local maintenance only — pausing every routine in one company on a local instance.
...
Flag Use -C, --company-id <id> Company to list routines for. Required (falls back to your context's company). --project-id <id> Filter to routines attached to a single project.
routine get
...
Flag Use -C, --company-id <id> Company to create the routine in. Required. --payload-json <json> Routine definition as a JSON string. Required.
...
Flag Use --payload-json <json> Fields to update as a JSON string. Required.
...
Flag Use --limit <n> Cap the number of runs returned. Omit for the server default.
routine run
...
Flag Use --payload-json <json> Run input as a JSON string. Defaults to {} .
...
Trigger kind Fires the routine when… Notes schedule A cron/recurrence schedule is due. The standard "run nightly / hourly / weekly" trigger. Its behavior under overlap and missed windows is governed by concurrency and catch-up. webhook An external system POSTs to the trigger's webhook URL. Carries a secret you can rotate. Use it to turn inbound events into Paperclip work. api (public) A caller fires its public endpoint by publicId . Fired with routine trigger:fire ; the public id is shareable without exposing the routine id.
...
"kind": "schedule",
"schedule": "0 7 * * *"
Flag Use --payload-json <json> Trigger definition as a JSON string. Defaults to {} .
...
Flag Use --payload-json <json> Payload to deliver with the firing. Defaults to {} .
...
Flag Use -C, --company-id <id> Company whose routines to pause. Required (falls back to PAPERCLIP_COMPANY_ID ). -c, --config <path> Path to the Paperclip config file, if not the default. -d, --data-dir <path> Paperclip data directory root, to isolate state from ~/.paperclip . --json Output the raw result object instead of a human summary.
...
See also
Run Commands — inspect and control heartbeat runs, including the runs a routine spawns.
Issue Commands — the tasks routines create; filter by originKind=routine_execution to find them.
Common Options — the client flags every routine command accepts.
Output And Scripting — using --json to pipe routine data into other tools.
Routines And Automation — the end-to-end guide to designing automation.
