...
- Requested URL: https://docs.openclaw.ai/channels/slack
- Final URL: https://docs.openclaw.ai/channels/slack
- Canonical URL: https://docs.openclaw.ai/channels/slack
- Fetched at: 2026-06-23T14:36:34Z
- Content type: text/html; charset=utf-8

## Description

Slack setup and runtime behavior (Socket Mode, HTTP Request URLs, and relay mode)
...
json5 Copy code { channels : { slack : { mode : "relay" , botToken : { source : "env" , provider : "default" , id : "SLACK_BOT_TOKEN" }, relay : { url : "wss://router.example.com/gateway/ws" , authToken : { source : "env" , provider : "default" , id : "SLACK_RELAY_AUTH_TOKEN" }, gatewayId : "team-gateway" , }, }, }, }
...
bash Copy code openclaw plugins install @openclaw/slack
...
bash Copy code export SLACK_APP_TOKEN=slack-app-token-example export SLACK_BOT_TOKEN=slack-bot-token-example cat > slack.socket.patch.json5 << 'JSON5' { channels: { slack: { enabled: true , mode: "socket" , appToken: { source : "env" , provider: "default" , id : "SLACK_APP_TOKEN" }, botToken: { source : "env" , provider: "default" , id : "SLACK_BOT_TOKEN" }, }, }, } JSON5 openclaw config patch --file ./slack.socket.patch.json5 --dry-run openclaw config patch --file ./slack.socket.patch.json5
Env fallback (default account only):
bash Copy code SLACK_APP_TOKEN=slack-app-token-example SLACK_BOT_TOKEN=slack-bot-token-example
Start gateway
bash Copy code openclaw gateway
HTTP Request URLs
...
bash Copy code export SLACK_BOT_TOKEN=slack-bot-token-example export SLACK_SIGNING_SECRET=... cat > slack.http.patch.json5 << 'JSON5' { channels: { slack: { enabled: true , mode: "http" , botToken: { source : "env" , provider: "default" , id : "SLACK_BOT_TOKEN" }, signingSecret: { source : "env" , provider: "default" , id : "SLACK_SIGNING_SECRET" }, webhookPath: "/slack/events" , }, }, } JSON5 openclaw config patch --file ./slack.http.patch.json5 --dry-run openclaw config patch --file ./slack.http.patch.json5 Note
...
json5 Copy code { channels : { slack : { mode : "socket" , socketMode : { clientPingTimeout : 20000 , serverPingTimeout : 30000 , pingPongLoggingEnabled : false , }, }, }, }
...
json Copy code { "slash_commands" : [ { "command" : "/new" , "description" : "Start a new session" , "usage_hint" : "[model]" , "url" : "https://gateway-host.example.com/slack/events" } , { "command" : "/help" , "description" : "Show the short help summary" , "url" : "https://gateway-host.example.com/slack/events" } ] }
...
Group Default messages enabled reactions enabled pins enabled memberInfo enabled emojiList enabled
...
Correct:
json5 Copy code { channels : { slack : { groupPolicy : "allowlist" , channels : { C12345678 : { allow : true , requireMention : true }, }, }, }, }
Incorrect (silently blocked under groupPolicy: "allowlist" ):
json5 Copy code { channels : { slack : { groupPolicy : "allowlist" , channels : { "#eng-my-channel" : { allow : true , requireMention : true }, }, }, }, }
...
json5 Copy code { messages : { ackReaction : "eyes" , ackReactionScope : "all" , // react in DMs and groups }, }
Text streaming
...
json Copy code { "channels" : { "slack" : { "streaming" : { "mode" : "progress" , "progress" : { "toolProgress" : true , "commandText" : "status" } } } } }
...
json5 Copy code { channels : { slack : { streaming : { mode : "partial" , nativeTransport : false , }, }, }, }
Opt in to Slack native progress task cards:
json5 Copy code { channels : { slack : { streaming : { mode : "progress" , progress : { nativeTaskCards : true , render : "rich" , }, }, }, }, }
Legacy keys:
...
Enable it globally:
json5 Copy code { channels : { slack : { capabilities : { interactiveReplies : true , }, }, }, }
Or enable it for one Slack account only:
json5 Copy code { channels : { slack : { accounts : { ops : { capabilities : { interactiveReplies : true , }, }, }, }, }, }
...
json5 Copy code { commands : { ownerAllowFrom : [ "slack:U12345678" ], }, }
...
json5 Copy code { channels : { slack : { execApprovals : { enabled : true , approvers : [ "U12345678" ], target : "both" , }, }, }, }
...
json5 Copy code { messages : { groupChat : { visibleReplies : "automatic" , }, }, }
Useful commands:
bash Copy code openclaw channels status --probe openclaw logs --follow openclaw doctor DM messages ignored
Check:
...
bash Copy code openclaw pairing list slack Socket mode not connecting
...
Image-capable model/tool paths can use image attachments from that context.
Non-image files remain available as file metadata or media references for tools that can handle them.
