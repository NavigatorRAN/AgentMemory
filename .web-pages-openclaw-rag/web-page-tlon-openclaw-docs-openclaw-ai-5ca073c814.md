...
- Requested URL: https://docs.openclaw.ai/channels/tlon
- Final URL: https://docs.openclaw.ai/channels/tlon
- Canonical URL: https://docs.openclaw.ai/channels/tlon
- Fetched at: 2026-06-23T14:36:39Z
...
bash Copy code openclaw plugins install @openclaw/tlon
Use the bare package to follow the current official release tag. Pin an exact
version only when you need a reproducible install.
Local checkout (when running from a git repo):
bash Copy code openclaw plugins install ./path/to/local/tlon-plugin
Details: Plugins
Setup
...
json5 Copy code { channels : { tlon : { enabled : true , ship : "~sampel-palnet" , url : "https://your-ship-host" , code : "lidlut-tabwed-pillex-ridrup" , ownerShip : "~your-main-ship" , // recommended: your ship, always allowed }, }, }
Private/LAN ships
...
json5 Copy code { channels : { tlon : { url : "http://localhost:8080" , allowPrivateNetwork : true , }, }, }
...
json5 Copy code { channels : { tlon : { groupChannels : [ "chat/~host-ship/general" , "chat/~host-ship/support" ], }, }, }
Disable auto-discovery:
json5 Copy code { channels : { tlon : { autoDiscoverChannels : false , }, }, }
Access control
DM allowlist (empty = no DMs allowed, use ownerShip for approval flow):
json5 Copy code { channels : { tlon : { dmAllowlist : [ "~zod" , "~nec" ], }, }, }
Group authorization (restricted by default):
json5 Copy code { channels : { tlon : { defaultAuthorizedShips : [ "~zod" ], authorization : { channelRules : { "chat/~host-ship/general" : { mode : "restricted" , allowedShips : [ "~zod" , "~nec" ], }, "chat/~host-ship/announcements" : { mode : "open" , }, }, }, }, }, }
Owner and approval system
Set an owner ship to receive approval requests when unauthorized users try to interact:
json5 Copy code { channels : { tlon : { ownerShip : "~your-main-ship" , }, }, }
...
json5 Copy code { channels : { tlon : { autoAcceptDmInvites : true , }, }, }
Auto-accept group invites from trusted ships:
json5 Copy code { channels : { tlon : { autoAcceptGroupInvites : true , groupInviteAllowlist : [ "~zod" ], }, }, }
...
Capabilities
Feature Status Direct messages ✅ Supported Groups/channels ✅ Supported (mention-gated by default) Threads ✅ Supported (auto-replies in thread) Rich text ✅ Markdown converted to Tlon format Images ✅ Uploaded to Tlon storage Reactions ✅ Via bundled skill Polls ❌ Not yet supported Native commands ✅ Supported (owner-only by default)
Troubleshooting
Run this ladder first:
bash Copy code openclaw status openclaw gateway status openclaw logs --follow openclaw doctor
Common failures:
