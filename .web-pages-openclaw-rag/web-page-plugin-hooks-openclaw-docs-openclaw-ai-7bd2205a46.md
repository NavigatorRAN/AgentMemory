...
- Requested URL: https://docs.openclaw.ai/plugins/hooks
- Final URL: https://docs.openclaw.ai/plugins/hooks
- Canonical URL: https://docs.openclaw.ai/plugins/hooks
- Fetched at: 2026-06-23T14:50:18Z
...
json Copy code { "plugins" : { "entries" : { "my-plugin" : { "hooks" : { "timeoutMs" : 30000 , "timeouts" : { "before_prompt_build" : 90000 , "agent_end" : 60000 } } } } } }
...
to the matching app-server approval request; see Codex harness runtime .
...
ts Copy code declare module "openclaw/plugin-sdk/channel-inbound" { interface PluginHookChannelSenderContext { unionId ?: string ; userId ?: string ; } }
Channel plugins pass those fields through the inbound SDK helper:
ts Copy code buildChannelInboundEventContext ({ // ... channelContext : { sender : { id : senderOpenId, unionId, userId }, chat : { id : chatId }, }, });
...
typescript Copy code type BeforeAgentFinalizeRetry = { instruction : string ; idempotencyKey ?: string ; maxAttempts ?: number ; };
...
json Copy code { "plugins" : { "entries" : { "my-plugin" : { "hooks" : { "allowConversationAccess" : true } } } } }
