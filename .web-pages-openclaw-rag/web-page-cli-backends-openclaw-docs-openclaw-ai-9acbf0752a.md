...
- Requested URL: https://docs.openclaw.ai/gateway/cli-backends
- Final URL: https://docs.openclaw.ai/gateway/cli-backends
- Canonical URL: https://docs.openclaw.ai/gateway/cli-backends
- Fetched at: 2026-06-23T14:43:32Z
...
ACP Agents instead. CLI backends are not ACP.
Tip
Building a new backend plugin? Use
CLI backend plugins . This page is for users
...
bash Copy code openclaw agent --agent main --message "hi" --model claude-cli/claude-sonnet-4-6
...
command path:
json5 Copy code { agents : { defaults : { cliBackends : { "claude-cli" : { command : "/opt/homebrew/bin/claude" , }, }, }, }, }
...
json5 Copy code { agents : { defaults : { model : { primary : "anthropic/claude-opus-4-6" , fallbacks : [ "claude-cli/claude-sonnet-4-6" ], }, models : { "anthropic/claude-opus-4-6" : { alias : "Opus" }, "claude-cli/claude-sonnet-4-6" : {}, }, }, }, }
Notes:
...
bash Copy code claude auth login claude auth status --text openclaw models auth login --provider anthropic --method cli --set-default
...
json5 Copy code imageArg : "--image" , imageMode : "repeat"
...
typescript Copy code api. registerCliBackend ({ id : "my-cli" , ownsNativeCompaction : true /* ... */ });
