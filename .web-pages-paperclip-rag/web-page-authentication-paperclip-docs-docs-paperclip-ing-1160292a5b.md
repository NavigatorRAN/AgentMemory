# Authentication | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/reference/api/authentication
- Final URL: https://docs.paperclip.ing/reference/api/authentication/
- Canonical URL: https://docs.paperclip.ing/reference/api/authentication
- Fetched at: 2026-06-23T13:39:07Z
...
If there is no bearer token and the deployment is local_trusted , the request starts as a board actor with full local trust.
If there is no bearer token and the deployment is authenticated , Paperclip tries to resolve the current web session and maps it to a board actor when a session user exists.
...
If it still does not match, Paperclip tries a local agent JWT.
