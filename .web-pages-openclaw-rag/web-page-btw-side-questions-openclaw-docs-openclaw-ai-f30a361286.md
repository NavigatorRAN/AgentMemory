...
- Requested URL: https://docs.openclaw.ai/tools/btw
- Final URL: https://docs.openclaw.ai/tools/btw
- Canonical URL: https://docs.openclaw.ai/tools/btw
- Fetched at: 2026-06-23T14:57:33Z
...
What it does
When you send:
text Copy code /btw what changed?
OpenClaw:
snapshots the current session context,
runs a separate ephemeral side query,
answers only the side question,
leaves the main run alone,
does not write the BTW question or answer to session history,
emits the answer as a live side result rather than a normal assistant message.
...
Examples:
text Copy code /btw what file are we editing? /side what changed while the main run continued? /btw what does this error mean? /btw summarize the current task in one sentence /btw what is 17 * 19?
When not to use BTW
