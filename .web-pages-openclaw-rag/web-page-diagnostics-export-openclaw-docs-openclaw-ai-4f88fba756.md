...
- Requested URL: https://docs.openclaw.ai/gateway/diagnostics
- Final URL: https://docs.openclaw.ai/gateway/diagnostics
- Canonical URL: https://docs.openclaw.ai/gateway/diagnostics
- Fetched at: 2026-06-23T14:43:41Z
...
Quick start
bash Copy code openclaw gateway diagnostics export
The command prints the written zip path. To choose a path:
bash Copy code openclaw gateway diagnostics export --output openclaw-diagnostics.zip
For automation:
bash Copy code openclaw gateway diagnostics export --json
Chat command
...
Send /diagnostics in the conversation where you noticed the problem. Add a
short note if it helps, for example /diagnostics bad tool choice .
OpenClaw sends the diagnostics preamble and asks for one explicit exec
approval. The approval runs openclaw gateway diagnostics export --json .
Do not approve diagnostics through an allow-all rule.
After approval, OpenClaw replies with a pasteable report containing the local
...
bash Copy code openclaw gateway stability openclaw gateway stability -- type payload.large openclaw gateway stability --json
Inspect the newest persisted stability bundle after a fatal exit, shutdown
timeout, or restart startup failure:
bash Copy code openclaw gateway stability --bundle latest
Create a diagnostics zip from the newest persisted bundle:
bash Copy code openclaw gateway stability --bundle latest -- export
Persisted bundles live under ~/.openclaw/logs/stability/ when events exist.
Useful options
bash Copy code openclaw gateway diagnostics export \ --output openclaw-diagnostics.zip \ --log-lines 5000 \ --log-bytes 1000000
...
json5 Copy code { diagnostics : { enabled : false , }, }
...
json5 Copy code { diagnostics : { memoryPressureSnapshot : true , }, }
