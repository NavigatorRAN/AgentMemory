# ACPX Local | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/reference/adapters/acpx-local
- Final URL: https://docs.paperclip.ing/reference/adapters/acpx-local/
- Canonical URL: https://docs.paperclip.ing/reference/adapters/acpx-local
- Fetched at: 2026-06-23T13:38:39Z
- Content type: text/html; charset=utf-8

## Description

acpx local runs an agent through the Agent Client Protocol via ACPX on the Paperclip host or a managed execution environment. Use it when you want a single built-in adapter that ca
...
⚠ ACPX runtime execution is still being enabled in stages. For today's stable Claude Code or Codex CLI wrapper behaviour, use Claude Local or Codex Local .
When To Use
...
Common Fields
Field Required Default Notes agent no claude claude , codex , or custom . agentCommand no — Custom ACP command when agent=custom , or an override for a built-in ACP agent command. mode no persistent persistent or oneshot . Paperclip keeps session state persistent and may close the live process between runs. cwd no — Default absolute working directory fallback for the agent process. permissionMode no approve-all ACPX permission policy. approve-all auto-approves ACPX permission requests. nonInteractivePermissions no deny Policy when no operator is available to answer a permission prompt. timeoutSec no 0 Run timeout in seconds. 0 means no timeout. warmHandleIdleMs no 0 Idle window before Paperclip releases a warm ACPX handle. 0 keeps the warm handle disabled.
...
Related
Claude Local — stable Claude Code CLI wrapper.
Codex Local — stable Codex CLI wrapper.
