...
- Requested URL: https://docs.litellm.ai/docs/pass_through/bedrock
- Final URL: https://docs.litellm.ai/docs/pass_through/bedrock
- Canonical URL: https://docs.litellm.ai/docs/pass_through/bedrock
- Fetched at: 2026-06-23T14:25:49Z
...
Copy as Markdown
Pass-through endpoints for Bedrock - call provider-specific endpoint, in native format (no translation).
Feature Supported Notes Cost Tracking ✅ For /invoke and /converse endpoints Load Balancing ✅ You can load balance /invoke , /converse routes across multiple deployments End-user Tracking ❌ Tell us if you need this Streaming ✅
...
Create a config.yaml file with your Bedrock model
...
Endpoint Description Example /model/{model_name}/converse Converse API http://0.0.0.0:4000/bedrock/model/my-claude-model/converse /model/{model_name}/converse-stream Streaming Converse http://0.0.0.0:4000/bedrock/model/my-claude-model/converse-stream /model/{model_name}/invoke Legacy Invoke API http://0.0.0.0:4000/bedrock/model/my-claude-model/invoke /model/{model_name}/invoke-with-response-stream Legacy Streaming http://0.0.0.0:4000/bedrock/model/my-claude-model/invoke-with-response-stream
...
Key Changes:
Original Endpoint Replace With https://bedrock-runtime.{aws_region_name}.amazonaws.com http://0.0.0.0:4000/bedrock (LITELLM_PROXY_BASE_URL=" http://0.0.0.0:4000 ") AWS4-HMAC-SHA256.. Bearer anything (use Bearer LITELLM_VIRTUAL_KEY if Virtual Keys are setup on proxy)
...
You can use the LangChain AWS SDK with LiteLLM Proxy to get cost tracking, load balancing, and other LiteLLM features.
...
Feature Supported Notes Text Generation ✅ Full support Streaming ✅ Use stream() method Document Processing ✅ PDF, images, etc. Citations ✅ Enable in document config Tool Use ✅ Function calling support Multi-modal ✅ Text + images + documents
Troubleshooting ​
