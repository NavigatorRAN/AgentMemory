# Agent Iteration Budgets | liteLLM

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.litellm.ai/docs/a2a_iteration_budgets
- Final URL: https://docs.litellm.ai/docs/a2a_iteration_budgets
- Canonical URL: https://docs.litellm.ai/docs/a2a_iteration_budgets
- Fetched at: 2026-06-23T14:22:33Z
...
Control Description Max Iterations Hard cap on the number of LLM calls per session Max Budget Per Session Dollar cap per session (identified by x-litellm-trace-id )
...
Configure your agent (name, model, etc.)
In Agent Settings , expand the Tracing section
Toggle on Require x-litellm-trace-id on calls BY this agent
Set Max Iterations to 25
Set Max Budget Per Session to 5.00
Proceed to create a new key for the agent
Click Create Agent
...
Variable Default Description LITELLM_MAX_ITERATIONS_TTL 3600 (1 hour) TTL in seconds for session iteration counters LITELLM_MAX_BUDGET_PER_SESSION_TTL 3600 (1 hour) TTL in seconds for session budget counters
Overview
Trace-ID Enforcement
Configuring via UI
Configuring via API
