# Require board approval before an agent spends money | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/how-to/require-board-approval-before-spend
- Final URL: https://docs.paperclip.ing/how-to/require-board-approval-before-spend/
- Canonical URL: https://docs.paperclip.ing/how-to/require-board-approval-before-spend
- Fetched at: 2026-06-23T13:38:28Z
- Content type: text/html; charset=utf-8

## Description

Budget caps are reactive. They warn at 80% and hard-stop at 100% — useful for bounding the runaway case, useless for "don't sign up for this $40/month SaaS without asking me first.
...
Concern Hard budget cap (HT1) Spend approval (this guide) Trigger Cumulative spend crosses a percent threshold Agent decides an action exceeds policy Direction Reactive — fires after the dollars are spent Proactive — fires before the action happens Granularity Aggregate per scope (company, agent, project) per month Per-decision, with the proposal in the payload What stops The whole scope pauses (no more heartbeats) Only the specific spend; the agent keeps working other tasks Who acts Board resolves the incident on the Costs page Board approves/rejects on the Approvals page
...
Already over a hard cap? That generates a budget_override_required approval automatically — see Set a monthly budget and enforce it for that flow. The gate this guide describes is the agent-initiated request_board_approval , which is a different pattern.
...
For the full UI walkthrough, including queue filters and the comments thread, see Approvals (board guide) .
...
Var Value PAPERCLIP_APPROVAL_ID UUID of the resolved approval. PAPERCLIP_APPROVAL_STATUS approved or rejected .
...
For BYO-agent runtimes that don't get Paperclip-driven wakes at all (the polling pattern in Bring your own agent → Option C ), poll the approval directly: GET /api/approvals/$APPROVAL_ID returns the current status. Loop with backoff and bail when status != "pending" .
...
Make sure the agent's instructions name the gate. Open the agent → Instructions , paste a minimal version of the rule from Section 2, save.
Trigger the spend. Assign the agent an issue that should hit the gate ("Sign up for the demo SaaS account"). Run a heartbeat — either let one fire on schedule or click Run Heartbeat in the agent header.
Confirm the approval lands. Open Approvals → Pending . You should see the request, with the proposal payload your agent submitted. Confirm issueIds links to the source issue.
Approve from the API. Use the /approve route from Section 4 against the approval id from step 3. The agent should be woken within seconds.
Confirm the agent resumed. Reopen the source issue. The status should be back to in_progress (or whatever your wake handler sets) and a new comment should be present, citing the approval id and stating what happened. Re-run with /reject to confirm the rejection branch behaves the same way.
...
Combine with budget caps, don't replace them. The approval gate is enforced by the agent's own instructions. A new agent imported from another company, or a model that ignored its instructions, can bypass the gate. The hard cap from HT1 catches the bypass and pauses the scope. Run both.
...
Notifications. The Approvals queue does not push to Slack or Discord on its own. If your board lives in chat and the queue would otherwise sit, wire a notification routine — the same shape as the hire-approval notifier.
See also
Set a monthly budget and enforce it — the reactive half of spend control. Run alongside this gate.
Approvals (board guide) — the full UI walkthrough for the queue and decision buttons.
Approvals API — every endpoint touched here, plus the lifecycle diagram and resubmit semantics.
Handle board approvals for hires — the same governance flow for the hire_agent approval type.
Wire Slack/Discord notifications — pipe approvals into a channel so the queue doesn't sit.
Debug a stuck heartbeat — first stop when the wake fires but the agent doesn't pick the approval up.
