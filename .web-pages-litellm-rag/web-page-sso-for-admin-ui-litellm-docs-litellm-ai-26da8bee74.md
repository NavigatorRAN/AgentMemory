...
- Requested URL: https://docs.litellm.ai/docs/proxy/admin_ui_sso
- Final URL: https://docs.litellm.ai/docs/proxy/admin_ui_sso
- Canonical URL: https://docs.litellm.ai/docs/proxy/admin_ui_sso
- Fetched at: 2026-06-23T14:29:35Z
...
Ensure users are assigned to the app in the Assignments tab. If Federation Broker Mode is enabled, you may need to disable it to assign users manually. Step 3: Set Environment Variables ​
...
Go to Security → API
Select the default authorization server (or your custom one)
Click on Access Policies tab, create a new policy assigned to your LiteLLM app
Add a rule that allows the Authorization Code grant type
See Okta's Access Policy documentation for more details. Step 4: Configure Okta Security Settings ​
...
LiteLLM will automatically handle PKCE parameter generation and verification during the OAuth flow. Step 5: Test the SSO Flow ​
Start your LiteLLM proxy
Navigate to https://<your-proxy-base-url>/ui
Click the SSO login button
Authenticate with Okta and verify you're redirected back to LiteLLM
Troubleshooting ​ Error Cause Solution redirect_uri error Redirect URI not configured Add <proxy_base_url>/sso/callback to Sign-in redirect URIs in Okta access_denied User not assigned to app Assign the user in the Assignments tab no_matching_policy Missing Access Policy (Custom Authorization Server only) Create an Access Policy in the Authorization Server (see Step 3a)
...
Use one of the supported role names above (e.g., proxy_admin )
Assign users to these roles in your Enterprise Application
...
Add /sso/debug/callback as a redirect URL in your Azure App Registration
Navigate to https://<proxy_base_url>/sso/debug/login
Complete the SSO flow to see the returned user attributes
...
For CLI SSO , you can map the same (or other) claims into user metadata and return scalars to the CLI via CLI_SSO_CLAIM_MAP — see CLI Authentication .
...
See Okta SSO — Step 4: Configure Okta Security Settings for details on GENERIC_CLIENT_STATE and PKCE configuration.
...
Add /sso/debug/callback as a redirect URL in your SSO provider
...
Select the user
Under Select a role , choose the app role you created (e.g., proxy_admin_viewer )
