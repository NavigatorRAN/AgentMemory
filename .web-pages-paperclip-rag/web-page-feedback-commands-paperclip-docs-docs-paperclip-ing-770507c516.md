# Feedback Commands | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/reference/cli/feedback
- Final URL: https://docs.paperclip.ing/reference/cli/feedback/
- Canonical URL: https://docs.paperclip.ing/reference/cli/feedback
- Fetched at: 2026-06-23T13:39:38Z
- Content type: text/html; charset=utf-8

## Description

Use these commands when you want to inspect, summarize, or extract the thumbs-up / thumbs-down feedback that gets recorded against a company's work — agent outputs, comments, and o
...
Command Summary
Command What it does feedback report Render a terminal report of a company's feedback traces, with a summary and per-trace details. feedback export Write votes plus raw trace bundles into a folder and a .zip archive. feedback trace <traceId> Fetch a single feedback trace by ID. feedback bundle <traceId> Fetch a single feedback trace's bundle (the trace plus its attached files).
All four subcommands accept the common client options described in Common Options : --data-dir , --api-base , --api-key , --context , --profile , and --json . report and export are company-scoped and additionally take -C, --company-id <id> .
...
Flag Use --target-type <type> Only include traces against this target type (for example a comment vs. a work product). --vote <vote> Only include traces with this vote value ( up or down ). --status <status> Only include traces in this export status (see the status table below). --project-id <id> Only include traces tied to this project. --issue-id <id> Only include traces tied to this issue. --from <iso8601> Only include traces created at or after this timestamp. --to <iso8601> Only include traces created at or before this timestamp. --shared-only Only include traces eligible for sharing/export.
...
Status Meaning pending Recorded and awaiting export/share. sent Already shared/exported off the instance. local_only Kept on this instance; not eligible to leave it. failed An export/share attempt failed.
Report
...
Path Contents index.json A manifest: when it was exported, the server URL, the company ID, a summary (counts plus uniqueIssues and the sorted list of issue identifiers), and the list of files written. votes/ One JSON file per trace holding a flattened vote record (vote, target, status, consent version, timestamps, and the reason). traces/ One JSON file per trace with the full trace object. full-traces/<issue>-<trace>/ One directory per trace containing bundle.json and every file from that trace's bundle, written out at its original relative path. <output-dir>.zip A stored (uncompressed) zip archive of index.json and every file above, rooted at the export folder name.
...
Scripting Notes
Use --json on every subcommand when you are piping into jq or another tool. See Output and Scripting for the conventions.
...
See also
Issue Commands — feedback traces are anchored to issues; use these to inspect the work that was voted on.
Company Commands — feedback is company-scoped; export and portability live here.
Common Options — the shared client flags every feedback subcommand accepts.
Output and Scripting — how --json output is structured for automation.
