...
- Requested URL: https://docs.openclaw.ai/clawhub/http-api
- Final URL: https://docs.openclaw.ai/clawhub/http-api
- Canonical URL: https://docs.openclaw.ai/clawhub/http-api
- Fetched at: 2026-06-23T14:37:01Z
...
http Copy code HTTP/2 429 content-type : text/plain; charset=utf-8 x-ratelimit-limit : 20 x-ratelimit-remaining : 0 x-ratelimit-reset : 1771404540 ratelimit-limit : 20 ratelimit-remaining : 0 ratelimit-reset : 34 retry-after : 34 Rate limit exceeded
Client guidance:
...
Response:
json Copy code { "results" : [ { "score" : 0.123 , "slug" : "gifgrep" , "displayName" : "GifGrep" , "summary" : "…" , "version" : "1.2.3" , "updatedAt" : 1730000000000 , "ownerHandle" : "openclaw" , "owner" : { "handle" : "openclaw" , "displayName" : "OpenClaw" , "image" : "https://example.com/avatar.png" } } ] }
Notes:
...
json Copy code { "items" : [ { "slug" : "gifgrep" , "displayName" : "GifGrep" , "summary" : "…" , "topics" : [ "Productivity" ] , "tags" : { "latest" : "1.2.3" } , "stats" : { } , "createdAt" : 0 , "updatedAt" : 0 , "latestVersion" : { "version" : "1.2.3" , "createdAt" : 0 , "changelog" : "…" } , "metadata" : { "os" : [ "macos" ] , "systems" : [ "aarch64-darwin" ] } } ] , "nextCursor" : null }
GET /api/v1/skills/{slug}
json Copy code { "skill" : { "slug" : "gifgrep" , "displayName" : "GifGrep" , "summary" : "…" , "topics" : [ "Productivity" ] , "tags" : { "latest" : "1.2.3" } , "stats" : { } , "createdAt" : 0 , "updatedAt" : 0 } , "latestVersion" : { "version" : "1.2.3" , "createdAt" : 0 , "changelog" : "…" } , "metadata" : { "os" : [ "macos" ] , "systems" : [ "aarch64-darwin" ] } , "owner" : { "handle" : "steipete" , "displayName" : "Peter" , "image" : null } , "moderation" : { "isSuspicious" : false , "isMalwareBlocked" : false , "verdict" : "clean" , "reasonCodes" : [ ] , "summary" : null , "engineVersion" : "v2.0.0" , "updatedAt" : 0 } }
...
json Copy code { "source" : { "kind" : "published" , "slug" : "gifgrep" , "version" : "1.2.3" } , "update" : false }
...
json Copy code { "items" : [ { "slug" : "gifgrep" , "version" : "1.2.3" } ] }
...
required.
json Copy code { "package" : { "name" : "@openclaw/example-plugin" , "displayName" : "Example Plugin" , "family" : "code-plugin" } , "release" : { "releaseId" : "packageReleases:..." , "version" : "1.2.3" , "artifactKind" : "npm-pack" , "artifactSha256" : "0123456789abcdef..." , "npmIntegrity" : "sha512-..." , "npmShasum" : "0123456789abcdef0123456789abcdef01234567" , "npmTarballName" : "example-plugin-1.2.3.tgz" , "createdAt" : 1730000000000 } , "trust" : { "scanStatus" : "malicious" , "moderationState" : "quarantined" , "blockedFromDownload" : true , "reasons" : [ "manual:quarantined" , "scan:malicious" ] , "pending" : false , "stale" : false } }
Response fields:
...
host targets
scan state
json Copy code { "package" : { "name" : "@openclaw/example-plugin" , "displayName" : "Example Plugin" , "family" : "code-plugin" , "isOfficial" : true , "latestVersion" : "1.2.3" } , "ready" : false , "checks" : [ { "id" : "clawpack" , "label" : "ClawPack artifact" , "status" : "fail" , "message" : "Latest version is legacy ZIP-only." } ] , "blockers" : [ "clawpack" ] }
...
json Copy code { "ok" : true , "reportId" : "packageReports:..." , "packageId" : "packages:..." , "status" : "confirmed" , "reportCount" : 0 }
POST /api/v1/packages/{name}/versions/{version}/moderation
Moderator/admin endpoint for package release review.
json Copy code { "state" : "quarantined" , "reason" : "Suspicious native payload." }
Supported states:
...
json Copy code { "slug" : "gifgrep" , "match" : { "version" : "1.2.2" } , "latestVersion" : { "version" : "1.2.3" } }
GET /api/v1/download
...
Code Copy code Authorization: Bearer clh_...
GET /api/v1/whoami
...
Optional JSON body:
json Copy code { "reason" : "Held for moderation pending legal review." }
...
Delete response:
json Copy code { "ok" : true , "slugReservedUntil" : 1730000000000 }
Status codes:
200 : ok
401 : unauthorized
403 : forbidden
...
Body:
json Copy code { "handle" : "user_handle" , "reason" : "optional ban reason" }
or
json Copy code { "userId" : "users_..." , "reason" : "optional ban reason" }
json Copy code { "ok" : true , "alreadyBanned" : false , "deletedSkills" : 3 }
POST /api/v1/users/unban
Unban a user and restore eligible skills (admin only).
json Copy code { "handle" : "user_handle" , "reason" : "optional unban reason" }
json Copy code { "userId" : "users_..." , "reason" : "optional unban reason" }
json Copy code { "ok" : true , "alreadyUnbanned" : false , "restoredSkills" : 3 }
...
json Copy code { "handle" : "user_handle" , "reason" : "bulk publishing spam" , "dryRun" : true }
json Copy code { "userId" : "users_..." , "reason" : "bulk publishing spam" , "dryRun" : false }
json Copy code { "ok" : true , "dryRun" : false , "userId" : "users_..." , "handle" : "user_handle" , "previousReason" : "malware auto-ban" , "nextReason" : "bulk publishing spam" , "changed" : true }
POST /api/v1/users/role
Change a user role (admin only).
json Copy code { "handle" : "user_handle" , "role" : "moderator" }
json Copy code { "userId" : "users_..." , "role" : "admin" }
json Copy code { "ok" : true , "role" : "moderator" }
GET /api/v1/users
List or search users (admin only).
q (optional): search query
query (optional): alias for q
limit (optional): max results (default 20, max 200)
json Copy code { "items" : [ { "userId" : "users_..." , "handle" : "user_handle" , "displayName" : "User" , "name" : "User" , "role" : "moderator" } ] , "total" : 1 }
POST /api/v1/stars/{slug} / DELETE /api/v1/stars/{slug}
Add/remove a star (highlights). Both endpoints are idempotent.
Responses:
json Copy code { "ok" : true , "starred" : true , "alreadyStarred" : false } json Copy code { "ok" : true , "unstarred" : true , "alreadyUnstarred" : false }
...
Schema:
json Copy code { "apiBase" : "https://clawhub.ai" , "authBase" : "https://clawhub.ai" , "minCliVersion" : "0.0.5" }
