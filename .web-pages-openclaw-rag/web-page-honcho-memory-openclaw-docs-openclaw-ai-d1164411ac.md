...
- Requested URL: https://docs.openclaw.ai/concepts/memory-honcho
- Final URL: https://docs.openclaw.ai/concepts/memory-honcho
- Canonical URL: https://docs.openclaw.ai/concepts/memory-honcho
- Fetched at: 2026-06-23T14:38:49Z
...
Honcho adds AI-native memory to OpenClaw. It persists
...
Tool What it does honcho_context Full user representation across sessions honcho_search_conclusions Semantic search over stored conclusions honcho_search_messages Find messages across sessions (filter by sender, date) honcho_session Current session history and summary
Q&A (LLM-powered):
Tool What it does honcho_ask Ask about the user. depth='quick' for facts, 'thorough' for synthesis
Getting started
Install the plugin and run setup:
bash Copy code openclaw plugins install @honcho-ai/openclaw-honcho openclaw honcho setup openclaw gateway --force
...
json5 Copy code { plugins : { entries : { "openclaw-honcho" : { config : { apiKey : "your-api-key" , // omit for self-hosted workspaceId : "openclaw" , // memory isolation baseUrl : "https://api.honcho.dev" , }, }, }, }, }
...
Builtin / QMD Honcho Storage Workspace Markdown files Dedicated service (local or hosted) Cross-session Via memory files Automatic, built-in User modeling Manual (write to MEMORY.md) Automatic profiles Search Vector + keyword (hybrid) Semantic over observations Multi-agent Not tracked Parent/child awareness Dependencies None (builtin) or QMD binary Plugin install
...
CLI commands
bash Copy code openclaw honcho setup # Configure API key and migrate files openclaw honcho status # Check connection status openclaw honcho ask <question> # Query Honcho about the user openclaw honcho search <query> [-k N] [-d D] # Semantic search over memory
Further reading
Plugin source code
