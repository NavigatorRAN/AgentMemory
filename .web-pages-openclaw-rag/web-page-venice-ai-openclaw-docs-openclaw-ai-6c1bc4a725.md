...
- Requested URL: https://docs.openclaw.ai/providers/venice
- Final URL: https://docs.openclaw.ai/providers/venice
- Canonical URL: https://docs.openclaw.ai/providers/venice
- Fetched at: 2026-06-23T14:56:01Z
...
Mode Description Models Private Fully private. Prompts/responses are never stored or logged . Ephemeral. Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored, etc. Anonymized Proxied through Venice with metadata stripped. The underlying provider (OpenAI, Anthropic, Google, xAI) sees anonymized requests. Claude, GPT, Gemini, Grok
Warning
...
Getting started
Install the plugin
bash Copy code openclaw plugins install @openclaw/venice-provider
Get your API key
...
bash Copy code openclaw onboard --auth-choice venice-api-key
This will:
...
bash Copy code export VENICE_API_KEY= "vapi_xxxxxxxxxxxx"
Non-interactive
bash Copy code openclaw onboard --non-interactive \ --auth-choice venice-api-key \ --venice-api-key "vapi_xxxxxxxxxxxx"
Verify setup
bash Copy code openclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
Model selection
...
bash Copy code openclaw models set venice/kimi-k2-5 openclaw models set venice/claude-opus-4-6
List all available models:
bash Copy code openclaw models list --all --provider venice
...
Feature Support Streaming All models Function calling Most models (check supportsFunctionCalling in API) Vision/Images Models marked with "Vision" feature JSON mode Supported via response_format
Pricing
...
Config file example json5 Copy code { env : { VENICE_API_KEY : "vapi_..." }, agents : { defaults : { model : { primary : "venice/kimi-k2-5" } } }, models : { mode : "merge" , providers : { venice : { baseUrl : "https://api.venice.ai/api/v1" , apiKey : "${VENICE_API_KEY}" , api : "openai-completions" , models : [ { id : "kimi-k2-5" , name : "Kimi K2.5" , reasoning : true , input : [ "text" , "image" ], cost : { input : 0 , output : 0 , cacheRead : 0 , cacheWrite : 0 }, contextWindow : 256000 , maxTokens : 65536 , }, ], }, }, }, }
Related
