# Prompt Security | liteLLM

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.litellm.ai/docs/proxy/guardrails/prompt_security
- Final URL: https://docs.litellm.ai/docs/proxy/guardrails/prompt_security
- Canonical URL: https://docs.litellm.ai/docs/proxy/guardrails/prompt_security
- Fetched at: 2026-06-23T14:30:53Z
- Content type: text/html; charset=utf-8

## Description

Use Prompt Security to protect your LLM applications from prompt injection attacks, jailbreaks, harmful content, PII leakage, and malicious file uploads through comprehensive input and output validation.
...
Extracts the file data from the message
Uploads the file to Prompt Security's sanitization API
Polls the API for sanitization results (with configurable timeout)
Takes action based on the verdict:
...
Best Practices ​
Use during_call mode for comprehensive protection of both inputs and outputs
Enable for production workloads using default_on: true to protect all requests by default
Configure user tracking to identify patterns across user sessions
Monitor violations in Prompt Security dashboard to tune policies
Test file uploads thoroughly with various file types before production deployment
Set appropriate timeouts for file sanitization based on expected file sizes
Combine with other guardrails for defense-in-depth security
Troubleshooting ​
...
Verify that:
Files are base64-encoded in proper data URL format
MIME type is included: data:image/png;base64,...
Content type is image_url , document , or file
High Latency ​
File sanitization adds latency due to upload and polling. To optimize:
Reduce poll_interval for faster polling (but more API calls)
Increase max_poll_attempts for larger files
Consider caching sanitization results for frequently uploaded files
Need Help? ​
Documentation : https://support.prompt.security
