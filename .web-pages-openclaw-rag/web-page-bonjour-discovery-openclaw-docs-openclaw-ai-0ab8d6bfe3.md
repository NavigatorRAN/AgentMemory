...
- Requested URL: https://docs.openclaw.ai/gateway/bonjour
- Final URL: https://docs.openclaw.ai/gateway/bonjour
- Canonical URL: https://docs.openclaw.ai/gateway/bonjour
- Fetched at: 2026-06-23T14:43:30Z
...
json5 Copy code { gateway : { bind : "tailnet" }, // tailnet-only (recommended) discovery : { wideArea : { enabled : true } }, // enables wide-area DNS-SD publishing }
One-time DNS server setup (gateway host)
bash Copy code openclaw dns setup --apply
This installs CoreDNS and configures it to:
listen on port 53 only on the gateway's Tailscale interfaces
serve your chosen domain (example: openclaw.internal. ) from ~/.openclaw/dns/<domain>.db
Validate from a tailnet-connected machine:
bash Copy code dns-sd -B _openclaw-gw._tcp openclaw.internal. dig @<TAILNET_IPV4> -p 53 _openclaw-gw._tcp.openclaw.internal PTR +short
...
bash Copy code dns-sd -L "<instance>" _openclaw-gw._tcp local .
...
bash Copy code openclaw plugins enable bonjour
...
bash Copy code OPENCLAW_DISABLE_BONJOUR=1
...
bash Copy code openclaw plugins disable bonjour
Docker gotchas
...
bash Copy code docker compose config | grep OPENCLAW_DISABLE_BONJOUR
Confirm the Gateway itself is reachable through the published port:
bash Copy code curl -fsS http://127.0.0.1:18789/healthz
...
wide-area DNS-SD
If you deliberately enabled the Bonjour plugin in Docker and forced advertising
