...
- Requested URL: https://docs.openclaw.ai/providers/lmstudio
- Final URL: https://docs.openclaw.ai/providers/lmstudio
- Canonical URL: https://docs.openclaw.ai/providers/lmstudio
- Fetched at: 2026-06-23T14:55:31Z
...
Quick start
Install LM Studio (desktop) or llmster (headless), then start the local server:
bash Copy code curl -fsSL https://lmstudio.ai/install.sh | bash
Start the server
Make sure you either start the desktop app or run the daemon using the following command:
bash Copy code lms daemon up bash Copy code lms server start --port 1234
...
If LM Studio authentication is enabled, set LM_API_TOKEN :
bash Copy code export LM_API_TOKEN= "your-lm-studio-api-token"
...
Run onboarding and choose LM Studio :
bash Copy code openclaw onboard
In onboarding, use the Default model prompt to pick your LM Studio model.
You can also set or change it later:
bash Copy code openclaw models set lmstudio/qwen/qwen3.5-9b
...
bash Copy code openclaw onboard \ --non-interactive \ --accept-risk \ --auth-choice lmstudio
Or specify the base URL, model, and optional API key:
bash Copy code openclaw onboard \ --non-interactive \ --accept-risk \ --auth-choice lmstudio \ --custom-base-url http://localhost:1234/v1 \ --lmstudio-api-key " $LM_API_TOKEN " \ --custom-model-id qwen/qwen3.5-9b
...
json5 Copy code { models : { providers : { lmstudio : { baseUrl : "http://localhost:1234/v1" , apiKey : "${LM_API_TOKEN}" , api : "openai-completions" , models : [ { id : "qwen/qwen3-coder-next" , name : "Qwen 3 Coder Next" , reasoning : false , input : [ "text" ], cost : { input : 0 , output : 0 , cacheRead : 0 , cacheWrite : 0 }, contextWindow : 128000 , maxTokens : 8192 , }, ], }, }, }, }
Troubleshooting
LM Studio not detected
Make sure LM Studio is running. If authentication is enabled, also set LM_API_TOKEN :
bash Copy code # Start via desktop app, or headless: lms server start --port 1234
Verify the API is accessible:
bash Copy code curl http://localhost:1234/api/v1/models
...
json5 Copy code { models : { providers : { lmstudio : { baseUrl : "http://localhost:1234/v1" , api : "openai-completions" , params : { preload : false }, models : [{ id : "qwen/qwen3.5-9b" }], }, }, }, }
...
json5 Copy code { models : { providers : { lmstudio : { baseUrl : "http://gpu-box.local:1234/v1" , apiKey : "lmstudio" , api : "openai-completions" , models : [{ id : "qwen/qwen3.5-9b" }], }, }, }, }
