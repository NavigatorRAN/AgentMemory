# Connect an AWS Secrets Manager vault | Paperclip Docs

Source-backed web page detail staged by AgentMemory bulk web importer.

- Requested URL: https://docs.paperclip.ing/how-to/connect-aws-secrets-vault
- Final URL: https://docs.paperclip.ing/how-to/connect-aws-secrets-vault/
- Canonical URL: https://docs.paperclip.ing/how-to/connect-aws-secrets-vault
- Fetched at: 2026-06-23T13:38:21Z
- Content type: text/html; charset=utf-8

## Description

Wire AWS Secrets Manager into Paperclip's Secrets page so it can discover the vaults you already use — region, namespace, name prefix — instead of asking you to copy ARNs by hand.
...
If you only need the conceptual model — strict mode, the local encrypted provider, environment overrides — read Secrets first. This page is the operator setup walkthrough.
Before you start
...
Sign in and open Company Settings → Secrets .
Switch to the Provider vaults tab. You should see one section per provider; the AWS Secrets Manager section is the one we want.
Click Add vault on the AWS row.
...
Leave Provider set to AWS Secrets Manager.
Type a Display name that will appear in the vault picker on each secret, for example prod-us-east-1 .
Fill AWS region with the region you want to inspect, for example us-east-1 . Region is the only required AWS field — discovery is disabled until it's set.
Optionally fill Namespace and Secret name prefix if you already know the slash-separated path you organize secrets under. Discovery uses whatever you typed as a starting query but works fine with the region alone.
Click Find existing AWS values .
...
On the Provider vaults tab, find the card and use its menu to Remove from Paperclip .
The confirmation dialog spells out the consequences for AWS specifically: "This does not delete the remote AWS Secrets Manager vault, secrets, or any AWS data." It only drops the routing metadata Paperclip stored.
Click Remove from Paperclip .
...
Related
Secrets — the secret-store model, strict mode, and how secret refs resolve at runtime.
Update or rotate a provider API key — Path B uses provider vaults like the one created above.
