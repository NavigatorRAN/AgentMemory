...
- Requested URL: https://hermes-agent.nousresearch.com/docs/integrations/providers
- Final URL: https://hermes-agent.nousresearch.com/docs/integrations/providers
- Canonical URL: https://hermes-agent.nousresearch.com/docs/integrations/providers
- Fetched at: 2026-06-23T14:15:29Z
...
Command Where to run What it does hermes model Your terminal (outside any session) Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints /model Inside a Hermes chat session Quick switch between already-configured providers and models
...
Type Prefix How to get OAuth token gho_ hermes model → GitHub Copilot → Login with GitHub Fine-grained PAT github_pat_ GitHub Settings → Developer settings → Fine-grained tokens (needs Copilot Requests permission) GitHub App token ghu_ Via GitHub App installation
...
Re-resolve token via the normal priority chain ( COPILOT_GITHUB_TOKEN → GH_TOKEN → GITHUB_TOKEN → gh auth token )
Rebuild the shared OpenAI client with refreshed headers
...
provider : "copilot"
default : "gpt-5.4"
Environment variable Description COPILOT_GITHUB_TOKEN GitHub token for Copilot API (first priority) HERMES_COPILOT_ACP_COMMAND Override the Copilot CLI binary path (default: copilot ) HERMES_COPILOT_ACP_ARGS Override ACP args (default: --acp --stdio )
...
Available VRAM Default context Less than 24 GB 4,096 tokens 24–48 GB 32,768 tokens 48+ GB 256,000 tokens
...
Flag Purpose --enable-auto-tool-choice Required for tool_choice: "auto" (the default in Hermes) --tool-call-parser <name> Parser for the model's tool call format
...
Restart the Ollama service (or reboot)
Windows Firewall ​
...
Context exhaustion — The model filled its context window. Increase model.context_length or enable context compression in Hermes.
...
Routing profiles:
Profile Strategy Savings blockrun/auto Balanced quality/cost 74-100% blockrun/eco Cheapest possible 95-100% blockrun/premium Best quality models 0% blockrun/free Free models only 100% blockrun/agentic Optimized for tool use varies
note
...
The cloud providers listed in Other Compatible Providers all speak OpenAI's REST dialect, so they wire up the same way under custom_providers: . Three worked recipes follow. Each drops into ~/.hermes/config.yaml and the matching API key goes in ~/.hermes/.env .
Together AI ​
