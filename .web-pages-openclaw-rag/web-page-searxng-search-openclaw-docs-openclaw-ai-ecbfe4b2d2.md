...
- Requested URL: https://docs.openclaw.ai/tools/searxng-search
- Final URL: https://docs.openclaw.ai/tools/searxng-search
- Canonical URL: https://docs.openclaw.ai/tools/searxng-search
- Fetched at: 2026-06-23T14:58:13Z
...
Setup
Install the plugin
bash Copy code openclaw plugins install @openclaw/searxng-plugin
Run a SearXNG instance
bash Copy code docker run -d -p 8888:8080 searxng/searxng
Or use any existing SearXNG deployment you have access to. See the
SearXNG documentation for production setup.
Configure
bash Copy code openclaw configure --section web # Select "searxng" as the provider
Or set the env var and let auto-detection find it:
bash Copy code export SEARXNG_BASE_URL= "http://localhost:8888"
Config
json5 Copy code { tools : { web : { search : { provider : "searxng" , }, }, }, }
Plugin-level settings for the SearXNG instance:
json5 Copy code { plugins : { entries : { searxng : { config : { webSearch : { baseUrl : "http://localhost:8888" , categories : "general,news" , // optional language : "en" , // optional }, }, }, }, }, }
...
Field Description baseUrl Base URL of your SearXNG instance (required) categories Comma-separated categories such as general , news , or science language Language code for results such as en , de , or fr
Notes
